	...
	*/
	{
		...
		*/
		...
		if ( !TxSuccess && ( pTxInfo->TxCount > 0 ) )/* all packet send failed */
		{
			...
			tr_Entry->ContinueTxFailCnt += pTxInfo->TxFailCount;
...
VOID MtAsicMcsLutUpdate(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():WCID=%d, HTPhyMode=0x%x\n",
				__FUNCTION__, pEntry->wcid, pEntry->HTPhyMode.word));

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\tCurTxRateIdx=%d, Mode/BW/MCS/STBC/SGI=%d/%d/%d/%d/%d\n\n",
...
VOID MtAsicDelWcidTab(RTMP_ADAPTER *pAd, UCHAR wcid_idx)
{
    ...
    for (cnt = cnt_s; cnt_s <= cnt_e; cnt_s++)
    {
		...
		MAC_IO_READ32(pAd, tb_entry.wtbl_addr[0] + 8, &Value);
#ifdef RT_BIG_ENDIAN
		Value = Value & 0xF << 25;
#else
		Value = Value & 0xF << 3;
