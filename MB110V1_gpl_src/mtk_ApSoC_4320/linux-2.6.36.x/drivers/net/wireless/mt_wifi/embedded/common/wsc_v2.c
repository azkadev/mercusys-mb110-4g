	...
	*/
	...
	if (TotalPinAttackCount >= WSC_WPS_AP_MAX_PIN_ATTACK)
	{
		...
		if (TotalPinAttackCount >= WSC_LOCK_FOREVER_PIN_ATTACK)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("2.4G WPS locked forever!\n"));
		}
		/* locked for minute */
		else
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("2.4G WPS locked for %d minutes!\n", pWscControl->SetupLockTime));
		}
