...
static VOID CmdPsRetrieveStartRspFromCR(RTMP_ADAPTER *pAd, char *Data, UINT16 Len)
{
   ...
   WlanIdx = le2cpu32(EvtPsRetrieveStart->u4Param1);
	...
	if (!(VALID_WCID(WlanIdx))) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("---->%s INVALID_MAC_WCID(%d)\n", __FUNCTION__, WlanIdx));
		goto NEXT;
   }
	...
	if (IS_ENTRY_NONE(pEntry))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("---->%s Entry(wcid=%d) left.\n", __FUNCTION__, WlanIdx));
		goto NEXT;
	}

	if (!(VALID_TR_WCID(WlanIdx))) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("---->%s INVALID_TR_WCID(%d)\n", __FUNCTION__, WlanIdx));
		goto NEXT;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("---->%s Entry(wcid=%d) ps state(%d) is not APPS_RETRIEVE_START_PS\n", __FUNCTION__, WlanIdx, tr_entry->ps_state));
		goto NEXT;
	}
	...
	if (tr_entry->ps_qbitmap == 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(%d): (ps_state = %d) token_enq_all_fail!! ==> send CMDTHREAD_PS_CLEAR cmd.\n",
   ...
   tr_entry = &pAd->MacTab.tr_entry[WlanIdx];
		...
		if (pEntry->PsMode == PWR_ACTIVE) {
			...
			if (MtStartPSRetrieve(pAd, ps_fifo_swq->swq[deq_qid]) == TRUE) {
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("=======> Error %s(%d) wcid=%d,  tr_entry->ps_state=%d\n", __FUNCTION__, __LINE__, ps_fifo_swq->swq[deq_qid], tr_entry->ps_state));
			}
			break;			
		}
	}
...
static VOID CmdPsRetrieveStartRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
	...
	if (!(VALID_WCID(WlanIdx))) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("---->%s INVALID_MAC_WCID(WlanIndex)\n", __FUNCTION__));
      return;
   }
	pEntry = &pAd->MacTab.Content[WlanIdx];
	...
	if (!(VALID_TR_WCID(WlanIdx))) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("---->%s INVALID_TR_WCID(WlanIndex)\n", __FUNCTION__));
	  	return;
	}
	tr_entry = &pAd->MacTab.tr_entry[WlanIdx];
...
static VOID CmdPsClearRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
   ...
   MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("wcid=%d, Receive Event of CmdPsClear tr_entry->ps_state=%d\n", WlanIndex,tr_entry->ps_state));
...
INT32 CmdPsRetrieveStartReq(RTMP_ADAPTER *ad, UINT32 WlanIdx)
{
   ...
   MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(%d): RTEnqueueInternalCmd comming!! WlanIdx: %x\n",__FUNCTION__, __LINE__,WlanIdx));
...
static VOID mt_FillSkuParameter(RTMP_ADAPTER *pAd,UINT8 channel,struct _EXT_CMD_CHAN_SWITCH_T *pCmdChanSwitch)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s: current bandwidth %d, ctrl channel %d, central channel %d\n", 
		__func__, pCmdChanSwitch->ucBW, pCmdChanSwitch->ucCtrlCh, pCmdChanSwitch->ucCentralCh));
...
INT32 CmdThermalProtect(RTMP_ADAPTER *ad, UINT8 HighEn, UINT8 HighTempTh, UINT8 LowEn, UINT8 LowTempTh)
{
	struct cmd_msg *msg;
	INT32 ret = 0;
...
error:
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s:(ERROR! ret = %d)\n", __FUNCTION__, ret));
	}
	return;
