...
/*	{"WnmMaxIdlePeriod",					Set_WnmMaxIdlePeriod_Proc}, */
	...
	{"SCSMode",   SetSCSMode_Proc},
	{"SCSRSSI",   SetSCSRSSI_Proc},
	{"SCSTxrx",   SetSCSTxrx_Proc},
	...
	IN INT cmd)
{
	...
	case OID_BNDSTRG_MSG:
		...
		break;
               ...
               case OID_AP_VENDOR_IE_SET:
               {
                       ...
                       //pAd->mesh_enable = FALSE;

                       if (wrq->u.data.length == 0) {
                               MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
                                       ("data length is 0, return!!\n"));
                               break;
                       }
                      ...
                      INT ret;
                       ...
                       if (ret == NDIS_STATUS_SUCCESS) {
                               ...
                               if (ap_customer_oui->pointer != NULL)
                                       os_free_mem(NULL, ap_customer_oui->pointer);
	...
	case OID_GET_TPIE_INFO:
	{
		#define MAX_TPIE_LEN (1024*24)
		...
		INT status = 0;
 		...
 		pObj = (POS_COOKIE) pAd->OS_Cookie;
 		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("==>OID_GET_TPIE_INFO\n"));
		...
		os_alloc_mem(NULL, (UCHAR **)(&msg), MAX_TPIE_LEN);
		if (msg == NULL)
		{
	 		...
	 		&& (pAd->ApCfg.ApCliTab[ifIndex].SsidLen != 0))
	 	{
	 		for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	 		{
	 			...
	 			if ( IS_ENTRY_APCLI(pEntry)
					...
					&& (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED))
	 				{
						/* Find out desired AP's TP-Link IE in scanTab */
						 for (k = 0; k < pAd->ScanTab.BssNr; k++)
				               {
				                       MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pAd->ScanTab.BssEntry[%d].Bssid = %02x:%02x:%02x:%02x:%02x:%02x\n", k, 
				                                               pAd->ScanTab.BssEntry[k].Bssid[0],
				                                               pAd->ScanTab.BssEntry[k].Bssid[1],
				                                               pAd->ScanTab.BssEntry[k].Bssid[2],
				                                               pAd->ScanTab.BssEntry[k].Bssid[3],
				                                               pAd->ScanTab.BssEntry[k].Bssid[4],
				                                               pAd->ScanTab.BssEntry[k].Bssid[5]
				                                               ));
				                       if (MAC_ADDR_EQUAL(pAd->ScanTab.BssEntry[k].Bssid, pEntry->Addr))
				                       {
								UINT j = 0;
								UINT tmpLen = 0;
								UCHAR *ptmp = NULL;	
								MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,  ("Find connected AP in ScanTab\n") );
				                            tmpLen = strlen(msg);
				                            if (pAd->ScanTab.BssEntry[k].TpIE.IELen > 0)
				                           {
				                                    sprintf(msg + tmpLen,"data    : ");
				                                    tmpLen += 10;
				                                       
				                                    /* vendor: 221 */
				                                    sprintf(msg + tmpLen,"%02x", 221);
				                                    tmpLen += 2;
				                                    /* len */
				                                    sprintf(msg + tmpLen,"%02x", pAd->ScanTab.BssEntry[k].TpIE.IELen + 3);
				                                    tmpLen += 2;
				                                    /* OUI */
				                                    for (j = 0; j < 3; j++)
				                                    {
				                                            sprintf(msg + tmpLen,"%02x", pAd->ScanTab.BssEntry[k].TpIE.OUI[j]);
				                                            tmpLen += 2;
				                                    }
				                                    /* IE */
				                                    ptmp = pAd->ScanTab.BssEntry[k].TpIE.IE;
				                                    for (j = 0; j < 23; j++)
				                                    {
				                                            sprintf(msg + tmpLen,"%02x", *ptmp);
				                                            ptmp++;
				                                            tmpLen += 2;
				                                    }
				                                    /* 濡傛灉涓烘墿灞曞悗鐨凾PIE锛岃鎺ュ彛闇�瑕佷笂浼爐pinfo_product_type(2 Bytes)鍜宼pinfo_reserved(2 Bytes)瀛楁淇℃伅锛岄粯璁や负0 */
				                                    if (pAd->ScanTab.BssEntry[k].TpIE.IELen + 3 == NEW_TPIE_DATA_LEN)
				                                    {
				                                            for (j = 0; j < 4; j++)
				                                            {
				                                                    sprintf(msg + tmpLen,"%02x", *ptmp);
				                            ...
				                            else
				                            {
				                                    ...
				                                    for (j = 0; j < NEW_TPIE_DATA_LEN + 2; j++)
				                                    {       
				                                            sprintf(msg + tmpLen,"%02x", 0);
				                                            tmpLen += 2;
				                                    }
           	                    ...
           	                    if (k >= pAd->ScanTab.BssNr)
		                   {
						status = -1;
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Can't find connected AP in ScanTab\n"));
				     }
				     break;
	 			}
	 		}
		...
		//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d wrq->u.data.length = %d, msg = %s\r\n", __FUNCTION__, __LINE__, wrq->u.data.length, msg)); 
		error = copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length); 
		if (error == 0) 	
		{			
			//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d: copy_to_user() OK: %s\n", __FUNCTION__, __LINE__, msg));	
			status = NDIS_STATUS_SUCCESS;
		}		
		else		
		{			
			//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d: copy_to_user() fail, bytes: %d\n", __FUNCTION__, __LINE__, error)); 
			status = NDIS_STATUS_FAILURE;	
		}
	...
	case OID_802_11R_GET_STATION_LIST_INFO:
		{
			...
			{		
				...
				{	
					...
					//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("copy entry index = %d\r\n", i));
					...
					msg->staInfo[idx].rssi = rssiAvg + 90; /*95是qca的电频基数*/;	
					//msg->staInfo[idx].rssi = rssiAvg + 95; /*95是qca的电频基数*/;	
					...
					msg->staInfo[idx].phyCapInfo.maxTxPower = GetCuntryMaxTxPwr(pAd, pAd->CommonCfg.Channel);
					idx++;		
				}	
			}	
			if (idx > 0)	
			{		
				...
				*/
				...
				//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d wrq->u.data.length = %d\r\n", __FUNCTION__, __LINE__, wrq->u.data.length)); 
				error = copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length); 
				if (error == 0) 	
				{			
					//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d: copy_to_user() OK\n", __FUNCTION__, __LINE__));	
					status = NDIS_STATUS_SUCCESS;
				}		
				else		
				{			
	...
	case OID_802_11R_GET_STATION_INFO:
		{
			...
			//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("sta mac = %02x:%02x:%02x:%02x:%02x:%02x\r\n", PRINT_MAC(pSta->mac)));	
			COPY_MAC_ADDR(sta.mac, pSta->mac);
			for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++)	
			{
				MAC_TABLE_ENTRY *pEntry = &pAd->MacTab.Content[i];		
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("sta mac = %02x:%02x:%02x:%02x:%02x:%02x, pEntry->Addr = %02x:%02x:%02x:%02x:%02x:%02x\r\n", PRINT_MAC(sta.mac), PRINT_MAC(pEntry->Addr)));
				...
				if (IS_EQUAL_MAC(sta.mac, pEntry->Addr))		
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s %d: sta.sta.lastRxRate = %d, sta.sta.lastTxRate = %d, sta.sta.rssi = %d, sta.sta.idle = %d\n", __FUNCTION__, __LINE__, sta.sta.lastRxRate, sta.sta.lastTxRate, sta.sta.rssi, sta.sta.idle));		
					found = 1;
					break;
				}
			}
			...
			{		
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s %d: Status = %d, wrq->u.data.length = %d\n", __FUNCTION__, __LINE__, Status, wrq->u.data.length));		
				if (Status == NDIS_STATUS_FAILURE)
				{			
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d: copy_to_user() fail\n", __FUNCTION__, __LINE__)); 		
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d: status info not found.\n", __FUNCTION__, __LINE__));		
				Status = NDIS_STATUS_FAILURE;
			}
		}
		break;
	...
	case OID_802_11R_SCAN_EXAP_INFO:
			{
				...
				printk("%s %d scan target SSID %s, scanType = %d\r\n", __FUNCTION__, __LINE__, pTargetSsid->Ssid, scanType);
				waitCnt = 0;
				ApSiteSurvey(pAd, pTargetSsid, scanType, FALSE);
				...
				printk("%s %d scan finished, send IEEE80211_REASON_COMPLETED to user space.\r\n", __FUNCTION__, __LINE__);
			}
		break;
	...
	case OID_802_11R_GET_EXAP_INFO:
		{
				...
				printk("%s %d pAd->ScanTab.BssNr = %d\r\n", __FUNCTION__, __LINE__, pAd->ScanTab.BssNr);
				if (0 == pAd->ScanTab.BssNr)
				{
				...
				/* sort by rssi */
				sort(pAd->ScanTab.BssEntry, pAd->ScanTab.BssNr, sizeof(BSS_ENTRY), _rtmp_BssEntry_cmp, NULL);

				for (i = 0; i < pAd->ScanTab.BssNr; i++)
				{
					pBss = &pAd->ScanTab.BssEntry[i];
					printk("%s %d: ScanTab.BssEntry[%d]: ssid = %s, Channel = %d, Rssi = %d\r\n", __FUNCTION__, __LINE__, i, pBss->Ssid, pBss->Channel, pBss->Rssi);

					if (pBss->Channel == 0)
					{
						printk("%s %d pBss[%d] channel == 0, continue", __FUNCTION__, __LINE__, i);
						continue;
					}
					...
					if (j >= maxBssNum)
					{
						printk("%s %d we just get %d entries.", __FUNCTION__, __LINE__, j);
						break;
					}
				}
	...
	case OID_802_11R_CLR_ASSOC_INFO:
		{			
			UCHAR Addr[MAC_ADDR_LEN];
			MAC_TABLE_ENTRY *pEntry = NULL;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d OID_802_11R_CLR_ASSOC_INFO.", __FUNCTION__, __LINE__));
			...
			if ( !IS_ENTRY_CLIENT(pEntry) )
				break;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d MacTableDeleteEntry.", __FUNCTION__, __LINE__));
			...
			if (pAd->ApCfg.bMACRepeaterEn == TRUE)
			{
				...
				pReptEntry = RTMPLookupRepeaterCliEntry(pAd, TRUE, Addr, TRUE, &isLinkValid);
	...
	IN INT cmd)
{
	...
	case OID_802_11_AUTHENTICATION_MODE:
		...
		{
			INT WscMode;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_WSC_MODE \n"));
#ifdef APCLI_SUPPORT
			if (pObj->ioctl_if_type == INT_APCLI)
			{
				INT ApCliIdx = pObj->ioctl_if;
...
#endif /* APCLI_SUPPORT */
			{
				WscMode = pAd->ApCfg.MBSSID[apidx].WscControl.WscMode;
			}
		...
		case RT_OID_WSC_QUERY_LOCK:
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_WSC_QUERY_LOCK \n"));
#ifdef APCLI_SUPPORT
			if (pObj->ioctl_if_type == INT_APCLI)
			{
				INT ApCliIdx = pObj->ioctl_if;
...
#endif /* APCLI_SUPPORT */
			{
				...
				{
					if(TRUE == pAd->ApCfg.MBSSID[apidx].WscControl.WscSetupLockTimerRunning)
					{
						_os_timer = (OS_NDIS_MINIPORT_TIMER *)(&(pAd->ApCfg.MBSSID[apidx].WscControl.WscSetupLockTimer.TimerObj));
						WscLock.setupLockTime = (_os_timer->expires - jiffies) / OS_HZ;
					}
					else
					{
	...
	IN	RTMP_STRING *arg)
{
	...
	if(val == 0 || ((val >= 10) && (val < MAX_REKEY_INTER)))
	...
	if (pAd->chipCap.MCUType == ANDES) {
		...
		if (pAd->chipCap.need_load_fw) {
			...
			for (loop = 0; loop < 16; loop++)
				printk("%c", *(cap->FWImageName + 16 + loop));
...
#define RSSI_BASE (-127)
	...
	for(i=0; i<pAd->ScanTab.BssNr && i < maxBssNum;i++)
	{
		pBss = &pAd->ScanTab.BssEntry[i];
	...
	printk("maxBssNum is %d\n", i);
	if(pAd->ScanTab.BssNr < maxBssNum)
...
//		sprintf(msg+strlen(msg), "WPS Query Status(ra%d)        = %d\n", idx, pAd->ApCfg.MBSSID[idx].WscControl.WscStatus);
    ...
    for (idx = 0; idx < pAd->ApCfg.ApCliTab[0].WscControl.WscProfile.ProfileCnt ; idx++)
    {
       ...
       /* Find WPS AP in ScanTab for mesh info */
#ifdef TP_IE_SUPPORT
       {
               MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pCredential->MacAddr = %02x:%02x:%02x:%02x:%02x:%02x\n", 
               ...
               MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pAd->ApCfg.ApCliTab[0].WscControl.WscBssid = %02x:%02x:%02x:%02x:%02x:%02x\n", 
                              pAd->ApCfg.ApCliTab[0].WscControl.WscBssid[0],
                               pAd->ApCfg.ApCliTab[0].WscControl.WscBssid[1],
                               pAd->ApCfg.ApCliTab[0].WscControl.WscBssid[2],
                               pAd->ApCfg.ApCliTab[0].WscControl.WscBssid[3],
                               pAd->ApCfg.ApCliTab[0].WscControl.WscBssid[4],
                               pAd->ApCfg.ApCliTab[0].WscControl.WscBssid[5]
                               ));
               for (i = 0; i < pAd->ScanTab.BssNr; i++)
               {
                       MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pAd->ScanTab.BssEntry[%d].Bssid = %02x:%02x:%02x:%02x:%02x:%02x\n", i, 
                                               pAd->ScanTab.BssEntry[i].Bssid[0],
                                               pAd->ScanTab.BssEntry[i].Bssid[1],
                                               pAd->ScanTab.BssEntry[i].Bssid[2],
                                               pAd->ScanTab.BssEntry[i].Bssid[3],
                                               pAd->ScanTab.BssEntry[i].Bssid[4],
                                               pAd->ScanTab.BssEntry[i].Bssid[5]
                                               ));
                       if (MAC_ADDR_EQUAL(pAd->ScanTab.BssEntry[i].Bssid, pAd->ApCfg.ApCliTab[0].WscControl.WscBssid))
                       {
                               UINT j = 0;
                               UINT tmpLen = 0;
                               UCHAR *ptmp = NULL;
                               MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,  ("Find WPS AP in ScanTab\n") );
                               tmpLen = strlen(msg);
                               if (pAd->ScanTab.BssEntry[i].TpIE.IELen > 0)
                               {
                                       sprintf(msg + tmpLen,"data    = ");
                                       tmpLen += 10;
                                       ...
                                       tmpLen += 2;
                                       /* len */
                                       sprintf(msg + tmpLen,"%02x", pAd->ScanTab.BssEntry[i].TpIE.IELen + 3);
                                       tmpLen += 2;
                                       /* OUI */
                                       for (j = 0; j < 3; j++)
                                       {
                                               sprintf(msg + tmpLen,"%02x", pAd->ScanTab.BssEntry[i].TpIE.OUI[j]);
                                               tmpLen += 2;
                                       }
                                       /* IE */
                                       ptmp = pAd->ScanTab.BssEntry[i].TpIE.IE;
                                       for (j = 0; j < 23; j++)
                                       {
                                                       sprintf(msg + tmpLen,"%02x", *ptmp);
                                                       ptmp++;
               ...
               {
                       MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Can't find WPS AP in ScanTab\n"));
               }
	}
#endif /* TP_IE_SUPPORT */
    }
	...
	IN	RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF(%02X:%02X:%02X:%02X:%02X:%02X) Set_WscGetConf_Proc trigger WSC state machine, wscGetConfMode=%ld\n",
				PRINT_MAC(mac_addr), simple_strtol(arg, 0, 10)));
	...
	IN	RTMP_STRING *arg)
{
	...
	{
		sharedWscLock = pWscControl->bSetupLock = TRUE;
...
{		
	...
	wreq.data.length = (data == NULL) ? 0 : sizeof(UCHAR);
	wireless_send_event(dev, siocType, &wreq, data);
}
EXPORT_SYMBOL(send_event_to_network_roaming);
#endif
