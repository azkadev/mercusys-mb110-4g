...
PNDIS_PACKET RTMPDeFragmentDataFrame(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	if (VALID_WCID(pRxBlk->wcid)) {
		...
		if (!pEntry)
			MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s pEntry not find in wcid:0x%x\n", __func__, pRxBlk->wcid));
		else
	...
	{	/*Middle & End of fragment*/ /*frag patch*/
...
#ifdef MWDS
#if 0
	if (pAd->MlmeDebug && pHeader->FC.Type == FC_TYPE_MGMT &&
		(/*pHeader->FC.SubType == SUBTYPE_PROBE_REQ ||*/ pHeader->FC.SubType == SUBTYPE_AUTH ||
			pHeader->FC.SubType == SUBTYPE_ASSOC_REQ))
	{
		ULONG tmpNow32;
		NdisGetSystemUpTime(&tmpNow32);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(%d) @@@@ time is %lu FRM=%d SUB=%d [SUBTYPE_PROBE_REQ=4 SUBTYPE_AUTH=11 SUBTYPE_ASSOC_REQ=0].\n",
			__FUNCTION__, __LINE__, tmpNow32, pHeader->FC.Type, pHeader->FC.SubType));
	}
#endif
#endif /* MWDS */
...
#ifdef MWDS
		if (!IS_MWDS_OPMODE_APCLI(pEntry) || (pRxInfo->Mcast || pRxInfo->Bcast))
#endif /* MWDS */
...
ret:
#ifdef MWDS
	hdr_len = (hdr_len == LENGTH_802_11_WITH_ADDR4)?LENGTH_802_11_WITH_ADDR4 : LENGTH_802_11;
#else
	...
	IN struct wifi_dev *wdev)
{
	...
	if (!RTMPCheckWPAframe(pAd, pEntry, pData, data_len, wdev_idx, eth_frame))
	{
		...
		if (pEntry && IS_ENTRY_MWDS(pEntry)) {
			...
			/* we just pass arp bcast requst packet */
			if ( (!((FC->FrDs == 1) && (FC->ToDs == 1))) 
				&& ( !RTMP_GET_PACKET_SPECIFIC_RX_ARP(pRxBlk->pRxPacket) )	){
#endif
            if ( (!((FC->FrDs == 1) && (FC->ToDs == 1)))
		...
		if ((pRxBlk->pHeader->FC.Wep == 0) && (pRxBlk->pHeader->FC.Type == FC_TYPE_DATA) && (pEntry->WepStatus != Ndis802_11EncryptionDisabled)) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("==> Receiving non-EAP data frame with FC.Wep=0, so de-Auth this STA(%02x:%02x:%02x:%02x:%02x:%02x) for \n",
				PRINT_MAC(pEntry->Addr)));
...
//---Add by shiangf for debug
	...
	*/
    ...
    {   
        ...
        if (!hdr_len)
        {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s: hdr_len = 0, drop this paket!", __FUNCTION__));
            goto drop;
        }
#endif /* MWDS */
    }
...
//add debug for DUT can't ping RootAP by ziqiang.yu(2015/12/04)
{
	...
	bool is_arp = false;
#endif /* MWDS */
#if defined(TP_NETWORK_ROAMING) && defined(MAC_REPEATER_SUPPORT)
	...
	{
		if ( 0x08 == pData[6] && 0x06 == pData[7] )//mark packet is ARP
		{
			/*fix arp problem*/
			if ( 0x00 == pData[14] && 0x02 == pData[15] )
			{
				pRxBlk->Arp = 1;
				/*code below is unnecessary, cause we'll do it in the OS_bridge
				  for convenience and for the sake of uniformity.
				printk("[7628]transfer src mac and sender mac\n");
				NdisMoveMemory(pHeader->Addr3, pRptCliEntry->OriginalAddress, MAC_ADDR_LEN);
				NdisMoveMemory(pData+16, pRptCliEntry->OriginalAddress, MAC_ADDR_LEN);

				printk("[7628]------addr3: %02X:%02X:%02X:%02X:%02X:%02X\n", PRINT_MAC(pHeader->Addr3));
				printk("[7628]------addr1: %02X:%02X:%02X:%02X:%02X:%02X\n", PRINT_MAC(pHeader->Addr1));
				printk("[7628]------addr2: %02X:%02X:%02X:%02X:%02X:%02X\n", PRINT_MAC(pHeader->Addr2));
				*/
			}
			else
				goto drop;
	...
	if (pData[6] == 0x81 &&pData[7]==0x00) /*vlan tag*/
	{
		...
		if (pData[10] == 0x08 && pData[11] == 0x00)
		{
			is_ip = true;   /*mark ip data*/
		}
		else if (pData[10] == 0x08 && pData[11] == 0x06)
		{
	...
	{
		pPktHdr = pData+8;// get ip header start addr
		if (pData[6] == 0x08 && pData[7] == 0x00)
		{
			is_ip = true;
		}
		else if (pData[6] == 0x08 && pData[7] == 0x06)
		{
			is_arp = true;
		}
	}
	...
	if ( is_ip && (*(pPktHdr + 9) == 0x1) ) /*9: offset of protocol in ip header*/
	{
	...
	} else if (is_arp)//mark packet is ARP
	{
		/* TP-link feature: bmcast arp packet use 3addr formate in mwds mode */
		USHORT arptype;
		pRxBlk->Arp = 1;
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[7628]rx path arp #(aid=%d,wcid=%d, pHeader seq=%d  ,ampdu = %d)\n",
						pEntry->Aid, pRxBlk->wcid, pRxBlk->pHeader->Sequence, RX_BLK_TEST_FLAG(pRxBlk, fRX_AMPDU)));
	} else if (is_ip && (*(pPktHdr + 9) == 0x11))//mark packet as udp
	{
		PUCHAR pUdpHdr;
		UINT16 srcPort, dstPort;
		//BOOLEAN bHdrChanged = FALSE;
