	...
	IN PRRM_MLME_BCN_REQ_INFO pMlmeBcnReq)
{
	...
	*/
	...
	if (pMlmeBcnReq->BcnReqCapFlag.field.ChannelRep == TRUE)
	{
		...
		while(pMlmeBcnReq->ChRepRegulatoryClass[idx] != 0)
		{
			...
			InsertChannelRepIE(pAd, (pOutBuffer+FrameLen), &FrameLen,
								...
								pMlmeBcnReq->ChRepRegulatoryClass[idx], NULL);
	...
	/* BCN request mode active(1) no need to report deatail by DengZhong 20180408 */
	/* inssert report detail sub field. */
	if (BcnReq.MeasureMode == RRM_BCN_REQ_MODE_BCNTAB || BcnReq.MeasureMode == RRM_BCN_REQ_MODE_ACTIVE)
		ReportDetail = 0;
	else
		ReportDetail = 1;
...
#define MIN(_x, _y) ((_x) > (_y) ? (_x) : (_y))
	...
	RRM_InsertNeighborRepIE(pAd, (pOutBuffer + FrameLen), &FrameLen,
			sizeof(RRM_NEIGHBOR_REP_INFO), pMbss->wdev.bssid,
			BssidInfo, get_regulatory_class(pAd),
			pMbss->wdev.channel, CondensedPhyType);
	...
	for(idx=0;idx<MAX_FTM_TBL_SIZE;idx++)
	{
		...
		{				
			...
			RRM_InsertNeighborRepIE(pAd, (pOutBuffer + FrameLen), &FrameLen,
				sizeof(RRM_NEIGHBOR_REP_INFO), pFtmNeighbor->NeighborBSSID,
				BssidInfo, pFtmNeighbor->NeighborOpClass,
				pFtmNeighbor->NeighborChannel, pFtmNeighbor->NeighborPhyType);
	...
	for (loop = 0; loop < pAd->ScanTab.BssNr; loop++)
	{
		...
		if (SsidLen != 0)
		{
			if ( RTMPEqualMemory(pBssEntry->Bssid, pAd->ApCfg.MBSSID[0].wdev.bssid, MAC_ADDR_LEN) )
			{
				BssMatch = FALSE;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("ignore local bss\n"));
			}
			else
			{
				BssMatch = RTMPEqualMemory(pBssEntry->Ssid, pSsid,MIN(SsidLen, pBssEntry->SsidLen));
			}
		}
		else
			BssMatch = TRUE;
