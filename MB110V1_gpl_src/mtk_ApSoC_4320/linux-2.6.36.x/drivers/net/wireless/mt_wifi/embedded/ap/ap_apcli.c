		...
		*/
		...
		COPY_MAC_ADDR(pApCliEntry->LastBssid, pAd->MlmeAux.Bssid);
			...
			*/
			...
			{
				if ( pAd->ApCfg.ApCliTab[ifIndex].wdev.bDefaultMwdsStatus &&
					pAd->ApCfg.ApCliTab[ifIndex].wdev.bSupportMWDS )
					MWDSAPCliPeerEnable(pAd, pApCliEntry, pMacEntry, FALSE); //OPEN
				else
					MWDSAPCliPeerDisable(pAd, pApCliEntry, pMacEntry); //OPEN
			}
#endif /* MWDS */
		...
		if (apcli_entry && (&apcli_entry->wdev == wdev))
		{
			...
			if ((apcli_entry->bEnableMWDS == FALSE) &&
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					("ApCliAllowToSendPacket:return FALSE as MWDS disabled even supported\n"));
				allowed = FALSE;
			}
#endif /* MWDS */
			break;
...
//			AsicSetDevMac(pAd, wdev->if_addr, 0x1);//set own_mac to HWBSSID1
            ...
            if (pAd->bCustomApcliMAC)
            {
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ApcliCustomAddress: %02x:%02x:%02x:%02x:%02x:%02x\n",
                PRINT_MAC(pAd->ApcliCustomAddress)));
                COPY_MAC_ADDR(wdev->if_addr, pAd->ApcliCustomAddress);
            }
		}
#endif /* MT_MAC */
...
#ifdef TP_IE_SUPPORT
#define GET_TPIE_NODE_IE(x)     (*(UINT16 *)((((BSS_ENTRY*)(x))->TpIE.IE) + 21))
#define GET_LEVEL_FROM_TPIE(x)  (UCHAR)(((BSS_ENTRY*)(x))->TpIE.level)

#define SUPPORT_MESH_FLAG (1)
#define UNSUPPORT_MESH_FLAG (0)
#define WEAK_RSSI (-83)
#define STRONG_RSSI (-75)

/* When call this function, must sort pBssTab first */
static VOID BssSelect(BSS_TABLE *pBssTab)
{
	BSS_ENTRY *pBssEntry = NULL;
	BSS_ENTRY TmpBssEntry;
	
	INT32 idx = 0;

	if (NULL == pBssTab)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: BssTab is NULL\n", __FUNCTION__));
		return;
	}

	if (pBssTab->BssNr <= 1)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: Need not to do bss select(%d)\n", __FUNCTION__, pBssTab->BssNr));
		return;		
	}

	/*1. If exist normal AP: contains RE which with out OneMesh function or OneMesh is turned off, select by old rule */
	for (idx = pBssTab->BssNr - 1; idx >= 0; idx--)
	{
		pBssEntry = &pBssTab->BssEntry[idx];
		if (pBssEntry->TpIE.support_mesh == UNSUPPORT_MESH_FLAG)
			return;
	}

	/* 2. Only Exist Both OneMesh AP or OneMesh RE or OneMesh AP and OneMesh RE */
	for (idx = pBssTab->BssNr - 1; idx >= 0; idx--)
	{
		pBssEntry = &pBssTab->BssEntry[idx];
		if (pBssEntry->TpIE.level == 0 && pBssEntry->Rssi >= STRONG_RSSI)
		{
			NdisMoveMemory(&TmpBssEntry, pBssEntry, sizeof(BSS_ENTRY));
			NdisMoveMemory(pBssEntry, &pBssTab->BssEntry[pBssTab->BssNr - 1], sizeof(BSS_ENTRY));
			NdisMoveMemory(&pBssTab->BssEntry[pBssTab->BssNr - 1], &TmpBssEntry, sizeof(BSS_ENTRY));
			return;				
		}
	}

	/* 3 Exist OneMesh AP or OneMesh RE or can't find OneMesh AP whose rssi >= -75, select by old rule */	
	/* do nothing here */
	/* beacuse the pBssTab is sorted before call this function */
}
#endif
	...
	IN  PRTMP_ADAPTER   pAd)
{
	...
	*/
	for (entryIdx=0; entryIdx<pScanTab->BssNr;entryIdx++)
	{
		...
		{
            /*
             * If one mesh feature enabled,
             * we should ensure our apcli always connected to the desired rootap,
             * which is identified by node id
             */
#ifdef TP_IE_SUPPORT
            if ((pAd->mesh_enable == TRUE) &&
                   (pAd->is_conn_specific_bssid == TRUE))
            {
                printk("pBssEntry node id: 0x%04x, rootap node id: 0x%04x\n",
                        GET_TPIE_NODE_IE(pBssEntry), pAd->node_id);
                if (pAd->node_id != GET_TPIE_NODE_IE(pBssEntry))
                {
                    printk("Node ID not match!!!");
                    continue;
                }
            }
            ...
            if (pAd->mesh_enable == TRUE)
            {
                printk("pBssEntry level %d\n", GET_LEVEL_FROM_TPIE(pBssEntry));
                if (GET_LEVEL_FROM_TPIE(pBssEntry) >= 2)
                {
                    printk("Can't connect to AP which level(%d) greater than 2!!!\n",
                            GET_LEVEL_FROM_TPIE(pBssEntry));
                    continue;
                }
            }
...
#if 0
#ifdef TP_IE_SUPPORT
	BssTableDisplay(pSsidBssTab);
	if ((pAd->mesh_enable == TRUE) && (pAd->is_conn_specific_bssid == FALSE))
	{
		/* select AP or RE: put the selected BSS in pSsidBssTab[pSsidBssTab->BssNr -1]. */
		BssSelect(pSsidBssTab);
	}
	BssTableDisplay(pSsidBssTab);
#endif /* TP_IE_SUPPORT */
#endif
