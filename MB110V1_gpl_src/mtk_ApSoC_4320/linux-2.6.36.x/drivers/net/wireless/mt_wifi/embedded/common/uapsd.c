	...
	if ((pEntry != NULL) && (pEntry->PsMode == PWR_SAVE))
	{
		...
		if (pEntry->bAPSDFlagSPStart != 0)
		{
...
#ifdef UAPSD_DEBUG
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("uapsd> [3] close SP!\n"));
#endif /* UAPSD_DEBUG */
			...
			if (pEntry->pUAPSDEOSPFrame != NULL)
			{
				...
				*/
#ifdef UAPSD_DEBUG
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("uapsd> [3] Free EOSP (UP = %d)!\n",
							RTMP_GET_PACKET_UP(
	...
	IN BOOLEAN FlgApsdCapable)
{
	...
	if ((pEntry) && (FlgApsdCapable == TRUE))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("apsd> UAPSD %d %d %d %d!\n",
					pQosInfo->UAPSD_AC_BE, pQosInfo->UAPSD_AC_BK,
					pQosInfo->UAPSD_AC_VI, pQosInfo->UAPSD_AC_VO));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("apsd> MaxSPLength = %d\n",
					pEntry->MaxSPLength));
		...
		{
			/* all AC are U-APSD */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("apsd> all AC are UAPSD\n"));
			pEntry->bAPSDAllAC = 1;
		}
		else
		{
			/* at least one AC is not U-APSD */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("apsd> at least one AC is not UAPSD %d %d %d %d\n",
	...
	*/
	...
	if (pEntry->bAPSDFlagSPStart != 0)
	{
...
#ifdef UAPSD_DEBUG
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("uapsd> sorry! SP is not yet closed!\n"));
#endif /* UAPSD_DEBUG */
		...
		*/
		...
		{
#ifdef UAPSD_DEBUG
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("uapsd> SP period is too large so SP is closed first!"
						" (%lu %lu %lu)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n",
	...
	if (pEntry->pUAPSDEOSPFrame != NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("uapsd> EOSP is not NULL!\n"));
		RTMP_SEM_UNLOCK(&pAd->UAPSDEOSPLock);
		return;
	}
#endif /* UAPSD_DEBUG */
