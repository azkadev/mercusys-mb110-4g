	...
	switch (RingType)
	{
		...
		case QID_RX:
			for (ring_id =0; ring_id < NUM_OF_RX_RING; ring_id++)
			{
...
#endif /* RT_BIG_ENDIAN */
					pDmaBuf = &pRxRing->Cell[i].DmaBuf;
					pRxD->SDP0 = RTMP_GetPhysicalAddressLow(pDmaBuf->AllocPa);
					pRxD->DDONE = 0;
					pRxD->SDL0 = RxBufferSize;
#ifdef MWDS
					//pRxD->LS0 = 0; // debug-7.6
#else
					pRxD->LS0 = 0;
#endif /* MWDS */
				...
				HIF_IO_WRITE32(pAd, pRxRing->hw_cnt_addr, RxRingSize);
				HIF_IO_WRITE32(pAd, pRxRing->hw_desc_base, RTMP_GetPhysicalAddressLow(pRxRing->Cell[0].AllocPa));
...
VOID PDMAResetAndRecovery(RTMP_ADAPTER *pAd)
{
	...
	{
#if 0 /*  solve guest no bcn issue.  liuliming */
        BSS_STRUCT *pMbss;
...
VOID PDMAResetAndRecovery_LS0(RTMP_ADAPTER *pAd)
{
	UINT32 RemapBase, RemapOffset;
	UINT32 Value;
	UINT32 RestoreValue;
...
BOOLEAN MonitorRx0FullLS0(RTMP_ADAPTER *pAd)
{
	  UINT32 rdidx0,rcidx0;
	  ...
	  if(condition_1 == TRUE)
	  {
		...
		pRxRing = &pAd->RxRing[0];
		  ...
		  if(rdidx0 == rcidx0)
		  {
			i = (rdidx0 == 0) ? (RxRingSize -1) : (rdidx0-1);
			pRxD = (RXD_STRUC *)pRxRing->Cell[i].AllocVa;
			if((pRxD->DDONE)&&(pRxD->LS0 == 0))
				return TRUE;
		  }
...
VOID PDMAWatchDog(RTMP_ADAPTER *pAd)
{
	...
	/* Rx 0 Full and LS0 detect */
	NoDataIn = MonitorRx0FullLS0(pAd);

	if (NoDataIn)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Rx 0 Full and LS0 detect\n"));
		pAd->Rx0FullLS0Count++;
		goto reset;
	}
...
//debug-7.6
	...
	/* disable bcn */
	HIF_IO_READ32(pAd, 0x24458, &ori_bcnsetting);
	HIF_IO_WRITE32(pAd, 0x24458, 0); 
	/* Reset Dma_sch */
...
//debug-7.6
	/* Enable BCN */
	HIF_IO_WRITE32(pAd, 0x24458, ori_bcnsetting);
	/* Enable Rmac */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("monitor pdma register:\n"));
	SetPDMARegister_Proc(pAd,"9");
	SetPDMARegister_Proc(pAd,"4");
	SetPDMARegister_Proc(pAd,"5");
	SetPDMARegister_Proc(pAd,"6");
#endif /* MWDS */
}
