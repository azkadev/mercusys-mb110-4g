...
UINT32 parse_rx_packet_type(RTMP_ADAPTER *ad, RX_BLK *rx_blk, VOID *rx_packet)
{
	...
	switch (rx_pkt_type) {
		...
		*(((UINT32 *)txd_1)) = SWAP32(*(((UINT32 *)txd_1)));
            ...
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("parse_rx_packet_type wlan_idx=%d,%d,%d,0x%x\n", txd_1->wlan_idx,hdr_info,padding,tr_entry->EntryType));
			...
			*/
				...
				|| (tr_entry->ps_state == APPS_RETRIEVE_START_PS))
			{
				if (qos_p[1] == PS_RETRIEVE_TOKEN) //retrive_token : 0x76
				{
					...
					if (tr_entry->ps_qbitmap == 0x0)
					{
						...
						{
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("(wcid=%d) put ps_queue packets(Number=%d) to tx_queue.\n", txd_1->wlan_idx, tr_entry->ps_queue.Number));
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("(ps_state = %d)Receives all RMAC_RX_PKT_TYPE_RETRIEVE packets and send CMDTHREAD_PS_CLEAR cmd.\n",
							tr_entry->ps_state));
				...
				else
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("da=(%02x:%02x:%02x:%02x:%02x:%02x), sa=(%02x:%02x:%02x:%02x:%02x:%02x), qos_p->ac%d, qos_p->value=%d, Sequence=%d txd_1->wlan_idx=%d\n"
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("parse_rx_packet_type  not token txd_1->wlan_idx: %x, rx_packet addr: %x rx_wcid: %x\n",txd_1->wlan_idx,(u32)rx_packet,RTMP_GET_PACKET_WCID(rx_packet)));
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("da=(%02x:%02x:%02x:%02x:%02x:%02x), sa=(%02x:%02x:%02x:%02x:%02x:%02x), qos_p->ac%d, qos_p->value=%d, Sequence=%d txd_1->wlan_idx=%d, ps_state = %d\n"
...
#ifdef MWDS
#if 0
	if (pAd->MlmeDebug && pHeader_802_11->FC.Type == FC_TYPE_MGMT &&
		(/*pHeader_802_11->FC.SubType == SUBTYPE_PROBE_RSP ||*/ pHeader_802_11->FC.SubType == SUBTYPE_AUTH ||
			pHeader_802_11->FC.SubType == SUBTYPE_ASSOC_RSP))
	{
		ULONG tmpNow32;
		NdisGetSystemUpTime(&tmpNow32);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(%d) @@@@ time is %lu FRM=%d SUB=%d [SUBTYPE_PROBE_RSP=5 SUBTYPE_AUTH=11 SUBTYPE_ASSOC_RSP=1].\n",
			__FUNCTION__, __LINE__, tmpNow32, pHeader_802_11->FC.Type, pHeader_802_11->FC.SubType));
	}
#endif
#endif /* MWDS */
#ifdef RTMP_MAC_PCI
...
#endif /* P2P_SUPPORT || RT_CFG80211_P2P_SUPPORT*/
			{
				...
				if (IS_ENTRY_MWDS(pMacEntry)) {
					...
					if (IS_MWDS_OPMODE_APCLI(pMacEntry)) {
						...
						if ((!RTMP_GET_PACKET_EAPOL(pTxBlk->pPacket)) && (!RTMP_GET_PACKET_SPECIFIC_TX_ARP_BCAST(pTxBlk->pPacket)))
...
VOID rtmp_tx_swq_dump(RTMP_ADAPTER *pAd, INT qidx)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\nDump TxSwQ[%d]: DeqIdx=%d, EnqIdx=%d, %s, capCount=%u\n",
							...
							(enq_id >=deq_id) ? (TX_SWQ_FIFO_LEN-enq_id+deq_id) : (deq_id-enq_id)));
...
enq_end:
...
#if 0
	if (enq_done == FALSE  ) {
		void log_sw_enqueue_drop(int Qindx,int isdhcp,int wcid);
		int isdhcp=0;
		if(RTMP_GET_PACKET_DHCP(pkt) && RTMP_GET_PACKET_DHCP_DEBUG(pkt) )
			isdhcp=1;
		log_sw_enqueue_drop(qidx,isdhcp,tr_entry->wcid );
	}
#endif
#endif /* MWDS */
	if (FlgIsLocked == FALSE)
	...
	if (enq_done == FALSE) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					...
					/*occupied_wcid*/fifo_swq->swq[enq_idx], tr_entry->enq_cap));
	}
#ifdef MT_SDIO_ADAPTIVE_TC_RESOURCE_CTRL
	else
	{
	...
	*/
	for (deq_qid = start_q; deq_qid >= end_q; deq_qid--)
	{
		...
		{	/* unicast */
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD , ("!!!STA in PSM\n"));
...
INT deq_packet_gatter(RTMP_ADAPTER *pAd, struct dequeue_info *deq_info, TX_BLK *pTxBlk, BOOLEAN in_hwIRQ)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("-->%s(): deq_info->wcid=%d, qidx=%d!\n",
...
dequeue:
		if ((qEntry = pQueue->Head) != NULL)
		{
			...
			if (RTMP_GET_PACKET_MGMT_PKT(pPacket)) {
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): Call deq_mgmt_frame()!\n", __FUNCTION__));
			...
			if (traffic_jam_chk(pAd, tr_entry) == TRUE) {
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): traffic jam detected! free pkt!\n", __FUNCTION__));
				RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
		...
		else
		{
			...
			{
				deq_info->deq_pkt_cnt++;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("<--%s():Try deQ a empty Q. pTxBlk.TxPktList.Num=%d, deq_info.pkt_cnt=%d\n",
...
start_kick:
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("<--%s():pTxBlk.TxPktList.Num=%d, deq_info.pkt_cnt=%d\n",
					__FUNCTION__, pTxBlk->TxPacketList.Number, deq_info->pkt_cnt));
		break;
	...
	IN INT max_cnt)
{
	...
	do
	{
		...
		if (QueIdx >= 0)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): deq_info:cur_wcid=%d, cur_qidx=%d, pkt_cnt=%d, pkt_bytes=%d\n",
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): deq_packet_gatter %s, TotalFrmNum=%d\n",
	...
	OUT UCHAR *pQueIdx)
{
	...
	if (TypeLen <= 1500)
	{
		...
		*/
#ifdef MWDS
		UCHAR l2up[6] = {0x00, 0x01, 0xAF, 0x81, 0x01, 0x02};
#endif /* MWDS */
...
#ifdef MWDS
		else if (NdisEqualMemory(pSrcBuf, l2up, 6))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("---------------%s:l2up frame found, let it go\n", wdev->if_dev->name));
		}
#endif /* MWDS */
		else {
			return FALSE;
		...
		case ETH_TYPE_IPv4:
			{
				...
				if (*(pSrcBuf + 9) == IP_PROTO_UDP)
				{
					...
					{	/*It's a BOOTP/DHCP packet*/
						...
						if (pAd->DhcpTxUpEn)
						{
							extern unsigned int tx_dhcp;
							extern unsigned int queue_debug_print ;
							extern INT   check_dhcp(IN PNDIS_PACKET pSkb, int forward);

							RTMP_SET_PACKET_DHCP_DEBUG(pPacket, 1);//just for dhcp packet drop,make it different from arp packet

							up = 7;
							tx_dhcp++;
							if (queue_debug_print)
								printk("%s DHCP to up=7\n",__FUNCTION__);

							check_dhcp(pPacket, 3);
						}
#endif
#endif /* MWDS */
		...
		case ETH_TYPE_ARP:
			{
				...
				{
					USHORT ArpType;
					UCHAR *pArpSrcBuf = NULL;
					RTMP_SET_PACKET_SPECIFIC_EXT(pPacket, 0);
					RTMP_SET_PACKET_SPECIFIC_TX_ARP(pPacket , 1);

					pArpSrcBuf = GET_OS_PKT_DATAPTR(pPacket);
					ArpType = (USHORT)(( (*(pSrcBuf + 6)) << 8 ) + (*(pSrcBuf + 7)));

					if( ArpType == 1 )
						RTMP_SET_PACKET_SPECIFIC_TX_ARP_REQ(pPacket , 1);
					else if( ArpType == 2 )
						RTMP_SET_PACKET_SPECIFIC_TX_ARP_RES(pPacket , 1);

					if( isBcastEtherAddr(pArpSrcBuf) )
						RTMP_SET_PACKET_SPECIFIC_TX_ARP_BCAST(pPacket , 1);
				}
#endif /* MWDS */
			}
			break;
...
#ifdef MWDS
#if 0
    if(pAd->DhcpTxUpEn && RTMP_GET_PACKET_DHCP(pPacket)  && (*pQueIdx) == 0)
    {
	   //printk("%s DHCP packet but pQueIdx=%d\n",__FUNCTION__, *pQueIdx);
		*pUserPriority = up;
		*pQueIdx = WMM_UP2AC_MAP[up];
    }
#endif
	...
	IN MAC_TABLE_ENTRY *pEntry)
{
	...
	if (protoType == ETH_P_ARP)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("rx path ARP #(aid=%d,wcid=%d,arp op = %d, ampdu = %d)\n",
						pEntry->Aid, pRxBlk->wcid, /*ArpOp*/OS_NTOHS(*((UINT16 *)(pData))), RX_BLK_TEST_FLAG(pRxBlk, fRX_AMPDU)));
