	...
	IN UCHAR *pWcid)
{
...
#ifdef MWDS
	UCHAR l2up_frame[6] = { 0x0, 0x01, 0xaf, 0x81, 0x01, 0x02};

	INT IS_l2UP = 0;
#endif /* MWDS */
	RTMP_QueryPacketInfo(pPacket, &PacketInfo, &pSrcBufVA, &SrcBufLen);
#ifdef MWDS
	if (pSrcBufVA &&  htons(0x06) == *(UINT16 *)(pSrcBufVA + 12 ) &&
			NdisEqualMemory(pSrcBufVA + 14, l2up_frame, 6) )
	{
		IS_l2UP = 1;
		printk("\n^^^^^^^^^^^^^^^%s l2up found\n", __FUNCTION__);
	}
	...
	if (MAC_ADDR_IS_GROUP(pSrcBufVA))
	{
		*pWcid = wdev->tr_tb_idx;
		...
		{
			if ( IS_l2UP )
			printk("\n^^^^^^^^^^^^^^^%s %d l2up found and not allowed\n", __FUNCTION__, __LINE__);
			return FALSE;
		}
		else
#endif /* MWDS */
...
#ifdef MWDS
#if 0
			if (pAd->DhcpTxUpEn && RTMP_GET_PACKET_DHCP(pPacket) && RTMP_GET_PACKET_DHCP_DEBUG(pPacket))
			{
				extern unsigned int queue_debug_print;
				if (queue_debug_print ==2)//for dump rx dhcp packet
					printk("MT7628 DHCP Multicast packet to QueIdx =%d UserPriority=%d\n",QueIdx,UserPriority);//do nothing
...
#ifdef MWDS
					if(RTMP_GET_PACKET_DHCP(pPacket) && RTMP_GET_PACKET_DHCP_DEBUG(pPacket) )
						printk("%s (%d)  BSS tx_queue full 01 ERROR DHCP Packet Drop ENTRY_CAT_MCAST\n",__FUNCTION__,__LINE__);
#endif /* MWDS */
					drop_reason = DROP_TXQ_ENQ_FAIL;
					goto drop_pkt;
				}
...
#ifdef MWDS
					if(RTMP_GET_PACKET_DHCP(pPacket) && RTMP_GET_PACKET_DHCP_DEBUG(pPacket) )
						printk("%s (%d)  BSS tx_queue full 02 ERROR DHCP Packet Drop\n",__FUNCTION__,__LINE__);
#endif /* MWDS */
					drop_reason = DROP_TXQ_ENQ_FAIL;
					goto drop_pkt;
				}
...
#ifdef MWDS
	if ( IS_L2UP )
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():drop pkt, drop_reason=%d!, wcid = %d\n", __FUNCTION__, drop_reason, wcid));
	else
#endif /* MWDS */
	{
		if (drop_reason != PKT_SUCCESS)
		{
...
*/
			...
			MWDSProxyTabUpdate(pAd, pEntry->func_tb_idx, pEntry->wcid,
				pHeader->Octet, ARPSenderIP);
		} else if (pEntry && IS_ENTRY_CLIENT(pEntry) && !IS_MWDS_OPMODE_APCLI(pEntry) )
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					("=============== WDS entry triggered:(%02X:%02X:%02X:%02X:%02X:%02X)\n", PRINT_MAC(pEntry->Addr)));
...
#ifdef MWDS
#if 0
        UCHAR tmpmac[6] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
       if(MAC_ADDR_EQUAL(pHeader->Addr1, tmpmac))
       {
           MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, 
                   ("ap_rx_pkt_allow: Addr1 - %02X:%02X:%02X:%02X:%02X:%02X \
...
#endif       
        if(MAC_ADDR_EQUAL(pHeader->Addr3, pAd->BridgeLanAddress))
        {
		    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
                        ("ap_rx_pkt_allow drop package which src mac is our bridge mac:\
...
#ifdef MWDS
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
                        ("ap_rx_pkt_allow uptade ApCliRcvBeaconTime\n"));
#endif /* MWDS */ 
			}
		}
#endif /* APCLI_SUPPORT */
		...
		*/
		...
		if (((pFmeCtrl->FrDs == 0) && (pFmeCtrl->ToDs == 1))) {
			if ((pFmeCtrl->SubType != SUBTYPE_DATA_NULL) &&
				(pFmeCtrl->SubType != SUBTYPE_QOS_NULL) && (!IS_BROADCAST_MAC_ADDR(pHeader->Addr3)) ) {
				if (pEntry && IS_MWDS_OPMODE_AP(pEntry))
					return FALSE;
			}
		}
	...
	if (pAd->ApCfg.bMACRepeaterEn)
	{
		...
		if (pRptCliEntry && (pRptCliEntry->CliValid))
		{
			...
			return false;
#else
			if ( IS_BROADCAST_MAC_ADDR(pHeader->Addr1) )
				return false;
#endif
...
#ifdef MWDS
#if 0
	if (pAd->CountDebug && PWR_SAVE == pHeader->FC.PwrMgmt)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(%d): receive packet PWR_SAVE!\n", __FUNCTION__, __LINE__));
	}
#endif
#endif /* MWDS */
