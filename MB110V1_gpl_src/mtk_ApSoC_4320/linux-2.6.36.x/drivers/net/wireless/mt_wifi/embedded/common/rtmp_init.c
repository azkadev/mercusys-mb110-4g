...
#ifdef CONFIG_AP_SUPPORT
{
...
#ifdef WSC_V2_SUPPORT
			pWscControl->WscConfigMethods= 0x2388;
#else
...
#endif /* P2P_APCLI_SUPPORT || RT_CFG80211_P2P_SUPPORT */
	{
		...
		for(j = BSS0; j < pAd->ApCfg.BssidNum; j++)
		{
           ...
           NdisAllocateSpinLock(pAd, &pAd->ApCfg.MBSSID[j].ap_vendor_ie.vendor_ie_lock);
			...
			//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("====>[zhulin] pAd->mesh_enable = FALSE, pAd->is_conn_specific_bssid = FALSE\n"));
#endif /* TP_IE_SUPPORT */

			for(i = 0; i < WLAN_MAX_NUM_OF_TIM; i++)
	        		mbss->TimBitmaps[i] = 0;

			mbss->StationKeepAliveTime = 0;
		}
	...
	*/
	...
	pAd->SCSCtrl.SCSMode = 1;
	pAd->SCSCtrl.SCSL2H = -72;
	pAd->SCSCtrl.SCSH2L = -64;
	pAd->SCSCtrl.SCSTxrx = 1;
#endif
	...
	pAd->admitctrl.admchecktmr = 1;/*1 second*/
	pAd->admitctrl.admdutylevel0 = THERMAL_PROTECT_ADMIT_TIME_100_PERCENT;
	pAd->admitctrl.admdutylevel1 = THERMAL_PROTECT_ADMIT_TIME_60_PERCENT;
	pAd->admitctrl.admdutylevel2 = THERMAL_PROTECT_ADMIT_TIME_30_PERCENT;
	pAd->admitctrl.admdutylevel3 = THERMAL_PROTECT_ADMIT_TIME_10_PERCENT;
	pAd->admitctrl.admiten = FALSE;
	pAd->admitctrl.currentduty = THERMAL_PROTECT_DUTY_CYCLE_LEVEL_0;
	pAd->admitctrl.highen = FALSE;
	pAd->admitctrl.hightempth = 80;
	pAd->admitctrl.lowen = FALSE;
	pAd->admitctrl.lowtempth = 0;
#endif
...
VOID CMDHandler(RTMP_ADAPTER *pAd)
{
	...
	while (pAd && pAd->CmdQ.size > 0)
	{
		...
		/* For worst case, avoid process cmd.queue too long which cause RCU_sched stall */
		process_cnt++;
		if((!in_interrupt())&&(process_cnt >= (MAX_LEN_OF_CMD_QUEUE>>4))) {/*process_cnt-8*/
			process_cnt = 0;
			OS_SCHEDULE();
		}
				...
				case CMDTHREAD_PS_CLEAR:
					{
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("wcid=0x%x, Send Ps Clear CMD to MCU\n", wlanidx));
				...
				case CMDTHREAD_PS_RETRIEVE_START:
					{
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("wcid=%d CmdPsRetrieveStartReq CMD to MCU\n", pMacEntry->wcid));
				...
				case CMDTHREAD_APCLI_PBC_TIMEOUT:
					{
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("channel=%d CMDTHREAD_APCLI_PBC_TIMEOUT\n", channel));
