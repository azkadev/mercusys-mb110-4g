...
typedef struct _AssocSta
{
    ...
    /* Represent the band of the station associated with. 0 means 2.4G band, and 1 means 5G */
    uint8_t band;
} AssocSta;
	...
	IN IE_LISTS *ie_list)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\tAuthMode=%d, WepStatus=%d, WpaState=%d, GroupKeyWepStatus=%d\n",
		pEntry->AuthMode, pEntry->WepStatus, pEntry->WpaState, pAd->ApCfg.MBSSID[pEntry->func_tb_idx].wdev.GroupKeyWepStatus));
...
#if 0
			if (NdisEqualMemory(eid_ptr->Octet, TP_OUI, 3))
			{
				if (7 == *((UCHAR *)(eid_ptr->Octet + 5)))
				{
					pEntry->bSupportMWDS = TRUE;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Peer onemesh supports 4addr\n"));
				}
				break;
			}
#endif /* TP_IE_SUPPORT */
#endif /* MWDS */
			//break;	/* stupid break here: STA(AuthMode is WPAPSK) cannot connect because IE_WPA IE will never be resolved  */
            case IE_WPA2:
#ifdef DOT11R_FT_SUPPORT
#endif /* DOT11R_FT_SUPPORT */
#ifdef CONFIG_HOTSPOT_R2
	...
	IN BOOLEAN isReassoc)
{
    ...
    {
        ULONG tmpNow32;
        NdisGetSystemUpTime(&tmpNow32);
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(%d) @@@@ time is %lu  receive association:MAC %02x:%02x:%02x:%02x:%02x:%02x: \n",
         __FUNCTION__, __LINE__, tmpNow32, PRINT_MAC(ie_list->Addr2)));
    }
#endif
#endif /* MWDS */
...
#ifdef MWDS
		if (StatusCode == FT_STATUS_CODE_INVALID_MDIE)
#else
...
SendAssocResponse:
	...
	if ((pFtCfg != NULL) && (pFtCfg->FtCapFlag.Dot11rFtEnable))
	{
		...
		if (FtInfoBuf.FtIeInfo.MICCtr.field.IECnt)
		{
			...
			/* Install pairwise key *//*wpa2 vulnerability*/
			...
			tr_entry->PortSecured = WPA_802_1X_PORT_SECURED;
#ifdef MWDS
			if ((Ndis802_11AuthModeWPA2 == pEntry->AuthMode || Ndis802_11AuthModeWPA2PSK == pEntry->AuthMode)
				&& isReassoc && MLME_SUCCESS == StatusCode && SST_ASSOC == pEntry->Sst
					&& wdev->AuthMode >= Ndis802_11AuthModeWPA)
			{	/* to fix reassoc success but GTKState is REKEY_NEGOTIATING */
				pEntry->GTKState = REKEY_ESTABLISHED;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(%d) ReASSOC set GTKState REKEY_ESTABLISHED.\n", __FUNCTION__, __LINE__));
			}
#endif /* MWDS */
		}
	...
	if (StatusCode == MLME_SUCCESS)
	{
		...
		if (pEntry->func_tb_idx == MAIN_MBSSID)
		{
			...
			*/
		    ...
		    assoc.datarate_info.max_txpower = GetCuntryMaxTxPwr(pAd, pAd->CommonCfg.Channel);;
			...
			KV_PRINT(("%s %d send ATH_EVENT_BSTEERING_NODE_ASSOCIATED\r\n", __FUNCTION__, __LINE__));
...
/*		if (IS_FT_RSN_STA(pEntry) && (FtInfo.FtIeInfo.Len != 0)) */
        ...
        memcpy(assocsta.mac, (u_int8_t*)pEntry->Addr, MAC_ADDR_LEN);
        assocsta.band = 0; /* 0 means this is 2.4G sta */
        RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_CUSTOM, IW_ASSOC_EVENT_FLAG, NULL, (PUCHAR)&assocsta, sizeof(assocsta));
