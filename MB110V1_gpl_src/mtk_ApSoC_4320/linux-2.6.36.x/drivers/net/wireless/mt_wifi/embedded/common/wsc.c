...
void send_success_status_to_user(void)
{
	...
	struct sk_buff *skb = NULL;
	struct net *net = NULL;
	char buf[BUF_SIZE_RTNL] = "SUCCESS WPS";
	int len = strlen(buf);
...
/*	UCHAR 			Data[256]; */
	...
	if ((CurOpMode == AP_MODE) && (pWpsCtrl->bSetupLock || sharedWscLock))
	{
		 ...
		 * but WPS protocol require setupLock must be boolean type in wireless frame,
...
{	
	...
	*/
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> WscThreadExit\n"));
