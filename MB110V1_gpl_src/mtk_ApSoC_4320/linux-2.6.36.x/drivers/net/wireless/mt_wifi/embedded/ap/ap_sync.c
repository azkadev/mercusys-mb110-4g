	...
	*/
	...
	if (pEntry)
	{
		if (pEntry->func_tb_idx == MAIN_MBSSID)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s %d probe.rssi = %d\r\n", __FUNCTION__, __LINE__, probe.rssi));
			mtk_deliver_band_steering_event(RtmpOsGetNetIfIndex(pAd->ApCfg.MBSSID[apidx].wdev.if_dev), ATH_EVENT_BSTEERING_PROBE_REQ, sizeof(probe), (u_int8_t*)&probe);
		}
	}
#endif
	}
