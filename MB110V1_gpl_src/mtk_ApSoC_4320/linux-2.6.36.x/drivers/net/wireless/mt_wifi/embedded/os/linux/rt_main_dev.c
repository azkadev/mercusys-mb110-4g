...
int MainVirtualIF_close(IN struct net_device *net_dev)
{
	...
	RtmpOsMsDelay(500);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s RTMPInfClose done.\n", net_dev->name));
...
int rt28xx_open(VOID *dev)
{
	...
	rtmp_set_boot_flag(pAd, TRUE);
	printk("sht:pAd->ApBootFlag = TRUE\n");
...
/*	rtmp_irq_init(pAd); */
	...
	*/
	...
	rtmp_set_boot_flag(pAd, FALSE);
	printk("sht:pAd->ApBootFlag = FALSE\n");
