...
INT Set_DriverVersion_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pAd->chipCap.MCUType == ANDES) {
		...
		if (pAd->chipCap.need_load_fw) {
			...
			printk("MT7628-->fw version:%d.%d.%02d ", (fw_ver & 0xf000) >> 8,
							(fw_ver & 0x0f00) >> 8, fw_ver & 0x00ff);
			printk("MT7628-->build:%x\n", build_ver);
			printk("MT7628-->build time:");
...
INT	Set_Channel_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		{	
			...
			//send channel change event to network-roaming
			send_event_to_network_roaming(pAd->net_dev, IWEVCUSTOM, IEEE80211_EV_CHAN_CHANGE, &pAd->CommonCfg.Channel);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d: send_event_to_network_roaming\r\n", __FUNCTION__, __LINE__));
#endif
...
/*
    ...
    ==========================================================================
*/
#define	LINE_LEN	(4+33+3+20+23+9+8+7+3)	/* Channel+SSID+Len+Bssid+Security+Signal+WiressMode+ExtCh+NetworkType*/
	...
	IN  UINT32 MsgLen)
{
	...
	sprintf(msg+strlen(msg),"%-3d", pBss->SsidLen);
...
#ifdef TP_IE_SUPPORT
static UINT8 OneMeshSort(BSS_TABLE *pScanTable)
{
       UINT32 i = 0, j = 0;
       UINT32 idx = 0;
       BSS_ENTRY tmpEntry;
       ...
       BssTableSortByRssi(pScanTable, FALSE);
       
       for (i = 0; i < pScanTable->BssNr; i++)
       {
               pBss = &pScanTable->BssEntry[i];

               if (pBss->TpIE.support_mesh)
               {
                       memcpy(&tmpEntry, pBss, sizeof(BSS_ENTRY));
                       for (j = i; j >= idx + 1; j--)
                       {
                               memcpy(&pScanTable->BssEntry[j ], &pScanTable->BssEntry[j - 1], sizeof(BSS_ENTRY));
                       }
                       memcpy(&pScanTable->BssEntry[idx], &tmpEntry, sizeof(BSS_ENTRY));
                       idx++;
               }
       }

       MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("=========>idx: %d\n", idx));
       return 0;
}
#endif  /* TP_IE_SUPPORT */
...
#endif /* AIRPLAY_SUPPORT */	
       /* 2018.5.18 added by zhulin, onemesh */
#ifdef TP_IE_SUPPORT
       max_len += sizeof(TP_IE);
#endif /* TP_IE_SUPPORT */
...
#ifdef AIRPLAY_SUPPORT
        sprintf(msg+strlen(msg),"%-4s%-33s%-4s%-20s%-23s%-9s%-8s%-7s%-3s\n",
            "Ch", "SSID", "UN", "BSSID", "Security", "Siganl(%)", "W-Mode", " ExtCH"," NT");
#else
        sprintf(msg+strlen(msg),"%-4s%-33s%-3s%-20s%-23s%-9s%-8s%-7s%-3s\n",
...
#ifdef TP_IE_SUPPORT
       OneMeshSort(&activeScanTab);
#endif /* TP_IE_SUPPORT */

	for(i=0; i<activeScanTab.BssNr ;i++)
	{
		pBss = &activeScanTab.BssEntry[i];
...
#ifdef TP_IE_SUPPORT
		/* RE can only connect to OmeMesh APs whose leve is 0 or 1. */
		if (pBss->TpIE.support_mesh && pBss->TpIE.level >= 2)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("=========>SSID: %s\n", pBss->Ssid));
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("=========>LEVEL: %d \n", pBss->TpIE.level));
			continue;
		}
#endif /* TP_IE_SUPPORT */
		...
		/* Add TP-Link Spcific Vendor IE Support */
#ifdef TP_IE_SUPPORT
               tmpLen = strlen(msg) - 1;
               if (pBss->TpIE.IELen > 0)
               {
                       /* vendor: 221 */
                       sprintf(msg + tmpLen,"%02x", 221);
                       tmpLen += 2;
                       /* len */
                       sprintf(msg + tmpLen,"%02x", pBss->TpIE.IELen + 3);
                       tmpLen += 2;
                       /* OUI */
                       for (j = 0; j < 3; j++)
                       {
                               sprintf(msg + tmpLen,"%02x", pBss->TpIE.OUI[j]);
                               tmpLen += 2;
                       }
                       /* IE */
                       ptmp = pBss->TpIE.IE;
               ...
               sprintf(msg+tmpLen,"\n");
#ifdef MWDS
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPIoctlGetSiteSurvey TpIE.IELen:%d TPIE: = %-128s ", pBss->TpIE.IELen, pBss->TpIE.IE));

		if (pBss->TpIE.IELen > 0 && pBss->TpIE.IELen < TP_IE_MAX_LEN)
		{
			INT32 i = 0;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("TPIE len:%d content:\n", pBss->TpIE.IELen));
			for(i = 0; i < pBss->TpIE.IELen; i++)
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("[%d]:0x%02X  ", i, pBss->TpIE.IE[i]));
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	for (i=0; i < pAd->ChannelListNum; i++)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BuildChannel # %d :: Pwr0 = %d, Pwr1 =%d, Flags = %x\n ",
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
    ...
    Idx = BssTableSearch(&pAd->ScanTab, tpie->tp_macaddr, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Channel);
    if (BSS_NOT_FOUND != Idx && TRUE == bConnect)
    {
        if (pAd->ScanTab.BssEntry[Idx].TpIE.IELen > 0 && pAd->ScanTab.BssEntry[Idx].TpIE.IELen < TP_IE_MAX_LEN)
        {
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPIoctlGetTPIE TPIE len:%d content:\n", pAd->ScanTab.BssEntry[Idx].TpIE.IELen));
            for(i=0; i < pAd->ScanTab.BssEntry[Idx].TpIE.IELen; i++)
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("[%d]:0x%02X  ", i, pAd->ScanTab.BssEntry[Idx].TpIE.IE[i]));
        }

        awnNetInfo.id = 221;
        awnNetInfo.len = pAd->ScanTab.BssEntry[Idx].TpIE.IELen;
        awnNetInfo.oui[0] = pAd->ScanTab.BssEntry[Idx].TpIE.OUI[0];
        awnNetInfo.oui[1] = pAd->ScanTab.BssEntry[Idx].TpIE.OUI[1];
        awnNetInfo.oui[2] = pAd->ScanTab.BssEntry[Idx].TpIE.OUI[2];
        memcpy(awnNetInfo.tpie, pAd->ScanTab.BssEntry[Idx].TpIE.IE, TP_IE_MAX_LEN);
    ...
    IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
    ...
    /* allocate memory */
    maxtblen = sizeof(MAC_STAT_ENTRY) * MAX_LEN_OF_MAC_TABLE;
    os_alloc_mem(NULL, (UCHAR **)&pMacTab, maxtblen);
    if (pMacTab == NULL)
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: Allocate memory fail!!!\n", __FUNCTION__));
        return;
    ...
    for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++)
    {
        ...
        if ((pEntry->Sst == SST_ASSOC) && (IS_ENTRY_APCLI(pEntry) || IS_ENTRY_CLIENT(pEntry)))
        {
            memcpy(pMacTab[entry_num].Addr, pEntry->Addr, sizeof(UCHAR) * MAC_ADDR_LEN);
#ifdef MAC_REPEATER_SUPPORT
			pMacTab[entry_num].IsReptCli = !!pEntry->bReptCli;
            memcpy(pMacTab[entry_num].ReptCliAddr, pEntry->ReptCliAddr, sizeof(UCHAR) * MAC_ADDR_LEN);
            pMacTab[entry_num].OneSecTxBytes = pEntry->OneSecTxBytesTotal;
            pMacTab[entry_num].OneSecRxBytes = pEntry->OneSecRxBytesTotal;
#endif
            pMacTab[entry_num].Aid = pEntry->Aid;
            pMacTab[entry_num].Wcid = pEntry->wcid;
            pMacTab[entry_num].IsCli = IS_ENTRY_CLIENT(pEntry);
            pMacTab[entry_num].IsApCli = IS_ENTRY_APCLI(pEntry);
            pMacTab[entry_num].AvgRssi0 = pEntry->RssiSample.AvgRssi[0];
            pMacTab[entry_num].AvgRssi1 = pEntry->RssiSample.AvgRssi[1];
            pMacTab[entry_num].AvgRssi2 = pEntry->RssiSample.AvgRssi[2];
            pMacTab[entry_num].LastTxOkCount = pEntry->LastTxOkCount;
            pMacTab[entry_num].LastTxPER = pEntry->LastTxPER;
            getRate(pEntry->HTPhyMode, &(pMacTab[entry_num].AssocRate));
            pMacTab[entry_num].AssocBW = pEntry->HTPhyMode.field.BW;
            pMacTab[entry_num].AssocMode = pEntry->HTPhyMode.field.MODE;

            entry_num++;
        }
    }

    tblen = entry_num * sizeof(MAC_STAT_ENTRY);
    if (wrq->u.data.length < tblen)
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Not enough user space for mac stat\n", __FUNCTION__));
        goto end;
    }

    wrq->u.data.length = tblen;
    if (copy_to_user(wrq->u.data.pointer, pMacTab, tblen))
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: copy_to_user() fail\n", __FUNCTION__));
    }
...
#ifdef MAC_REPEATER_SUPPORT
VOID RTMPIoctlGetReptCliMapTable(
    IN RTMP_ADAPTER *pAd,
    IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
    REPTCLI_MAP_ENTRY *pReptCliTab = NULL;
    ...
    /* allocate memory */
    maxtblen = sizeof(REPTCLI_MAP_ENTRY) * MAX_EXT_MAC_ADDR_SIZE;
    os_alloc_mem(NULL, (UCHAR **)&pReptCliTab, maxtblen);
    if (pReptCliTab == NULL)
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: Allocate memory fail!!!\n", __FUNCTION__));
        return;
    ...
    NdisZeroMemory(pReptCliTab, maxtblen);
    for (i = 0; i < MAX_EXT_MAC_ADDR_SIZE; i++)
    {
        pReptCliEntry = &(pAd->ApCfg.ApCliTab[0].RepeaterCli[i]);

        if (pReptCliEntry->CliEnable || pReptCliEntry->CliValid)
        {
            memcpy(pReptCliTab[entry_num].OriAddr, pReptCliEntry->OriginalAddress, sizeof(UCHAR) * MAC_ADDR_LEN);
            memcpy(pReptCliTab[entry_num].ReptCliAddr, pReptCliEntry->CurrentAddress, sizeof(UCHAR) * MAC_ADDR_LEN);
            pReptCliTab[entry_num].IsEnabled = pReptCliEntry->CliEnable;
            pReptCliTab[entry_num].IsValid = pReptCliEntry->CliValid;
            pReptCliTab[entry_num].IsEthCli = pReptCliEntry->bEthCli;
            pReptCliTab[entry_num].MatchApCliIdx = pReptCliEntry->MatchApCliIdx;
            pReptCliTab[entry_num].MatchLinkIdx = pReptCliEntry->MatchLinkIdx;
            pReptCliTab[entry_num].Wcid = pReptCliEntry->MacTabWCID;

            entry_num++;
        }
    }

    tblen = entry_num * sizeof(REPTCLI_MAP_ENTRY);
    if (wrq->u.data.length < tblen)
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Not enough user space for mac stat\n", __FUNCTION__));
        goto end;
    ...
    wrq->u.data.length = tblen;
    if (copy_to_user(wrq->u.data.pointer, pReptCliTab, tblen))
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: copy_to_user() fail\n", __FUNCTION__));
    }
...
#if 0 //def TP_IE_SUPPORT
		if (TRUE == bConnect)
		{
				ULONG Idx;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ApCli%d Connected Status : Connected \n", ifIndex ));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ApCli%d Connected AP-BSSID : %02X:%02X:%02X:%02X:%02X:%02X \n", ifIndex, PRINT_MAC(pEntry->Addr)));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ApCli%d Connected AP-SSID :  %s \n", ifIndex, pAd->ApCfg.ApCliTab[ifIndex].Ssid));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ApCli%d Connected LinkTime:  %lu \n", ifIndex, pAd->ApCfg.ApCliTab[ifIndex].ApCliLinkUpTime));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ApCli%d Connected RSSI : %-5d,%-5d,%-5d \n", ifIndex, pEntry->RssiSample.AvgRssi0,pEntry->RssiSample.AvgRssi1,pEntry->RssiSample.AvgRssi2));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ApCli%d Connected Rate : [BW-PhyMode-ShortGI-ldpc-STBC-MCS]= %-4s%-6s%-2d%-2d%-2d%-4d \n", ifIndex, get_bw_str(pEntry->HTPhyMode.field.BW),
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ApCli%d Connected WirelessMode : %-6s \n", ifIndex, get_phymode_str(pEntry->HTPhyMode.field.MODE)));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ApCli%d Connected BW : %-4s \n", ifIndex, get_bw_str(pEntry->MaxHTPhyMode.field.BW)));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ApCli%d Connected Channel : %d \n", ifIndex, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Channel));

				Idx = BssTableSearch(&pAd->ScanTab, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Bssid, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Channel);
				if (Idx != BSS_NOT_FOUND)
				{
					if (pAd->ScanTab.BssEntry[Idx].TpIE.IELen > 0 && pAd->ScanTab.BssEntry[Idx].TpIE.IELen < TP_IE_MAX_LEN)
					{
						INT32 i = 0;
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("TPIE len:%d content:\n", pAd->ScanTab.BssEntry[Idx].TpIE.IELen));
						for(i=0; i < pAd->ScanTab.BssEntry[Idx].TpIE.IELen; i++)
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[%d]:0x%02X  ", i, pAd->ScanTab.BssEntry[Idx].TpIE.IE[i]));
					}

					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ApCli%d Connected TPIE : OUI[%02X-%02X-%02X],IE[%-128s] \n",
						ifIndex,
						pAd->ScanTab.BssEntry[Idx].TpIE.OUI[0],
						pAd->ScanTab.BssEntry[Idx].TpIE.OUI[1],
						pAd->ScanTab.BssEntry[Idx].TpIE.OUI[2],
						pAd->ScanTab.BssEntry[Idx].TpIE.IE));
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ApCli%d Connected TPIE : NULL \n",ifIndex));
		}
#endif /* TP_IE_SUPPORT */
...
INT show_memdump_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
...
#endif /* MWDS */
	{
		printk("mem addr error! should be 0x80xxxxxx or 0xa0xxxxxx\n");
		return TRUE;
	}
	...
	//hex_dump
	{
		...
		printk("memdump: %08x, len = %d\n",  addr, MemDumpLen);
		for (x = 0; x < MemDumpLen; x++)
		{
			if (x % 16 == 0)
				printk("0x%04x : ", x);
			printk("%02x ", (*((unsigned char*)addr+x)));
			if (x%16 == 15) printk("\n");
		}
		printk("\n");
	}
...
INT show_trinfo_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	// TODO: shiang-7603
		...
		if (pAd->chipCap.hif_type == HIF_MT)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tPDMAMonitorEn=%d, TxRCounter = %lu, TxDMACheckTimes = %d,  RxRCounter = %lu, Rx0FullLS0Count = %lu, RxDMACheckTimes = %d, PDMARFailCount = %lu\n", pAd->PDMAWatchDogEn, pAd->TxDMAResetCount, pAd->TxDMACheckTimes, pAd->RxDMAResetCount, pAd->Rx0FullLS0Count,pAd->RxDMACheckTimes, pAd->PDMAResetFailCount));
#endif			
				...
				&& (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED))
 				{
					...
					if (pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.bSupportMWDS)
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MWDSCap : YES\n"));
					else
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MWDSCap : NO\n"));
#endif /* MWDS */
...
{
#ifdef DBG
#if !defined(TP_LOG) || (defined(TP_LOG_TRACE)||defined(TP_LOG_INFO)||defined(TP_LOG_LOUD)||defined(TP_LOG_NOISY))
	HT_CAP_INFO			*pHTCap;
...
INT show_admit_ctrl_info(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT16 temp = 0;
	UINT8 pctglv0 = 0 ,pctglv1 = 0,pctglv2 = 0,pctglv3 = 0;
	UINT32 macval = 0;
	temp = MtAsicGetThemalSensor(pAd, 0);

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TEMP[%d]\nCR SETTING:\n", temp));

	RTMP_IO_READ32(pAd,WF_AON_MTR1,&macval);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("THERM AVERAGE PERIOD: 0X%x\n",
		(macval & MTR1_THERM_AVERAGE_PERIOD_MASK) >> 8));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("THERM UINT:%s\n",
		((macval & MTR1_THERM_MT_UNIT_64_US) == MTR1_THERM_MT_UNIT_64_US) ? "64US":"SEC"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("THERM ADMIT %s\n",
		((macval & MTR1_THERM_ADMIT_TIME_EN) == MTR1_THERM_ADMIT_TIME_EN) ? "ENABLED":"DISABLED"));

	RTMP_IO_READ32(pAd,WF_AON_MTR0,&macval);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("THERM ADMIT TIME: 0X%x\nDUTY LEVEL:\n",
		(macval & MTR0_THERM_ADMIT_TIME_MASK) >> 16));
	/*recovery the percentage*/
	pctglv0 = (pAd->admitctrl.admdutylevel0*1000/THERMAL_PROTECT_ADMIT_TIME_100_PERCENT + 5)/10;
	pctglv1 = (pAd->admitctrl.admdutylevel1*1000/THERMAL_PROTECT_ADMIT_TIME_100_PERCENT + 5)/10;
	pctglv2 = (pAd->admitctrl.admdutylevel2*1000/THERMAL_PROTECT_ADMIT_TIME_100_PERCENT + 5)/10;
	pctglv3 = (pAd->admitctrl.admdutylevel3*1000/THERMAL_PROTECT_ADMIT_TIME_100_PERCENT + 5)/10;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("lv0(0x%x,%d%%)-lv1(0x%x,%d%%)-lv2(0x%x,%d%%)-lv3(0x%x,%d%%)\n",
		pAd->admitctrl.admdutylevel0,pctglv0,
		pAd->admitctrl.admdutylevel1,pctglv1,
		pAd->admitctrl.admdutylevel2,pctglv2,
		pAd->admitctrl.admdutylevel3,pctglv3));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Hen(%d)-HTemp(%d)-Len(%d)-LTemp(%d)\n",
		pAd->admitctrl.highen,pAd->admitctrl.hightempth,pAd->admitctrl.lowen,pAd->admitctrl.lowtempth));

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("THERM CHECK TIME %d SECONDS\n", pAd->admitctrl.admchecktmr));

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("THERM ADMIT DRIVER %s\n", pAd->admitctrl.admiten ? "ENABLED":"DISABLED"));
