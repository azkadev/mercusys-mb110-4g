    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
		...
		pEntry->AuthMode == Ndis802_11AuthModeWPA2)
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			if ( 0xFF == CliIdx )
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d Clear CfgApCliBssid.\n", __FUNCTION__, __LINE__));
			...
			if (pEntry && (pEntry->func_tb_idx < MAX_APCLI_NUM) && tr_entry) {
				if (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED)
				{
					...
					pAd->ApCfg.ApCliTab[ifIndex].wdev.bSupportMWDS )
						...
						MWDSAPCliPeerDisable(pAd, &pAd->ApCfg.ApCliTab[pEntry->func_tb_idx], pEntry);
				}
			}
#endif /* MWDS */
#endif /* APLCI_SUPPORT */
	...
	IN  MAC_TABLE_ENTRY *pEntry)
{
	...
	{
		/* Prevent the GTK reinstall key attack *//*wpa2 vulnerability*/
