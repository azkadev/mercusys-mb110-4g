	...
	if (pAd->Mlme.OneSecPeriodicRound % 4 == 0)
	{
		...
		if (bAutoTxAgc)
		{
			...
			if (BbpR49.byte > pTssiMinusBoundary[1])
			{
				...
				*pTxAgcCompensate = -(TxAgcStep * (idx-1));			
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-- Tx Power, BBP R49=%x, TssiRef=%x, TxAgcStep=%x, step = -%d\n",
					                BbpR49.byte, (pAd->CommonCfg.Channel <= 14) ? pAd->TssiRefG : pAd->TssiRefA, TxAgcStep, idx-1));                    
			}
			...
			else
			{
				*pTxAgcCompensate = 0;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("   Tx Power, BBP R49=%x, TssiRef=%x, TxAgcStep=%x, step = +%d\n",
				                	BbpR49.byte, (pAd->CommonCfg.Channel <= 14) ? pAd->TssiRefG : pAd->TssiRefA, TxAgcStep, 0));
			}
		}
	}
	else
