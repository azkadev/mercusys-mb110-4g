...
#ifdef TP_NETWORK_ROAMING
		mtk_band_steering_netlink_init();
#endif
...
VOID MacTableMaintenance(RTMP_ADAPTER *pAd)
{
	...
	for (i = startWcid; i < MAX_LEN_OF_MAC_TABLE; i++)
	{
		...
		*/
		...
		{
			RTMPGetScSRssi tmpgetscsrssi = pAd->SCSCtrl.SCSMode ? RTMPMaxRssi : RTMPMinRssi;
			CHAR tmpRssi = tmpgetscsrssi(pAd, pEntry->RssiSample.AvgRssi[0], pEntry->RssiSample.AvgRssi[1], pEntry->RssiSample.AvgRssi[2]);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("STA %02x:%02x:%02x:%02x:%02x:%02x ,RSSI 0=%d 1=%d 2=%d Selected=%d\n",
					PRINT_MAC(pEntry->Addr), pEntry->RssiSample.AvgRssi[0],  pEntry->RssiSample.AvgRssi[1], pEntry->RssiSample.AvgRssi[2], tmpRssi));
...
void tx_admit_ctrl_switch(RTMP_ADAPTER *pAd,UINT8 ctrl)
{
	UINT32 macval = 0;
	RTMP_IO_READ32(pAd,WF_AON_MTR1,&macval);
	if (ctrl) {
		macval |= MTR1_THERM_ADMIT_TIME_EN;
	}
	else {
		macval &= ~MTR1_THERM_ADMIT_TIME_EN;
	}
	RTMP_IO_WRITE32(pAd,WF_AON_MTR1,macval);

	return;
}
void tx_admit_parameter_set(RTMP_ADAPTER *pAd,UINT16 admit_time,UINT8 admit_period,UCHAR time_unit)
{
	UINT32 macval = 0;
	/*set time unit is 64us , and average period.*/
	RTMP_IO_READ32(pAd,WF_AON_MTR1,&macval);
	macval &= ~(MTR1_THERM_AVERAGE_PERIOD_MASK | MTR1_THERM_MT_UNIT_64_US);
	macval |= MTR1_THERM_AVERAGE_PERIOD(admit_period);
	if (time_unit) {
		macval |= MTR1_THERM_MT_UNIT_64_US;
	}
	RTMP_IO_WRITE32(pAd,WF_AON_MTR1,macval);
	/*set admit time*/
	RTMP_IO_READ32(pAd,WF_AON_MTR0,&macval);
	macval &= ~(MTR0_THERM_ADMIT_TIME_MASK);
	macval |= MTR0_THERM_ADMIT_TIME(admit_time);
	RTMP_IO_WRITE32(pAd,WF_AON_MTR0,macval);

	return;
}
void tx_admit_thermal_protect(RTMP_ADAPTER *pAd)
{
	INT32 temperature = 0;
	temperature = MtAsicGetThemalSensor(pAd, 0);
	if (!pAd->admitctrl.admiten)
		return;

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s CHECK TIME RUN!\n",__FUNCTION__));
	if (pAd->admitctrl.highen && (temperature >= pAd->admitctrl.hightempth)) {/*High Temp*/
		if ((pAd->admitctrl.currentduty == THERMAL_PROTECT_DUTY_CYCLE_LEVEL_0)
			&& (pAd->admitctrl.admdutylevel1 < pAd->admitctrl.admdutylevel0)) {
			pAd->admitctrl.currentduty = THERMAL_PROTECT_DUTY_CYCLE_LEVEL_1;
			tx_admit_ctrl_switch(pAd,FALSE);
			tx_admit_parameter_set(pAd,pAd->admitctrl.admdutylevel1,THERMAL_PROTECT_ADMIT_PERIOD,TIME_UNIT_64US);
			tx_admit_ctrl_switch(pAd,TRUE);
		}
		else if ((pAd->admitctrl.currentduty == THERMAL_PROTECT_DUTY_CYCLE_LEVEL_1)
			&& (pAd->admitctrl.admdutylevel2 < pAd->admitctrl.admdutylevel1)) {
			pAd->admitctrl.currentduty = THERMAL_PROTECT_DUTY_CYCLE_LEVEL_2;
			tx_admit_ctrl_switch(pAd,FALSE);
			tx_admit_parameter_set(pAd,pAd->admitctrl.admdutylevel2,THERMAL_PROTECT_ADMIT_PERIOD,TIME_UNIT_64US);
			tx_admit_ctrl_switch(pAd,TRUE);
		}
		else if ((pAd->admitctrl.currentduty == THERMAL_PROTECT_DUTY_CYCLE_LEVEL_2)
			&& (pAd->admitctrl.admdutylevel3 < pAd->admitctrl.admdutylevel2)) {
			pAd->admitctrl.currentduty = THERMAL_PROTECT_DUTY_CYCLE_LEVEL_3;
			tx_admit_ctrl_switch(pAd,FALSE);
			tx_admit_parameter_set(pAd,pAd->admitctrl.admdutylevel3,THERMAL_PROTECT_ADMIT_PERIOD,TIME_UNIT_64US);
			tx_admit_ctrl_switch(pAd,TRUE);
		}
		else {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Thermal higer(%d),please adjust duty!\n",temperature));
		}
	}
	else {/*Low Temp*/
		if (pAd->admitctrl.lowen && (temperature <= pAd->admitctrl.lowtempth)) {
			if (pAd->admitctrl.currentduty == THERMAL_PROTECT_DUTY_CYCLE_LEVEL_3) {
				pAd->admitctrl.currentduty = THERMAL_PROTECT_DUTY_CYCLE_LEVEL_2;
				tx_admit_ctrl_switch(pAd,FALSE);
				tx_admit_parameter_set(pAd,pAd->admitctrl.admdutylevel2,THERMAL_PROTECT_ADMIT_PERIOD,TIME_UNIT_64US);
				tx_admit_ctrl_switch(pAd,TRUE);
				...
				if (pAd->admitctrl.admdutylevel0 != THERMAL_PROTECT_ADMIT_TIME_100_PERCENT)
					...
					tx_admit_ctrl_switch(pAd,FALSE);/*turn off admit control.*/
			}
		}
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:Tem(%d),Duty(%d)\n",__FUNCTION__,temperature,pAd->admitctrl.currentduty));
	return;
}
#endif