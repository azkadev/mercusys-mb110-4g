...
INT	RT_CfgSetApcliMacAddress(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT	i, mac_len;
	...
	for (i = 0; i < MAC_ADDR_LEN; i++)
	{
		AtoH(arg, &pAd->ApcliCustomAddress[i], 1);
		arg = arg + 3;
	}
    pAd->bCustomApcliMAC = TRUE;
		...
		/* interface up */
		{
			...
			if (VIRTUAL_IF_NUM(pAd) == 0)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("WiFi Startup Cost (%s): %lu.%03lus\n",
						RTMP_OS_NETDEV_GET_DEVNAME(pAd->net_dev),((end-start)*1000/OS_HZ)/1000,((end-start)*1000/OS_HZ)%1000));
			}
			else
			{
				VIRTUAL_IF_INC(pAd);
...
INT Set_MemDumpLen_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	{
		MemDumpLen = Dbg;
		printk("set MemDumpLen=>%d bytes\n",Dbg);
	}
	else
	{
		MemDumpLen = 32;
		printk("set MemDumpLen=>default 32 bytes\n");
	}
...
INT Set_RX0LS0_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	UINT32 Dbg;

    Dbg = simple_strtol(arg, 0, 10);

	if (Dbg >= 100 && Dbg < (100+RX_RING_SIZE) )//100+RX ring size
	{
...
#ifdef RT_BIG_ENDIAN
		pDestRxD = (RXD_STRUC *)pRxRing->Cell[Dbg-100].AllocVa;
		RxD = *pDestRxD;
		pRxD = &RxD;
		RTMPDescriptorEndianChange((PUCHAR)pRxD, TYPE_RXD);
#else
		/* Point to Rx indexed rx ring descriptor*/
		pRxD  = (RXD_STRUC *)pRxRing->Cell[Dbg-100].AllocVa;
#endif /* RT_BIG_ENDIAN */
		pRxD->DDONE = 0;//1 will lead Tx/Rx coherent

		printk("set RX0 one done bit\n");
	}
	else if (Dbg >= 400 && Dbg < (400+RX_RING_SIZE) )//400+RX ring size
	{
		RTMP_RX_RING *pRxRing;
		RXD_STRUC *pRxD;
#ifdef RT_BIG_ENDIAN
		...
		pRxRing = &pAd->RxRing[0];
#ifdef RT_BIG_ENDIAN
		pDestRxD = (RXD_STRUC *)pRxRing->Cell[Dbg-400].AllocVa;
		RxD = *pDestRxD;
		pRxD = &RxD;
		RTMPDescriptorEndianChange((PUCHAR)pRxD, TYPE_RXD);
#else
		/* Point to Rx indexed rx ring descriptor*/
		pRxD  = (RXD_STRUC *)pRxRing->Cell[Dbg-400].AllocVa;
#endif /* RT_BIG_ENDIAN */
		pRxD->DDONE = 0;

		printk("set RX0 one done bit\n");
	}
	else if(Dbg >= 700 && Dbg < (700+RX_RING_SIZE))//700+RX ring size
	{
		RTMP_RX_RING *pRxRing;
		RXD_STRUC *pRxD;
#ifdef RT_BIG_ENDIAN
		...
		pRxRing = &pAd->RxRing[0];
#ifdef RT_BIG_ENDIAN
		pDestRxD = (RXD_STRUC *)pRxRing->Cell[Dbg-700].AllocVa;
		RxD = *pDestRxD;
		pRxD = &RxD;
		RTMPDescriptorEndianChange((PUCHAR)pRxD, TYPE_RXD);
#else
		/* Point to Rx indexed rx ring descriptor*/
		pRxD  = (RXD_STRUC *)pRxRing->Cell[Dbg-700].AllocVa;
#endif /* RT_BIG_ENDIAN */
		pRxD->LS0 = 1;

		printk("set RX0 one LS0 bit\n");
	}
	else if(Dbg >= 1000 && Dbg < (1000+RX_RING_SIZE))//1000+RX ring size
	{
		RTMP_RX_RING *pRxRing;
		RXD_STRUC *pRxD;
#ifdef RT_BIG_ENDIAN
		...
		pRxRing = &pAd->RxRing[0];
#ifdef RT_BIG_ENDIAN
		pDestRxD = (RXD_STRUC *)pRxRing->Cell[Dbg-1000].AllocVa;
		RxD = *pDestRxD;
		pRxD = &RxD;
		RTMPDescriptorEndianChange((PUCHAR)pRxD, TYPE_RXD);
#else
		/* Point to Rx indexed rx ring descriptor*/
		pRxD  = (RXD_STRUC *)pRxRing->Cell[Dbg-1000].AllocVa;
#endif /* RT_BIG_ENDIAN */
		pRxD->LS0 = 0;

		printk("set RX0 one LS0 bit\n");
	}
	else if((Dbg == 3)||(Dbg == 30))
	{
		RTMP_RX_RING *pRxRing;
		RXD_STRUC *pRxD;
#ifdef RT_BIG_ENDIAN
...
#endif /* RT_BIG_ENDIAN */
			pRxD->DDONE = (Dbg==3)?1:0;
		}

		printk("set RX0 all done bit\n");
	}
	else if((Dbg == 4)||(Dbg == 40))
	{
		RTMP_RX_RING *pRxRing;
		RXD_STRUC *pRxD;
#ifdef RT_BIG_ENDIAN
	...
	else if(Dbg == 5)
	{
		...
		printk("set DMAD then reset\n");

	}
	else
	{
		printk("Input 1/2/3/4/5,others not work! \n");
	}
...
INT set_get_rx_fid(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
    //TODO: Carter, at present, only can read pkt in Port2(LMAC port)
    volatile UCHAR   q_idx = 0, loop = 0, dw_idx = 0;
    volatile UINT32  head_fid_addr = 0, next_fid_addr = 0, value = 0x00000000L, dw_content;
    ...
    value = 0x00000000 | (q_idx << 16);//port0. queue by input value.
    RTMP_IO_WRITE32(pAd, 0x8024, value);
    RTMP_IO_READ32(pAd, 0x8024, (UINT32 *)&head_fid_addr);//get head FID.
    head_fid_addr = head_fid_addr & 0xfff;
...
#ifdef DUTY_ADMIT_SUPPORT
	Pos = rtstrstr(Pos, "chkt-");
	if (Pos != NULL) {
		Pos = Pos + 5;
		checktimer = simple_strtol(Pos, 0, 10);
	}
	else
		goto error;
#endif
...
#ifdef MT_MAC
#ifdef DUTY_ADMIT_SUPPORT
	if (pAd->admitctrl.admiten) { /*duty cycle enabled, disable TXStream*/
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: high_en=%d, high_thd = %d, low_en = %d, low_thd = %d, checktimer = %d\n",
			__FUNCTION__, HighEn, HighTempTh, LowEn, LowTempTh,checktimer));
		/*update the admit control parameters from commands*/
		pAd->admitctrl.highen = HighEn;
		pAd->admitctrl.hightempth = HighTempTh;
		pAd->admitctrl.lowen = LowEn;
		pAd->admitctrl.lowtempth = LowTempTh;
		pAd->admitctrl.admchecktmr = checktimer;/*How many seconds.*/
	}
	else
#endif
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: high_en=%d, high_thd = %d, low_en = %d, low_thd = %d\n",
	...
	IN RTMP_STRING		*arg)
{
	...
	UINT32 dutylv0 = 0,dutylv1 = 0,dutylv2 = 0,dutylv3 = 0;
	INT ret = 0;
	pAd->admitctrl.admiten = FALSE;
	ret = sscanf(arg,"%d-%d-%d-%d",&dutylv0,&dutylv1,&dutylv2,&dutylv3);
	...
	if (ret !=4) {
		...
		pAd->admitctrl.admdutylevel0 = dutylv0*THERMAL_PROTECT_ADMIT_TIME_100_PERCENT/100;
		pAd->admitctrl.admdutylevel1 = dutylv1*THERMAL_PROTECT_ADMIT_TIME_100_PERCENT/100;
		pAd->admitctrl.admdutylevel2 = dutylv2*THERMAL_PROTECT_ADMIT_TIME_100_PERCENT/100;
		pAd->admitctrl.admdutylevel3 = dutylv3*THERMAL_PROTECT_ADMIT_TIME_100_PERCENT/100;
		tx_admit_parameter_set(pAd,pAd->admitctrl.admdutylevel0,THERMAL_PROTECT_ADMIT_PERIOD,TIME_UNIT_64US);
		pAd->admitctrl.currentduty = THERMAL_PROTECT_DUTY_CYCLE_LEVEL_0;
		if (pAd->admitctrl.admdutylevel0 == THERMAL_PROTECT_ADMIT_TIME_100_PERCENT)
			tx_admit_ctrl_switch(pAd,FALSE);
		else
			tx_admit_ctrl_switch(pAd,TRUE);
		pAd->admitctrl.admiten = TRUE;
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("dutylv0 = %d, dutylv1 = %d, dutylv2 = %d, dutylv3 = %d, admiten = %d\n",
		dutylv0,dutylv1,dutylv2,dutylv3,pAd->admitctrl.admiten));

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("Regdutylv0 = 0x%x, Regdutylv1 = 0x%x, Regdutylv2 = 0x%x, Regdutylv3 = 0x%x\n",
		pAd->admitctrl.admdutylevel0,pAd->admitctrl.admdutylevel1,
		pAd->admitctrl.admdutylevel2,pAd->admitctrl.admdutylevel3));
	return TRUE;
}

INT set_admin_en_dis_proc(
	IN PRTMP_ADAPTER	pAd,
	IN RTMP_STRING		*arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,(" admit ctrl %s.",pAd->admitctrl.admiten ? "Enabled":"Disabled"));
	return TRUE;
}
#endif
...
INT SetSCSMode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	UINT32 value = 0;

	value = simple_strtol(arg, 0, 10);
	pAd->SCSCtrl.SCSMode = value == 0 ? 0 : 1;
	return TRUE;
}
INT SetSCSRSSI_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	long h2l = 0, l2h = 0;
	char *sep = strchr(arg, ':');
	...
	if (sep == NULL || *sep == 0)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("==>%s(%d)  wrong format. example:\n-57:-66\n", __FUNCTION__, __LINE__));
		return TRUE;
	}
	*sep = 0;
	h2l = simple_strtol(arg, 0, 10);
	l2h = simple_strtol(sep + 1, 0, 10);
	if (h2l >= 0 || h2l <= -127 || l2h >= 0 || l2h <= -127 || h2l <= l2h)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("==>%s(%d)  wrong format.  example:\n-57:-66\n", __FUNCTION__, __LINE__));
		return TRUE;
	}
	pAd->SCSCtrl.SCSH2L = h2l;
	pAd->SCSCtrl.SCSL2H = l2h;

	return TRUE;
}
INT SetSCSTxrx_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	UINT32 value = 0;

	value = simple_strtol(arg, 0, 10);
	pAd->SCSCtrl.SCSTxrx = value == 0 ? 0 : 1;
	return TRUE;
}
...
rssi_stat));
	...
	idx = start;
	do {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t%d", (ed_stat[idx] * 100) / (pAd->ed_chk_period * 1000)));
		INC_RING_INDEX(idx, ED_STAT_CNT);
	} while (idx != end);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
	...
	*/
	...
	idx = start;
	do {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t%d", (idle_stat[idx] *100)/ (pAd->ed_chk_period * 1000)));
		INC_RING_INDEX(idx, ED_STAT_CNT);
	} while (idx != end);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n==========================\n"));	
	...
	idx = start;
	do {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t%d", (busy_stat[idx] *100 )/ (pAd->ed_chk_period * 1000)));
		INC_RING_INDEX(idx, ED_STAT_CNT);
	} while (idx != end);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n==========================\n"));
