...
VOID MlmeHandler(RTMP_ADAPTER *pAd) 
{
		...
		) 
	{
		...
		process_cnt++;
		if((!in_interrupt())&&(process_cnt >= (MAX_LEN_OF_MLME_QUEUE>>2))) {/*process_cnt-8*/
			process_cnt = 0;
			OS_SCHEDULE();
		}
...
extern unsigned int TP_WSCLEDStatus_5G;
extern unsigned int TP_WSCLEDStatus_24G;
...
extern void led_WpsFinish(void);
#if 0
/* added by ZC for single led */
extern void led_startWpsConn(void);
extern void led_stopWpsConn(void);
/* end added */
#endif
...
{
	/*
	if (TP_WSCLEDStatus_24G == TP_WSC_LED_SUCCESS && WSCLedStatus == TP_WSC_LED_END)
	{
		return;
	}
	*/
	TP_WSCLEDStatus_24G = WSCLedStatus;

	switch (TP_WSCLEDStatus_24G)
	{
	case TP_WSC_LED_END:
		TP_WSCLedTimeout = 0;
		//led_stopWpsConn();
...
VOID TP_CheckWSCLEDStatus(PWSC_CTRL pWscCtrl)
{
	...
	{
		TP_WSCLEDStatus_24G = TP_WSC_LED_END;
	}
	...
	if (TP_WSCLedStatus_old == TP_WSCLEDStatus_24G)
	{
	...
	{
		cnt = 0;
		TP_WSCLedStatus_old = TP_WSCLEDStatus_24G;
	}
	...
	/* zeng weiji, 2.4G LED灯优先级高于5G, 如果5G WPS已连接, 2.4G WPS还在进行, 会导致灯持续闪烁, 不合理, 所以这里同步5G的状态 */
	if (TP_WSCLEDStatus_5G == TP_WSC_LED_SUCCESS)
		TP_WSCLEDStatus_24G = TP_WSC_LED_END;
...
/* pAd->admitctrl.admchecktmr: how many seconds */
	if(pAd->Mlme.OneSecPeriodicRound % pAd->admitctrl.admchecktmr == 0) {
		// MAJOR API
		tx_admit_thermal_protect(pAd);
	}
#endif
#endif /* CONFIG_AP_SUPPORT */
	...
	IN PNDIS_802_11_VARIABLE_IEs pVIE) 
{
...

	{
		...
		if ( !pEid )
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: oops: NULL pEid!!!\n", __FUNCTION__));
			return ;
		}
...
/* Add TP-Link Spcific Vendor IE Support */
#ifdef TP_IE_SUPPORT
                                       if (NdisEqualMemory(pEid->Octet, TP_OUI, 3))
                                       {
                                               NdisMoveMemory(pBss->TpIE.OUI, pEid->Octet, 3);
                                               pBss->TpIE.IELen = pEid->Len - 3;
                                               if(pBss->TpIE.IELen >= (TP_IE_MAX_LEN - 1))
                                                       pBss->TpIE.IELen = TP_IE_MAX_LEN - 1;

                                               NdisMoveMemory(pBss->TpIE.IE, pEid->Octet + 3, pBss->TpIE.IELen);
                                               pBss->TpIE.IE[pBss->TpIE.IELen]='\0';
												TPIE_vaild = 1;
						     /* mesh support */
						     if (CHECK_TPIE_SUPPORT_MESH(*((UCHAR *)(pEid->Octet + 5))) && CHECK_TPIE_MESH_ENABLE(*((UCHAR *)(pEid->Octet + 5))))
						     {
						     		pBss->TpIE.support_mesh = 1;
						     }	
						     else
						     {
						     		pBss->TpIE.support_mesh = 0;
						     }
#ifdef MWDS
						     /* wds support */
						     if ( *((UCHAR *)(pEid->Octet + 5)) & 0x4)
						     {
								pBss->TpIE.support_wds = 1;
						     }
						     else
						     {
								pBss->TpIE.support_wds = 0;
						     }
#endif /* MWDS */
						     /* level */
 						    pBss->TpIE.level = *((UCHAR *)(pEid->Octet + 6));
                                       }
#endif /* TP_IE_SUPPORT */
...
#ifdef TP_IE_SUPPORT
		if (0 == TPIE_vaild && pBss->TpIE.IELen > 0)
		{
			memset(&pBss->TpIE, 0, sizeof(TP_IE));
		}
#endif /* TP_IE_SUPPORT */
	...
	IN PNDIS_802_11_VARIABLE_IEs pVIE)
{
	...
	PAPCLI_STRUCT pApCliEntry = NULL;
		...
		BOOLEAN bInsert_2 = FALSE;
#endif
#if defined(TP_NETWORK_ROAMING) || defined(APCLI_SUPPORT)
	UCHAR i;
#endif
	...
	if (Idx == BSS_NOT_FOUND) 
	{
		if (Tab->BssNr >= MAX_LEN_OF_BSS_TABLE)
	    {
			...
			*/
			...
			{
				if (MAC_ADDR_EQUAL(pAd->ApCfg.MBSSID[i].wdev.bssid, ie_list->Bssid)
					|| SSID_EQUAL(pAd->ApCfg.MBSSID[i].Ssid,pAd->ApCfg.MBSSID[i].SsidLen, ie_list->Ssid, ie_list->SsidLen))
				{
					printk("BssTableSetEntry: local match\n");
					bInsert_2 = TRUE;
					break;
				}
			}
