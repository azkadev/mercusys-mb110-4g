	...
	IN RTMP_STRING *buffer)
{
	...
	{
		/* remove ";" split; we just configure only 1 apcli interface, avoid ssid with ";" setting error  */
		for (i=0, macptr = rstrtok(tmpbuf,""); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,""), i++)
		{
			pApCliEntry = &pAd->ApCfg.ApCliTab[i];
	...
	{
	    /* remove ";" split; we just configure only 1 apcli interface, avoid key with ";" setting error  */
		for (i = 0, macptr = rstrtok(tmpbuf,""); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,""), i++)
		{
			int retval = TRUE;
	...
	IN RTMP_STRING *pBuffer)
{
	...
	do
	{
        ...
        /* ApcliMacAddress, TP add */
		if (RTMPGetKeyParameter("ApcliMacAddress", tmpbuf, 25, pBuffer, TRUE))
		{
			retval = RT_CfgSetApcliMacAddress(pAd, tmpbuf);
			if (retval)
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MacAddress = %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	/*#endif*/
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			if (RTMPGetKeyParameter("AutoChannelSkipList", tmpbuf, 50, pBuffer, FALSE))
			{
				...
				if ( pAd->ApCfg.AutoChannelSkipListNum > 13 )
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Your no. of AutoChannelSkipList( %d ) is larger than 13 (boundary)\n",pAd->ApCfg.AutoChannelSkipListNum));
					pAd->ApCfg.AutoChannelSkipListNum = 13;
				}
				...
				IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
				{
					...
					if(RTMPGetKeyParameter("RekeyInterval", tmpbuf, 255, pBuffer, TRUE))
					{
						for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
						{
							...
							if(value_interval == 0 || ((value_interval >= 10) && (value_interval < MAX_REKEY_INTER)))
