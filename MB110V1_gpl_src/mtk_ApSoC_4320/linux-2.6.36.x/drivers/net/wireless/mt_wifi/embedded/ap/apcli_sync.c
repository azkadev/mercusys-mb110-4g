		...
		*/
		...
		if (matchFlag)
		{
			...
			if ((pAd->mesh_enable == TRUE) && (pAd->is_conn_specific_bssid == TRUE))
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("########: pAd->ScanTab.BssEntry[%ld] node id: %d, rootap node id: %d\n", Bssidx,
				                  GET_TPIE_NODE_IE(&pAd->ScanTab.BssEntry[Bssidx]), pAd->node_id));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("########: Node ID not match!!!\n"));
