...
static void mt76x2_switch_channel(RTMP_ADAPTER *ad, u8 channel, BOOLEAN scan)
{
	...
	/*
	 * If the macro WORKQUEUE_BH is applied, then when scanning a channel or restoring into
	 * the channel from scanning, there is some probability that Tx/Rx is disabled in later
	 * execution, which causes that the number of scanned APs is fewer and the beacon frame
	 * stops transmitting.
	 * The reason is NOT clear. Enable Tx/Rx here to avoid it.
	 */
#ifdef WORKQUEUE_BH
	RTMP_IO_WRITE32(ad, MAC_SYS_CTRL, 0xc);
#endif
					 ...
					 for (i = 0; i < SINGLE_SKU_TABLE_STBC_LENGTH; i++)
						{
							...
							if (i >=8 || ad->chipCap.rate_pwr_table.VHT1SS[i].mcs_pwr < ad->chipCap.rate_pwr_table.HT[i].mcs_pwr)				
