...
/* YF@20120419: Fix IOT Issue with Atheros STA on Windows 7 When IEEE80211H flag turn on. */
	...
	*/
...
#ifdef TP_IE_SUPPORT
	RTMP_SPIN_LOCK_IRQSAVE(&pAd->ApCfg.MBSSID[apidx].ap_vendor_ie.vendor_ie_lock, &flags);
	...
	RTMP_SPIN_UNLOCK_IRQRESTORE(&pAd->ApCfg.MBSSID[apidx].ap_vendor_ie.vendor_ie_lock, &flags);
	...
	MlmeFreeMemory(pAd, pOutBuffer);
		...
		if (pEntry)
		{
			if (pEntry->apidx == MAIN_MBSSID)
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s %d probe.rssi = %d\r\n", __FUNCTION__, __LINE__, probe.rssi));
				mtk_deliver_band_steering_event(RtmpOsGetNetIfIndex(wdev->if_dev), ATH_EVENT_BSTEERING_PROBE_REQ, sizeof(probe), (u_int8_t*)&probe);
			}
		}
#endif
	}
