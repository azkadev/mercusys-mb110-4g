...
static int _rtmp_BssEntry_cmp_ori(const void *a, const void *b)
{
    	...
    	case RT_SET_IAPP_PID:
			{
    			...
    			else
    			{
					...
					DBGPRINT(RT_DEBUG_TRACE, ("RT_SET_APD_PID::(IappPid=%lu(%ld))\n", IappPid, (ULONG)pObj->IappPid));
				}
    		}
			break;
#endif /* IAPP_SUPPORT */
    	...
    	case RT_SET_APD_PID:
			{
    			...
    			else
    			{
					...
					DBGPRINT(RT_DEBUG_TRACE, ("RT_SET_APD_PID::(ApdPid=%lu(%ld))\n", apd_pid, (ULONG)pObj->apd_pid));
				}
    		}
			break;
...
/*			pKey = kmalloc(wrq->u.data.length, GFP_KERNEL); */
	...
	case OID_802_11R_GET_STATION_INFO:
		{
			...
			{
				MAC_TABLE_ENTRY *pEntry = &pAd->MacTab.Content[i];		
				DBGPRINT(RT_DEBUG_TRACE, ("sta mac = %02x:%02x:%02x:%02x:%02x:%02x, pEntry->Addr = %02x:%02x:%02x:%02x:%02x:%02x\r\n", PRINT_MAC(sta.mac), PRINT_MAC(pEntry->Addr)));
				...
				if (IS_EQUAL_MAC(sta.mac, pEntry->Addr))		
				{
					...
					DBGPRINT(RT_DEBUG_TRACE, ("%s %d sta.sta.rssi = %d \r\n", __FUNCTION__, __LINE__, sta.sta.rssi));
					...
					DBGPRINT(RT_DEBUG_TRACE, ("%s %d: sta.sta.lastRxRate = %d, sta.sta.lastTxRate = %d, sta.sta.rssi = %d, sta.sta.idle = %d\n", __FUNCTION__, __LINE__, sta.sta.lastRxRate, sta.sta.lastTxRate, sta.sta.rssi, sta.sta.idle));
					found = 1;
					break;
				}
			}
	...
	case OID_802_11R_GET_PHYCAP_INFO:
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("%s %d: target mac = %02x:%02x:%02x:%02x:%02x:%02x, Bssid = %02x:%02x:%02x:%02x:%02x:%02x\n", __FUNCTION__, __LINE__, PRINT_MAC(staPhyCapInfo.mac), PRINT_MAC(pAd->CommonCfg.Bssid)));
			...
			for( i = 0; i < pAd->ApCfg.BssidNum; i++ )
			{
				if ( IS_EQUAL_MAC(staPhyCapInfo.mac, pAd->ApCfg.MBSSID[i].wdev.bssid))	
				{
					...
					DBGPRINT(RT_DEBUG_TRACE, ("MaxHTPhyMode BW:%d\nMCS:%d\nmaxTxPower:%d\nStreams:%d\n",staPhyCapInfo.phyCapInfo.maxChWidth,
	...
	case OID_802_11R_SCAN_EXAP_INFO:
		{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s %d ssidLength = %d\r\n", __FUNCTION__, __LINE__, ssidLength));
				if (ssidLength !=0)
				{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s %d scan target SSID %s, scanType = %d\r\n", __FUNCTION__, __LINE__, pTargetSsid->Ssid, scanType));
				waitCnt = 0;
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s %d scan finished, send IEEE80211_REASON_COMPLETED to user space.\r\n", __FUNCTION__, __LINE__));
			}
		break;
	case OID_802_11R_GET_EXAP_INFO:
		{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s %d pAd->ScanTab.BssNr = %d\r\n", __FUNCTION__, __LINE__, pAd->ScanTab.BssNr));
				...
				for (i = 0; i < pAd->ScanTab.BssNr; i++)
				{
					...
					DBGPRINT(RT_DEBUG_TRACE, ("%s %d: ScanTab.BssEntry[%d]: ssid = %s, Channel = %d, Rssi = %d\r\n", __FUNCTION__, __LINE__, i, pBss->Ssid, pBss->Channel, pBss->Rssi));

					if (pBss->Channel == 0)
					{
						DBGPRINT(RT_DEBUG_TRACE, ("%s %d pBss[%d] channel == 0, continue", __FUNCTION__, __LINE__, i));
						continue;
					}
					...
					if (j >= maxBssNum)
					{
						DBGPRINT(RT_DEBUG_TRACE, ("%s %d we just get %d entries.", __FUNCTION__, __LINE__, j));
						break;
					}
				}
	...
	case OID_802_11R_CLR_ASSOC_INFO:
		{			
			UCHAR Addr[MAC_ADDR_LEN];
			MAC_TABLE_ENTRY *pEntry = NULL;
			DBGPRINT(RT_DEBUG_TRACE, ("%s %d OID_802_11R_CLR_ASSOC_INFO.", __FUNCTION__, __LINE__));
			...
			if ( !IS_ENTRY_CLIENT(pEntry) )
				break;
			DBGPRINT(RT_DEBUG_TRACE, ("%s %d MacTableDeleteEntry.", __FUNCTION__, __LINE__));
...
#endif		
   	default:
			DBGPRINT(RT_DEBUG_ERROR, ("Set::unknown IOCTL's subcmd = 0x%08x\n", cmd));
			Status = -EOPNOTSUPP;
			break;
    }
	...
	case OID_802_11_AUTHENTICATION_MODE:
		...
		case OID_802_11_CHANNEL_WIDTH:
		{
		     ...
		     ulInfo = pAd->CommonCfg.BBPCurrentBW;	
	            Status = copy_to_user(wrq->u.data.pointer, &ulInfo, wrq->u.data.length);
	            DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_802_11_CHANNEL_WIDTH (%ld)\n",ulInfo));
	            break;
	        }
		break;
	        ...
	        case RT_OID_802_11_COUNTRY_REGION:
		     wrq->u.data.length = sizeof(ulInfo);
	            ...
	            DBGPRINT(RT_DEBUG_TRACE, ("Query::RT_OID_802_11_COUNTRY_REGION (%ld)\n",ulInfo));
	            break;
		...
		case OID_802_11_BEACON_PERIOD:
		     ...
		     if (copy_to_user(wrq->u.data.pointer, &ulInfo, wrq->u.data.length))
	            ...
	            DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_802_11_BEACON_PERIOD (%ld)\n",ulInfo));
		     break;
...
#define RSSI_BASE (-100)
	...
	/* sort by rssi */
	sort(pAd->ScanTab.BssEntry, pAd->ScanTab.BssNr, sizeof(BSS_ENTRY), _rtmp_BssEntry_cmp_ori, NULL);
