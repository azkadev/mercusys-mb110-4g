	...
	if(TX_BLK_TEST_FLAG(pTxBlk, fTX_bMWDSFrame))
	{
		...
		if(pTxBlk->pMacEntry)
		{
			...
			if(IS_MWDS_OPMODE_AP(pTxBlk->pMacEntry))
			{
				...
				COPY_MAC_ADDR(pHeader80211->Addr2, pTxBlk->pMacEntry->wdev->if_addr/*pAd->CurrentAddress*/); /* from AP1 */
			}
	...
	if(TX_BLK_TEST_FLAG(pTxBlk, fTX_bMWDSFrame))
	{
		...
		if(pTxBlk->pMacEntry)
		{
			...
			if(IS_MWDS_OPMODE_AP(pTxBlk->pMacEntry))
			{
				...
				COPY_MAC_ADDR(wifi_hdr->Addr2, pTxBlk->pMacEntry->wdev->if_addr/*pAd->CurrentAddress*/); /* from AP1 */
			}
	...
	*/
	...
	if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 1))
	{
		...
		if (pEntry && IS_MWDS_OPMODE_APCLI(pEntry))
		{
			...
			FromWhichBSSID = pEntry->wdev_idx + MIN_NET_DEVICE_FOR_APCLI;
			break;
		} else if (pEntry && IS_MWDS_OPMODE_AP(pEntry)) {
	...
	else
	{
		...
		if((pFmeCtrl->SubType != SUBTYPE_DATA_NULL) && (pFmeCtrl->SubType != SUBTYPE_QOS_NULL) && (!IS_BROADCAST_MAC_ADDR(pHeader->Addr3))) {
		    if(pEntry && IS_MWDS_OPMODE_AP(pEntry)){
    			 DBGPRINT(RT_DEBUG_ERROR,("Recv a unicast 3Addr pkt, but Entry is MWDS_OPMODE_AP.\n"));
    			 return;
    		 }
		}
#endif /* MWDS */
		...
		*/
			...
			(pAd->ApCfg.MBSSID[FromWhichBSSID].StaCount > 1)))
		{
			if (pAd->ApCfg.MBSSID[FromWhichBSSID].IsolateInterStaMBCast == 0 &&
				pAd->ApCfg.MBSSID[FromWhichBSSID].IsolateInterStaTraffic == 0)
			{
				bDirectForward  = TRUE;
			}
		}
