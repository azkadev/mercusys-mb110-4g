...
static int rtnetlink_fill_statusinfo(struct sk_buff *skb, int type, char *data, int data_len)
{
	struct ifinfomsg *r;
	struct nlmsghdr  *nlh;
...
void send_new_wlclientAddr_to_user(bool state, IN PUCHAR pAddr)
{
	unsigned int size = BUF_SIZE_RTNL;
	char buf[BUF_SIZE_RTNL];
	int len = 0;
	struct sk_buff *skb;
	struct net *net;
	...
	if(state == 0)
		memcpy(buf, "5G NEW CLI JOIN", sizeof("5G NEW CLI JOIN"));
	else
		memcpy(buf, "5G NEW CLI LEAVE", sizeof("5G NEW CLI LEAVE"));

	len = strlen(buf);
	memcpy(buf + len, pAddr, MAC_ADDR_LEN);
	...
	if (rtnetlink_fill_statusinfo(skb, RTM_NEWLINK,
				  buf, len) < 0)
	{
		printk("fill wps status info error!\n");
		kfree_skb(skb);
		return;
	}
	net = dev_net(skb->dev);
	...
	IN BOOLEAN CleanAll)
{
	...
	if (pAd->StaCfg.BssType == BSS_INFRA)
		FirstWcid = 2;
#endif /* CONFIG_STA_SUPPORT */
	send_new_wlclientAddr_to_user(0, pAddr);
	...
	/*				RTMP_OS_Init_Timer(pAd, &pEntry->RetryTimer, GET_TIMER_FUNCTION(WPARetryExec), pAd);*/
...
#ifdef TP_NETWORK_ROAMING
						{
							if ( APID_USED_FOR_ROAMING == apidx )
							{
								bs_current_sta_num_status num_status;
								memset(&num_status, 0, sizeof(num_status));
								num_status.max_sta_num = pAd->ApCfg.MBSSID[apidx].MaxStaNum;
								num_status.current_sta_num = pAd->ApCfg.MBSSID[apidx].StaCount;
								mtk_deliver_band_steering_event(RtmpOsGetNetIfIndex(wdev->if_dev), ATH_EVENT_BSTEERING_STA_NUM_REPORT, sizeof(num_status), (u_int8_t*)&num_status);
							}	
						}
#endif
						break;
			...
			pEntry->Aid = (USHORT)i;
			asic_set_drop_mask(pAd, pEntry->Aid, 0);
			...
			for (fifoExtTblIdx= 0; i < FIFO_EXT_HW_SIZE; fifoExtTblIdx ++)
			{
				if (pAd->FifoExtTbl[fifoExtTblIdx].wcid == pEntry->wcid)
				{
					pEntry->bUseHwFifoExt = TRUE;
					pEntry->hwFifoExtIdx = fifoExtTblIdx;
					break;
				}
			}
			break;
...
BOOLEAN MacTableDeleteEntry(RTMP_ADAPTER *pAd, USHORT wcid, UCHAR *pAddr)
{
	...
	asic_set_drop_mask(pAd, pEntry->Aid, 1);
	if (pEntry && !IS_ENTRY_NONE(pEntry))
	{
		send_new_wlclientAddr_to_user(1, pEntry->Addr);
		...
		if (MAC_ADDR_EQUAL(pEntry->Addr, pAddr))
		{
			...
			)
			{
				...
				{
					DBGPRINT(RT_DEBUG_OFF, ("MacTableDeleteEntry: Notify to Clear SFE rules \n"));
					sfe_destroy_all_rules_for_dev(NULL);
				}
#endif
				DBGPRINT(RT_DEBUG_ERROR, ("%s %d: call RtmpOSWrielessEventSendExt to send msg RT_WLAN_EVENT_EXPIRED\r\n", __FUNCTION__, __LINE__));
				RtmpOSWrielessEventSendExt(pAd->net_dev, RT_WLAN_EVENT_EXPIRED, -1, pEntry->Addr,
												NULL, 0,pEntry->apidx);
#endif
...
//   			NdisZeroMemory(pEntry, sizeof(MAC_TABLE_ENTRY));
			...
			{
				DBGPRINT(RT_DEBUG_ERROR, ("Impossible remove, a insert action must be happened during mac_table_reset\n"));
			}
