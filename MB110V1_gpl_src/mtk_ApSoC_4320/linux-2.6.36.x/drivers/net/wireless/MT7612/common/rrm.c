	...
	IN PMEASURE_REQ_ENTRY pDialogEntry)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s(): ie_list->Bssid  is %02X:%02X:%02X:%02X:%02X:%02X!\n", 
	__FUNCTION__,  ie_list->Bssid[0], ie_list->Bssid[1],ie_list->Bssid[2],
	ie_list->Bssid[3],ie_list->Bssid[4],ie_list->Bssid[5]));
	DBGPRINT(RT_DEBUG_TRACE, ("%s(): pBcnRep->Bssid  is %02X:%02X:%02X:%02X:%02X:%02X!\n", 
	...
	{
		DBGPRINT(RT_DEBUG_TRACE, ("%s():BcnReq->BSSID not equal ie_list->Bssid!\n", __FUNCTION__));
	...
	if (Idx != BSS_NOT_FOUND)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("%s:%d  BssTableSetEntry ok, idx:%lu\n", __FUNCTION__, __LINE__, Idx));
#if 0 /*for debug*/
{
	UCHAR i;
	...
	for (i = 0; i < Tab->BssNr; i++) 
	{
		...
		*/
		...
		{
			DBGPRINT(RT_DEBUG_TRACE, ("%s:%d  BssTableSetEntry failed\n", __FUNCTION__, __LINE__));
		}
	}
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	*/
	if (DialogToken != 0) {
		...
		DBGPRINT(RT_DEBUG_TRACE,("DialogToken auto report(%d)!\n", DialogToken));
		//return;
	}
...
#ifdef TP_NETWORK_ROAMING
		DBGPRINT(RT_DEBUG_TRACE, ("%s %d bcnrptIdx = %d\r\n", __FUNCTION__, __LINE__, bcnrptIdx));
		if (bcnrptIdx >= 0)
		{
			int i = 0;
			for (i = 0; i <= bcnrptIdx; i++)
			{
				DBGPRINT(RT_DEBUG_TRACE, ("%s %d : bcnrptIdx[%d], values : bcnrpt.bssid=%02x:%02x:%02x:%02x:%02x:%02x, bcnrpt.chnum = %d, bcnrpt.rcpi = %d, bcnrpt.rsni = %d\r\n",
			...
			DBGPRINT(RT_DEBUG_TRACE, ("%s %d wext_send_bcn_rsp_event to send ATH_EVENT_BSTEERING_RRM_REPORT, rrm.macaddr = %02x:%02x:%02x:%02x:%02x:%02x\r\n", __FUNCTION__, __LINE__, PRINT_MAC(rrm_rep_data.macaddr)));
