	...
	IN	PSTRING			arg)
{
	...
	if (pAd->chipCap.MCUType == ANDES) { 
		...
		if (pAd->chipCap.need_load_fw) {
			...
			printk("MT7612--> fw version:%d.%d.%02d ", (fw_ver & 0xf000) >> 8,
							(fw_ver & 0x0f00) >> 8, fw_ver & 0x00ff);
			printk("MT7612--> build:%x\n", build_ver);
			printk("MT7612--> build time:");
    ...
    IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
    ...
    for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++)
    {
        ...
        if ((pEntry->Sst == SST_ASSOC) && (IS_ENTRY_APCLI(pEntry) || IS_ENTRY_CLIENT(pEntry)))
        {
            ...
            pMacTab[entry_num].OneSecTxBytes = 0/*pEntry->OneSecTxBytesTotal*/;
            pMacTab[entry_num].OneSecRxBytes = 0/*pEntry->OneSecRxBytesTotal*/;
...
//+++Add by shiang for debug
			...
			if(IS_ENTRY_APCLI(pEntry))
			{
				...
				{
					if (pAd->ApCfg.ApCliTab[pEntry->apidx].MlmeAux.bSupportMWDS)
						printk("%-8s", "YES");
					else
						printk("%-8s", "NO");
				}
...

{
	...
	RtmpDrvRateGet(pAd, HTSetting.field.MODE,HTSetting.field.ShortGI,
		HTSetting.field.BW,HTSetting.field.MCS,newRateGetAntenna(HTSetting.field.MCS),(UINT32 *)fLastTxRxRate);
...
INT	Set_WlanResetB_Proc(PRTMP_ADAPTER	pAd, PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Set_WlanResetB_Proc::(reset=%ld)\n", reset));
