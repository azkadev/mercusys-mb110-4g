		...
		else
		{
...
#endif /* SOFT_SOUNDING */
			{
				...
				if ((pHeader_802_11->FC.SubType == SUBTYPE_PROBE_RSP) && (pHeader_802_11->FC.Type == FC_TYPE_MGMT))
				{
					...
					spe_apidx = get_apidx_by_addr(pAd, pHeader_802_11->Addr2);

					if ( !(spe_apidx >= pAd->ApCfg.BssidNum) && 
					     (pAd->ApCfg.MBSSID[spe_apidx].TxPwrAdj != -1) /* &&
					     (pAd->CommonCfg.MlmeTransmit.field.MODE == MODE_CCK) &&
					     (pAd->CommonCfg.MlmeTransmit.field.MCS == RATE_1)*/)
					{
						TxPwrAdj = pAd->ApCfg.MBSSID[spe_apidx].TxPwrAdj;
					}
#endif /* SPECIFIC_TX_POWER_SUPPORT */
#endif /* CONFIG_AP_SUPPORT */
				}
	...
	IN INT Max_Tx_Packets)
{
	...
	for (QueIdx = sQIdx; QueIdx >= eQIdx; QueIdx--)
	{
		...
		while (1)
		{
			...
			/* to enhance 5G throughput from mtk's patch. zhuhaiming 2017.9.28
