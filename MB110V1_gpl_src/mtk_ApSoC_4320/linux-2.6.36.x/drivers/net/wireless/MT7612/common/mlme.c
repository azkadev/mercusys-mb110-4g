...
/* Add TP-Link Spcific Vendor IE Support. liuliming */
#ifdef TP_IE_SUPPORT
...
{
	// åœ¨WSCå¤„äºŽæˆåŠŸçŠ¶å†µä¸‹é•¿äº®çš„300så†…ï¼Œå³ä½¿å‡ºçŽ°äº†ENDï¼Œä¹Ÿä¸è¦åšå¤„ç†.è¿™æ ·ä¿è¯æ— çº¿é‡å¯åŽè¿˜èƒ½ç»§ç»­äº®ç€
	/*
	...
	*/
	TP_WSCLEDStatus_5G = WSCLedStatus;
	//printk("%s : %d\n", __FUNCTION__, WSCLedStatus);
	switch (TP_WSCLEDStatus_5G)
	{
	case TP_WSC_LED_END:
...
VOID TP_CheckWSCLEDStatus(PWSC_CTRL pWscCtrl)
{
	...
	{
		TP_WSCLEDStatus_5G = TP_WSC_LED_END;
	}
	...
	if (TP_WSCLedStatus_old == TP_WSCLEDStatus_5G)
	{
	...
	{
		cnt = 0;
		TP_WSCLedStatus_old = TP_WSCLEDStatus_5G;
	}

	/* zeng weiji, Èç¹û2.4G WPSÒÑ³É¹¦Á¬ÉÏ£¬Ôò5G WPS×´Ì¬ÉèÎª½áÊø£¬·ñÔòLEDµÆ»áÅ¼¶ûÉÁË¸ */
	if (TP_WSCLEDStatus_24G == TP_WSC_LED_SUCCESS)
	{
		TP_WSCLEDStatus_5G = TP_WSC_LED_END;
	}
	...
	) 
{
		...
		if (ie_list->vht_op_len) {
				...
				(pBss->CentralChannel != ie_list->AddHtInfo.ControlChan))
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s():VHT cent_ch=%d, vht_op_info->center_freq_1=%d, Bss->CentCh=%d, change from CentralChannel to cent_ch!\n",
											__FUNCTION__, vht_cent_ch_freq(pAd, ie_list->AddHtInfo.ControlChan), vht_op->vht_op_info.center_freq_1, pBss->CentralChannel));
...

	{
		...
		if ( !pEid )
		{
			DBGPRINT(RT_DEBUG_ERROR, ("%s: oops: NULL Eid!!!\n", __FUNCTION__));
			return ;
...
#define WPS_AP		0x01
			switch(pEid->Eid)
			{
				case IE_WPA:
					...
					/* mesh support */
					if ((3 == *((UCHAR *)(pEid->Octet + 5)))  || (7 == *((UCHAR *)(pEid->Octet + 5))))
					{
	...
	IN ULONG Priv)
{
	...
	RTMP_SPIN_LOCK_IRQSAVE(&(Queue->Lock), &flags);
	Tail = Queue->Tail;
	/*
	...
	*/
	...
	{
		RTMP_SPIN_UNLOCK_IRQRESTORE(&(Queue->Lock), &flags);
		return FALSE;
	}
	Queue->Tail++;
	Queue->Num++;
...
{
	unsigned long flags;
	RTMP_SPIN_LOCK_IRQSAVE(&(pQueue->Lock), &flags);

	pQueue->Num  = 0;
	pQueue->Head = 0;
	pQueue->Tail = 0;
	RTMP_SPIN_UNLOCK_IRQRESTORE(&(pQueue->Lock), &flags);
