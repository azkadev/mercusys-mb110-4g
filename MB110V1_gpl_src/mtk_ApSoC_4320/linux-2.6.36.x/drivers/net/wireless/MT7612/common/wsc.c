...
extern UCHAR TotalPinAttackCount;
...
/* zeng weiji, 2018-04-13
 * send wps status info to user
 */
...
void send_success_status_to_user(void)
{
	unsigned int size = BUF_SIZE_RTNL;
	struct sk_buff *skb = NULL;
	struct net *net = NULL;
	char buf[BUF_SIZE_RTNL] = "SUCCESS 5G WPS";
	int len = strlen(buf);
	...
	switch (MsgType)
	{
		...
		case WSC_MSG_M2:
				...
				pWscControl->WscState == WSC_STATE_RX_M2D)
			{
					...
					{
#ifdef CONFIG_AP_SUPPORT
						if ((CurOpMode == AP_MODE) && (pWscControl->bSetupLock || sharedWscLock))
						{
		...
		case WSC_MSG_WSC_ACK:
			...
			if (pWscControl->WscState == WSC_STATE_WAIT_ACK)
			{
				...
				pWscControl->WscStatus = STATUS_WSC_CONFIGURED;
				/* send success info to user */
				send_success_status_to_user();
			}
			break;
#endif /* CONFIG_AP_SUPPORT */
...
Done:
	...
	if (rv == 1)
	{
		...
		if ((pWscControl->WscConfStatus == WSC_SCSTATE_UNCONFIGURED)
			...
			|| ((CurOpMode == AP_MODE) && !pWscControl->bSetupLock && !sharedWscLock)
#endif /* CONFIG_AP_SUPPORT */
			)
		{
			pWscControl->WscStatus = STATUS_WSC_CONFIGURED;
...
#ifdef CONFIG_AP_SUPPORT
	if ((CurOpMode == AP_MODE) && (pWpsCtrl->bSetupLock || sharedWscLock))
	{
		// AP Setup Lock
		/* zeng weiji, we use 1 stand for locked for minute, 2 stand for locked forever,
		 * but WPS protocol require setupLock to be boolean type in wireless frame,
		 * so we always set setupLock be 1 here.
		 */
		WscLock = 1;
		templen = AppendWSCTLV(WSC_ID_AP_SETUP_LOCKED, pData, (UINT8 *)&WscLock, 0);
		pData += templen;
		Len   += templen;
	}
#endif /* CONFIG_AP_SUPPORT */
...
{	
	...
	*/
	DBGPRINT(RT_DEBUG_TRACE, ("----->WscThreadExit\n"));
	...
	IN  PWSC_CTRL		pWscControl)
{
	...
	*/
	...
	/* zengweiji, modify, 2018/02/02 */
	/* if already locked forever, then return directly */
	if (WPS_LOCK_FOREVER == pWscControl->bSetupLock)
		return;

	TotalPinAttackCount++;
	DBGPRINT(RT_DEBUG_TRACE, ("[5G]WPS attackCount = %d.\n", TotalPinAttackCount));

	/* pin attack */
	if (TotalPinAttackCount >= WSC_WPS_AP_MAX_PIN_ATTACK)
	{
		/* locked forever */
		if (TotalPinAttackCount >= WSC_LOCK_FOREVER_PIN_ATTACK)
		{
			pWscControl->bSetupLock = WPS_LOCK_FOREVER;
			if(pWscControl->WscSetupLockTimerRunning)
			{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("5G WPS locked forever!\n"));
		}
		/* locked for minutes */
		else
		{
			pWscControl->bSetupLock = TRUE;
			/* if a timer is running, cancel and run a new timer */
			if(pWscControl->WscSetupLockTimerRunning)
			{
				RTMPCancelTimer(&pWscControl->WscSetupLockTimer, &bCancelled);
				pWscControl->WscSetupLockTimerRunning = FALSE;
			...
			DBGPRINT(RT_DEBUG_TRACE, ("5G WPS locked for %d minutes!\n", pWscControl->SetupLockTime));
		}
		sharedWscLock = pWscControl->bSetupLock;
