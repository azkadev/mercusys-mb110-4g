	...
	IN UCHAR wcid)
{
    ...
    DBGPRINT(RT_DEBUG_ERROR, ("%s: pAd: %p, ifIndex: %d, wcid: %d\n",__FUNCTION__, pAd, ifIndex, wcid));
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s: pMbss: %p, pNewConnEntry: %p\n",__FUNCTION__, pMbss, pNewConnEntry));
	if(pNewConnEntry)
	{
	    DBGPRINT(RT_DEBUG_ERROR, ("%s: MWDSConnEntryLock: %p, MWDSConnEntryLock: %p, List: %p\n",
	...
	IN MAC_TABLE_ENTRY *pEntry)
{
	...
	if(pEntry->bEnableMWDS)
		DBGPRINT(RT_DEBUG_WARN,("MWDSAPPeerDisable: Disable MWDS for entry : %02x-%02x-%02x-%02x-%02x-%02x \n", PRINT_MAC(pEntry->Addr)));
