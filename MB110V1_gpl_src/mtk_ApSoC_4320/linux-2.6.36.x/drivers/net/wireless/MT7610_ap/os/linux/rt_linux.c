...
int raspi_flash_read(u_char *buf, u32 from, u32 len)
{
	...
	if (!((from >= RF_OFFSET) && (from + len < RF_OFFSET + MAX_RF_DATA_SIZE)))
	{
...
exit:
	if (partition_buf)
		os_free_mem(NULL, partition_buf);	
	
	return ret;

}

int raspi_flash_write(const u_char *buf, u32 to, u32 len)
{
	int ret = 0;
	int index = 0;
	u_char *partition_buf = NULL;
	/* Write all eeprom data in memory into flash. */
	if (!((to >= RF_OFFSET) && (to + len < RF_OFFSET + MAX_RF_DATA_SIZE)))
	{
	...
	ULONG b)
{
	...
	printk("read radio success, offset 0x%lx, size 0x%lx\n", a, b);
	...
	ULONG b)
{
	...
	printk("write radio success, offset 0x%lx, size 0x%lx\n", a, b);
#if 0
	...
	IN INT devIdx)
{
	...
	for (ifNameIdx = devIdx; ifNameIdx < 32; ifNameIdx++) {
		...
		ASSERT(((strlen(suffixName) + prefixLen) < IFNAMSIZ));
