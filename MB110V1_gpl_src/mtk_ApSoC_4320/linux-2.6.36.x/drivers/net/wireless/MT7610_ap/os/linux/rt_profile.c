	...
	IN UCHAR OpMode)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
#if defined(MAT_SUPPORT) && defined(APCLI_SUPPORT)
		...
		if( RTMP_GET_PACKET_SPECIFIC_RX_ARP(pPacket)
			&& RTMP_GET_PACKET_SPECIFIC_RX_ARP_BCAST(pPacket)
			&& RTMP_GET_PACKET_SPECIFIC_RX_ARP_REQ(pPacket) )
		{
			PUCHAR	    sendMac = NULL;
			PUCHAR 		pPktHdr = NULL;
			UINT16		protoType;

			//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("==> %s %d arp packet hanlder trace \n", __FUNCTION__, __LINE__));
			...
			if ( pPktHdr )
			{
				...
				{
					sendMac = (pPktHdr + MAT_ETHER_HDR_LEN) + 8;
					//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("==> %s %d arp packet hanlder trace, sendMac = %02x-%02x-%02x-%02x-%02x-%02x \n", __FUNCTION__, __LINE__,PRINT_MAC(sendMac)));
				}
				if (sendMac)
