...
int rt28xx_packet_xmit(void *skbsrc)
{
    ...
    if ((RTMP_GET_PACKET_NET_DEVICE(pPacket) >= MIN_NET_DEVICE_FOR_APCLI) && IS_BROADCAST_MAC_ADDR(skb->data))
    {
		DBGPRINT(RT_DEBUG_INFO, ("%s: copy broadcast packet to apcli %d %02x:%02x:%02x:%02x:%02x:%02x!!!\n", 
			__FUNCTION__, RTMP_GET_PACKET_NET_DEVICE(pPacket), PRINT_MAC(skb->data)));
        pPacket = (PNDIS_PACKET)OS_PKT_COPY(pPacket);
