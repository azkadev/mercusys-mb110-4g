...
/*
	...
	========================================================================
*/
extern INT32 mt76x0_sku_calculate_TxPwrAdj(struct _RTMP_ADAPTER *ad,struct _TXWI_NMAC *txwi_n);
...
#ifdef SINGLE_SKU_V2	
	txwi_n->TxPwrAdj= mt76x0_sku_calculate_TxPwrAdj(pAd,txwi_n); 
#endif /* SINGLE_SKU_V2 */
...
*/
	...
	if (pAd->fpga_on & 0x6)
	{
		...
		if (pAd->data_basize)
			pTxWI->TxWIBAWinSize = pAd->data_basize;
        tmpTransmit.field.BW = pAd->data_bw; 
		tmpTransmit.field.ShortGI = pAd->data_gi;
		tmpTransmit.field.STBC = pAd->data_stbc;
		//tmpTransmit.field.ldpc = ldpc;
		tmpTransmit.field.MCS = pAd->data_mcs;
		tmpTransmit.field.MODE = pAd->data_phy;
