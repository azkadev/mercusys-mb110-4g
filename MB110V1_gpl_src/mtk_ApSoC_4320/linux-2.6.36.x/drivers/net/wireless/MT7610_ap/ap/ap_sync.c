	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (! PeerProbeReqSanity(pAd, Elem->Msg, Elem->MsgLen, Addr2, Ssid, &SsidLen, &bRequestRssi, &vendor))
		return;
	...
	for(apidx=0; apidx<pAd->ApCfg.BssidNum; apidx++)
	{
        ...
        pAd->ApCfg.MBSSID[apidx].wdev.PhyMode = PhyMode;
	...
	BND_STRG_CHECK_CONNECTION_REQ(	pAd,
...
#ifdef TP_IE_SUPPORT
        if ((vendor.element_id == IE_VENDOR_SPECIFIC) && (vendor.len > 0))
        {
            struct probe_req_report pProbeReqReportTemp;
            memset(&pProbeReqReportTemp, 0, sizeof(struct probe_req_report));
            COPY_MAC_ADDR(pProbeReqReportTemp.sta_mac, Addr2);
            pProbeReqReportTemp.vendor_ie.element_id = vendor.element_id;
            pProbeReqReportTemp.vendor_ie.len = vendor.len;
            NdisMoveMemory(pProbeReqReportTemp.vendor_ie.custom_ie, vendor.custom_ie, vendor.len);
            RtmpOSWrielessEventSend(pAd->ApCfg.MBSSID[apidx].MSSIDDev, RT_WLAN_EVENT_CUSTOM, RT_PROBE_REQ_REPORT_EVENT,
                NULL, (PUCHAR)&pProbeReqReportTemp, MAC_ADDR_LEN + 2 + vendor.len);
        }
#endif /* TP_IE_SUPPORT */
        ...
        FrameLen += build_vendor_ie(pAd, &(pAd->ApCfg.MBSSID[apidx].wdev), (pOutBuffer + FrameLen));
#endif /* DOT11N_DRAFT3 */
#endif /* DOT11_N_SUPPORT */
		 ...
		 */
{
	...
	FrameLen += TmpLen;
		...
		if (pEntry)
		{
			...
			{
				probe.rssi = (RTMPAvgRssi(pAd, &pEntry->RssiSample) + 90);
				memcpy(probe.sender_addr, Addr2, MAC_ADDR_LEN);
				//printk("%s %d call func mtk_deliver_band_steering_event to send ATH_EVENT_BSTEERING_PROBE_REQ\r\n", __FUNCTION__, __LINE__);
				DBGPRINT(RT_DEBUG_ERROR, ("%s %d probe.rssi = %d\r\n", __FUNCTION__, __LINE__, probe.rssi));
				mtk_deliver_band_steering_event(RtmpOsGetNetIfIndex(pAd->ApCfg.MBSSID[apidx].MSSIDDev), ATH_EVENT_BSTEERING_PROBE_REQ, sizeof(probe), (u_int8_t*)&probe);
			}
		}
#endif
	}
...
#endif /* APCLI_SUPPORT */
#ifdef APCLI_SUPPORT
        if(NdisEqualMemory(pAd->MacTab.Content[MAX_NUMBER_OF_MAC].Addr, ie_list->Bssid, MAC_ADDR_LEN))
		{
            if ( pAd->ApCfg.ApCliTab[0].CtrlCurrState == APCLI_CTRL_CONNECTED )
            {   
			    MAC_TABLE_ENTRY *pEntry = &pAd->MacTab.Content[MAX_NUMBER_OF_MAC];/*apcli0 */
                RXWI_STRUC	RxWI;
                ...
                RxWI.RXWI_O.phy_mode = 0;
#endif

                /*DBGPRINT(RT_DEBUG_ERROR, ("\n=========Update_Rssi_Sample: \n%02X:%02X:%02X:%02X:%02X:%02X, channel: %d\n=============\n",
                            pEntry->ReptCliAddr[0], pEntry->ReptCliAddr[1], pEntry->ReptCliAddr[2],
					pEntry->ReptCliAddr[3], pEntry->ReptCliAddr[4], pEntry->ReptCliAddr[5],  pAd->CommonCfg.Channel));*/
