	...
	OUT UCHAR		*pWcid)
{
	...
	if (MAC_ADDR_IS_GROUP(pSrcBufVA))
	{
		*pWcid = (UCHAR)Aid;
		...
		MWDSProxyEntryDelete(pAd, apidx, (pSrcBufVA + MAC_ADDR_LEN));
#endif
		return TRUE;
	}
	...
	else
	{
		...
		if (pEntry == NULL) {
			...
			if (MWDSProxyLookup(pAd, apidx, pSrcBufVA, FALSE, &Wcid)) {
				if (VALID_WCID(Wcid))
...
#ifdef MWDS
            apCliIdx = RTMP_GET_PACKET_NET_DEVICE(pPacket) - MIN_NET_DEVICE_FOR_WDS;
#endif
		}
		else
#endif /* WDS_SUPPORT */
        {
			allowToSend = ApAllowToSendPacket(pAd, pPacket, &wcid);//rai0
#ifdef MWDS
			apCliIdx = RTMP_GET_PACKET_NET_DEVICE(pPacket);
#endif
        }
...
//2019.8.13 mjzh  fix bug 302534 & 302576
#if 0
            pSrcBufVA = GET_OS_PKT_DATAPTR(pPacket);
            if (pSrcBufVA &&
                    IS_BROADCAST_MAC_ADDR(pSrcBufVA)) {
                MWDSSendClonePacket(pAd, apCliIdx, pPacket, NULL);
            } else if (pSrcBufVA &&
	...
	if(TX_BLK_TEST_FLAG(pTxBlk, fTX_bMWDSFrame))
	{
		...
		if(pTxBlk->pMacEntry)
		{
			...
			if(IS_MWDS_OPMODE_APCLI(pTxBlk->pMacEntry))
			{
				...
				COPY_MAC_ADDR(pHeader80211->Addr2, pTxBlk->pApCliEntry->CurrentAddress);
            }
			else
#endif /* APCLI_SUPPORT */
			if(IS_MWDS_OPMODE_AP(pTxBlk->pMacEntry))
			{
                ...
                COPY_MAC_ADDR(pHeader80211->Addr2, pTxBlk->pMacEntry->HdrAddr2/*pAd->CurrentAddress*/); /* from AP1 */
			}
    ...
    if(TX_BLK_TEST_FLAG(pTxBlk, fTX_bMWDSFrame))
    {
        ...
        if(pTxBlk->pMacEntry)
        {
            ...
            if(IS_MWDS_OPMODE_APCLI(pTxBlk->pMacEntry))
            {
                ...
                COPY_MAC_ADDR(wifi_hdr->Addr2, pTxBlk->pApCliEntry->CurrentAddress);
            }
            else
#endif /* APCLI_SUPPORT */
            if(IS_MWDS_OPMODE_AP(pTxBlk->pMacEntry))
            {
                ...
                COPY_MAC_ADDR(wifi_hdr->Addr2, pTxBlk->pMacEntry->HdrAddr2/*pAd->CurrentAddress*/); /* from AP1 */
            }
            //hex_dump("mwds-pkt", pTxBlk->pSrcBufHeader 12);
...
REPEATER_CLIENT_ENTRY *pReptEntry = NULL;
	...
	HAL_LastTxIdx(pAd, pTxBlk->QueIdx, /*LastTxIdx*/0);
...
VOID dynamic_tune_be_tx_op(RTMP_ADAPTER *pAd, ULONG nonBEpackets)
{
	...
	)
	{
		...
		else
		{
			...
			{
				/* enable AC0(BE) TX_OP */
				UCHAR	txop_value_burst = 0x60;	/* default txop for Tx-Burst */
				UCHAR   txop_value;
...
static int APCheckRxMgmtFrame(IN PRTMP_ADAPTER pAd, IN RX_BLK *pRxBlk) {
	PHEADER_802_11 pHeader = pRxBlk->pHeader;
	MAC_TABLE_ENTRY *pEntry = NULL;
	UCHAR *ra = pHeader->Addr1;
	UCHAR *ta = pHeader->Addr2;
	UINT32 apidx;

	if (pHeader->FC.SubType == SUBTYPE_DEAUTH) {
		pEntry = MacTableLookup(pAd, ta);
		if (!pEntry) {
			printk("ERROR: cannot find %02x:%02x:%02x:%02x:%02x:%02x in mactable\n", PRINT_MAC(ta));
			return FALSE;
		}

		for (apidx=0; apidx<pAd->ApCfg.BssidNum; apidx++)
		{
			if (RTMPEqualMemory(ra, pAd->ApCfg.MBSSID[apidx].Bssid, MAC_ADDR_LEN))
				break;
		}
		if (apidx >= pAd->ApCfg.BssidNum) {
			printk("ERROR: cannot find %02x:%02x:%02x:%02x:%02x:%02x in MBSSID\n", PRINT_MAC(ra));
			return FALSE;
		}

		if (pAd->ApCfg.MBSSID[apidx].wdev.func_idx != pEntry->func_tb_idx) {
			printk("ERROR: func_idx does not match: target=%d current=%d\n",
			       pEntry->func_tb_idx, pAd->ApCfg.MBSSID[apidx].wdev.func_idx);
			return FALSE;
		}
	}
	...
	IN RX_BLK *pRxBlk)
{
	...
	do
	{
		if (!APCheckRxMgmtFrame(pAd, pRxBlk)) {
			break;
		}
	...
	IN UCHAR FromWhichBSSID)
{
	...
	if (!RTMPCheckWPAframe(pAd, pEntry, pRxBlk->pData, pRxBlk->DataSize, FromWhichBSSID))
	{
		...
		if (pEntry && IS_ENTRY_MWDS(pEntry))
		{
			if ( (!((FC->FrDs == 1) && (FC->ToDs == 1)))
			     && !(pRxBlk->pRxInfo->Mcast || pRxBlk->pRxInfo->Bcast)
   			     && !(RTMP_GET_PACKET_SPECIFIC_RX_ARP_BCAST(pRxBlk->pRxPacket) && RTMP_GET_PACKET_SPECIFIC_RX_ARP_REQ(pRxBlk->pRxPacket)))
			{
				/* release packet */
				RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
				return;
		...
		if (pEntry->PrivacyFilter == Ndis802_11PrivFilter8021xWEP)
		{
			...
			DBGPRINT(RT_DEBUG_OFF, ("==> drop all non-EAP DATA frame before peers's Port-Access-Control is secured\n"));
			/* release packet */
			RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
			return;
		}
		...
		if (IS_ENTRY_CLIENT(pEntry) &&
			...
			DBGPRINT(RT_DEBUG_ERROR, ("YF ==> De-Auth this STA(%02x:%02x:%02x:%02x:%02x:%02x) due to OPEN packet\n",
				PRINT_MAC(pEntry->Addr)));
			RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
...
void get_pn_from_rxblk(IN PRTMP_ADAPTER pAd, IN RX_BLK *pRxBLK)
{
	...
	if ((!pRxInfo) || (!pRxInfo->pn_len))
		return;

	pn_len_byte = (pRxInfo->pn_len) << 2;
	if (pRxBLK->DataSize <= pn_len_byte) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s :DataSize: %u <= pn_len: %u\n", __FUNCTION__,pRxBLK->DataSize, pn_len_byte));
		return;
	}
	...
	if (unlikely(pn_len_byte != 8)) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s abnormal pn_len %u\n", __FUNCTION__, pRxInfo->pn_len));
		return;
	}
	pnvh = (((UINT64)pnb[7] << 24) | ((UINT64)pnb[6] << 16) | ((UINT64)pnb[5] << 8)| (UINT64)pnb[4]);
	pnvl = (((UINT64)pnb[1] << 8) | (UINT64)pnb[0]);
	pnv = ((pnvh << 16) | pnvl);
	pRxBLK->CCMP_PN= pnv;
}
	...
	*/
	...
	if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 1))
	{
		do
		{
			...
			pEntry = MacTableLookup(pAd, pHeader->Addr2);
#ifdef APCLI_SUPPORT
			if (pEntry && IS_MWDS_OPMODE_APCLI(pEntry))
			{
				if (IS_ENTRY_APCLI(pEntry))
				{
					PAPCLI_STRUCT pApCliEntry = NULL;
					...
					if(pApCliEntry)
					{
						...
						if(MAC_ADDR_EQUAL(pHeader->Octet, pApCliEntry->CurrentAddress))
						{
						   DBGPRINT(RT_DEBUG_OFF,("ApCli receive a looping packet!\n"));
						   return;
						}
					}
				}
			...
			if (pEntry && IS_MWDS_OPMODE_AP(pEntry)) {
				...
				if (NdisEqualMemory(SNAP_802_1H,pRxBlk->pData+ARPSenderIPSkipLen, 6)  ||
	                NdisEqualMemory(SNAP_BRIDGE_TUNNEL, pRxBlk->pData, 6)) {
				...
				MWDSProxyTabUpdate(pAd, pEntry->apidx, pEntry->wcid, pHeader->Octet, ARPSenderIP);
				RX_BLK_SET_FLAG(pRxBlk, fRX_WDS);
				/* hdr_len = LENGTH_802_11_WITH_ADDR4; */
				FromWhichBSSID = pEntry->apidx;
				bWdsPacket = TRUE;
			...
			else if (pEntry && IS_ENTRY_CLIENT(pEntry) && !IS_ENTRY_APCLI(pEntry) )
			{
				DBGPRINT(RT_DEBUG_OFF,("====================WDS entry triggered:%02X:%02X:%02X:%02X:%02X:%02X\n", PRINT_MAC(pEntry->Addr)));
		    ...
		    else
		    {
				if (pEntry)
				{
					DBGPRINT(RT_DEBUG_ERROR, ("\n!!!!!!!Recv a wds pkt, but Entry:%02X:%02X:%02X:%02X:%02X:%02X is non-wds.\n", PRINT_MAC(pEntry->Addr)));
				}
		        pEntry = NULL;
			}
#endif /* MWDS */
...
#ifdef MWDS
		pEntry = MacTableLookup(pAd, pHeader->Addr2);
		if((pFmeCtrl->SubType != SUBTYPE_DATA_NULL) && (pFmeCtrl->SubType != SUBTYPE_QOS_NULL)  && (!IS_BROADCAST_MAC_ADDR(pHeader->Addr3)) ) {
			if(pEntry && IS_MWDS_OPMODE_AP(pEntry)){
				DBGPRINT(RT_DEBUG_ERROR,
				   ("Recv a unicast 3Addr pkt, but Entry is MWDS_OPMODE_AP.\n"));
				return;
			}
			else{
			}
...
// TODO: shiang@PF#2, is this atheros protection still necessary here????
	...
	*/
...
#ifdef MWDS
    {
        ...
        if (pData[6] == 0x81 &&pData[7]==0x00) /*vlan tag*/
        {
            pPktHdr = pData + 8 + 4;// get ip header start addr, need add vlan tag(4 bytes)
            if (pData[10] == 0x08 && pData[11] == 0x06)
            {
                is_arp = true; /*mark arp data*/
            }
        }
        else
        {
            pPktHdr = pData+8;// get ip header start addr
            if (pData[6] == 0x08 && pData[7] == 0x06)
            {
                is_arp = true;
            }
        }

        if (is_arp)//mark packet is ARP
        {
            /* TP-link feature: bmcast arp packet use 3addr formate in mwds mode */
            USHORT arptype;
            //pRxBlk->Arp = 1;
            /* Recived Specific ARP Packet */
            RTMP_SET_PACKET_SPECIFIC_EXT(pRxPacket,0);
            RTMP_SET_PACKET_SPECIFIC_RX_ARP(pRxPacket,1);
            /* Record Arp requst & response */
            arptype = (*(pPktHdr + 6) << 8) | *(pPktHdr + 7);
            if( arptype == 1 )
                RTMP_SET_PACKET_SPECIFIC_RX_ARP_REQ(pRxPacket,1);
            else
                RTMP_SET_PACKET_SPECIFIC_RX_ARP_RES(pRxPacket,1);
            /* when mwds mode, we recived 3addr arp bmcast apcket ,
            if pHeader.Addr1 = ff:ff:ff:ff:ff:ff mean apcli recived ap's arp bmcast apcket,
            if pHeader.Addr3 = ff:ff:ff:ff:ff:ff mean ap recived apcli's arp bmcast apcket */
            if( /*isBcastEtherAddr(pHeader->Addr1) ||*/ isBcastEtherAddr(pHeader->Addr3) )
                RTMP_SET_PACKET_SPECIFIC_RX_ARP_BCAST(pRxPacket,1);
        }
    }
#endif
...
#ifdef MAC_REPEATER_SUPPORT
            if (pHeader->FC.FrDs == 1 && pHeader->FC.ToDs == 0 && pRxInfo->Bcast && RTMPLookupRepeaterCliEntry(pAd, FALSE, pHeader->Addr3))
            {
                DBGPRINT(RT_DEBUG_INFO, ("%s(): drop apclii0 loopback packet %02x:%02x:%02x:%02x:%02x:%02x !\n", 
					__FUNCTION__, PRINT_MAC(pHeader->Addr3)));
                RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_SUCCESS);
                continue;
            }
#endif
	...
	IN	ULONG			FromWhichBSSID)
{
	...
	else
	{
		...
		if (pEntry && pEntry->Sst == SST_ASSOC && IS_ENTRY_CLIENT(pEntry))
		{
			...
			pSrcEntry = MacTableLookup(pAd, pHeader802_3 + MAC_ADDR_LEN);
			if (pSrcEntry && (pSrcEntry->PortSecured == WPA_802_1X_PORT_NOT_SECURED)) {
				DBGPRINT(RT_DEBUG_ERROR, ("%s():Not PortSecured Pkt FWD to Sta from Wcid(%d) to Aid(%d)!\n",
				__FUNCTION__, pSrcEntry->wcid, pEntry->wcid));
				bDirectForward = FALSE;
				bAnnounce = FALSE;
			}
		}
#ifdef MWDS
        else if(!pEntry && MWDSProxyLookup(pAd, FromWhichBSSID, pHeader802_3, FALSE, &wcid))
        {
            if (VALID_WCID(wcid))
                pEntry = &pAd->MacTab.Content[wcid];
...
//2019.8.13 mjzh  fix bug 302534 & 302576
			/* Ret = */ APSendPacket(pAd, pForwardPacket);
#ifdef MWDS
#if 0
            if ((pHeader802_3[0] & 0x01) && (Ret == NDIS_STATUS_SUCCESS))
            {
                pSrcAddr = pHeader802_3 + MAC_ADDR_LEN;
                if(ISMWDSValid(pAd, FromWhichBSSID))
                     MWDSSendClonePacket(pAd, FromWhichBSSID, pPacket, pSrcAddr);
            }
#endif
#endif /* MWDS */
