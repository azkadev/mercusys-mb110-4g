 ...
 */
#ifdef TP_IE_SUPPORT
#define GET_TPINFO_NODE_IE(x)   (*(UINT16 *)((x) + 26))
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
								...
								FALSE))
	{
		...
		*/
		...
		{
#ifdef TP_IE_SUPPORT
            /* When onemesh is enable and conn_specific_bssid is enable, if node id not match, end */
            if ((pAd->mesh_enable == TRUE) && (pAd->is_conn_specific_bssid == TRUE))
            {
                if (pAd->node_id != GET_TPIE_NODE_IE(&pAd->ScanTab.BssEntry[Bssidx]))
                {
                    DBGPRINT(RT_DEBUG_ERROR, ("########: pAd->ScanTab.BssEntry[%ld] node id: %d, rootap node id: %d\n", Bssidx,
                        GET_TPIE_NODE_IE(&pAd->ScanTab.BssEntry[Bssidx]), pAd->node_id));
                    DBGPRINT(RT_DEBUG_ERROR, ("########: Node ID not match!!!\n"));
                    pApCliEntry->ProbeReqCnt = 6;
            ...
            if(ie_list->vendor_ie.mtk_cap_found)
            {
                ...
                {
                    pApCliEntry->ApCliMlmeAux.bSupportMWDS = TRUE;
                    DBGPRINT(RT_DEBUG_OFF, ("AP supports MWDS\n"));
                }
                else
                {
                    pApCliEntry->ApCliMlmeAux.bSupportMWDS = FALSE;
                    DBGPRINT(RT_DEBUG_OFF, ("AP non-supports MWDS\n"));
                }
            }
#endif /* MWDS */
	...
	IN USHORT ifIndex)
{
	...
	{
		DBGPRINT(RT_DEBUG_ERROR, ("EnqueueProbeRequest() allocate memory fail\n"));
		return;
	}
	else
	{
