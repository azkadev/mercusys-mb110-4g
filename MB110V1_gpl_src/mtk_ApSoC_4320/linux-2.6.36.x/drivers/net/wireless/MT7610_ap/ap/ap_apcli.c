	...
	do
	{
			...
			)
		{
			...
			if ((pAd->ApCfg.bMACRepeaterEn)
				...
				&& ((pAd->ApCfg.ApCliTab[ApCliIdx].wdev.bSupportMWDS == FALSE) ||
					(pAd->ApCfg.ApCliTab[ApCliIdx].ApCliMlmeAux.bSupportMWDS == FALSE))
#endif /* MWDS */
				)
			{
				if (ifIndex < MAX_APCLI_NUM)
		...
		if (pMacEntry)
		{
            ...
            if ((CliIdx != 0xFF)
                ...
                (pApCliEntry->ApCliMlmeAux.bSupportMWDS == FALSE))
#endif /* MWDS */
                )
			{
	...
	OUT UCHAR		*pWcid)
{
    ...
    apcli_entry = &pAd->ApCfg.ApCliTab[apCliIdx];
	if (ValidApCliEntry(pAd, apCliIdx))
	{
		...
		*pWcid = pAd->ApCfg.ApCliTab[apCliIdx].MacTabWCID;
        ...
        if((apcli_entry->bEnableMWDS == FALSE) &&
            (apcli_entry->ApCliMlmeAux.bSupportMWDS) &&
            (apcli_entry->wdev.bSupportMWDS)){
	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	*/
	...
	if(IS_MWDS_OPMODE_APCLI(pEntry) &&
		  MAC_ADDR_EQUAL(pHeader->Octet, pApCliEntry->CurrentAddress)) //pApCliEntry->wdev.if_addr <==> pApCliEntry->CurrentAddress
		return FALSE;
#endif /* MWDS */
	...
	if ((pRxInfo->MyBss == 0) && (pRxInfo->Decrypted == 0) && (pHeader->FC.Wep == 1)) 
	{
		...
		UINT8 *pnb;
		if (pApCliEntry->SharedKey[pRxWI->RxWIKeyIndex].CipherAlg == CIPHER_AES) {
			pnb = &pRxBlk->pData[0];
...
/*	RTMP_OS_NETDEV_OP_HOOK	netDevOpHook; */
	...
	for(apcli_index = 0; apcli_index < MAX_APCLI_NUM; apcli_index++)
	{
        ...
        {
            DBGPRINT(RT_DEBUG_TRACE, ("ApcliCustomAddress: %02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(pAd->ApcliCustomAddress)));
            COPY_MAC_ADDR(pNetDevOps->devAddr, pAd->ApcliCustomAddress);
        }
		else
		{
	...
	IN  PRTMP_ADAPTER   pAd)
{
	...
	if (pScanTab->BssNr < 10)
	{
		DBGPRINT(RT_DEBUG_WARN, ("---> ApCliAutoConnectExec() ScanTab BssNr %d\n", pScanTab->BssNr));
	}
	...
	*/
	for (entryIdx = 0; entryIdx < pScanTab->BssNr; entryIdx++)
	{
		...
		if (pBssEntry->SsidLen &&
			(pBssEntry->SsidLen == CfgSsidLen) &&
			NdisEqualMemory(pCfgSsid, pBssEntry->Ssid, CfgSsidLen) &&
	...
	*/
	...
	{
		DBGPRINT(RT_DEBUG_WARN, ("No match entry.\n"));
		pAd->ApCfg.ApCliAutoConnectRunning = FALSE;
	}
