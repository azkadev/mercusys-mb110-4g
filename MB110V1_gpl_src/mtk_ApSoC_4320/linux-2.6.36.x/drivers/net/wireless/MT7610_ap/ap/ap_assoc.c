	...
	IN BOOLEAN isReassoc)
{
	...
	if (pAd->FragFrame.wcid == pEntry->wcid) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s: Clear Wcid(%d) FragBuffer!!!\n", __FUNCTION__, pEntry->wcid));
		RESET_FRAGFRAME(pAd->FragFrame);
	}
	PhyMode = wdev->PhyMode;
    pAd->ApCfg.MBSSID[pEntry->apidx].wdev.PhyMode = wdev->PhyMode;
...
//---Add by shiang for debug
    ...
    FrameLen += build_vendor_ie(pAd, &(pAd->ApCfg.MBSSID[pEntry->apidx].wdev), (pOutBuffer + FrameLen));
{
	ULONG TmpLen;
...
#define BW_80		BAND_WIDTH_80
	        ...
	        */
	        ...
	        mtk_deliver_band_steering_event(RtmpOsGetNetIfIndex(wdev->MSSIDDev), ATH_EVENT_BSTEERING_NODE_ASSOCIATED, sizeof(assoc), (u_int8_t*)&assoc);
		}
#endif
