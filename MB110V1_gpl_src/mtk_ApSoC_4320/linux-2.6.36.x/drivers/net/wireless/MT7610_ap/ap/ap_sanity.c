	...
	IE_LISTS *ie_lists)
{
    ...
    while (((UCHAR *)eid_ptr + eid_ptr->Len + 1) < ((UCHAR *)Fr + MsgLen))
    {
        switch(eid_ptr->Eid)
        {
			...
			case IE_EXT_CAPABILITY:
	            NdisMoveMemory(&ie_lists->ExtCapInfo, eid_ptr->Octet,
	                eid_ptr->Len > sizeof(EXT_CAP_INFO_ELEMENT) ? sizeof(EXT_CAP_INFO_ELEMENT) : eid_ptr->Len);
            ...
            case IE_WPA2:
                ...
                if(NdisEqualMemory(MTK_OUI, eid_ptr->Octet, 3))
                {
                    ...
                    else
                    {
						...
						DBGPRINT(RT_DEBUG_OFF, ("Peer does not support MWDS\n"));
                    }
				...
				if (NdisEqualMemory(eid_ptr->Octet, TP_OUI, 3))
				{
					...
					{
						pEntry->bSupportMWDS = TRUE;
						DBGPRINT(RT_DEBUG_OFF, ("Peer onemesh supports 4addr\n"));
					}
					break;
				}
#endif /* TP_IE_SUPPORT */
	...
	else
	{
		...
		{
			pEntry->bSupportMWDS = TRUE;
			DBGPRINT(RT_DEBUG_OFF, ("%s() - onemesh forces to support 4addr on backhaul\n", __FUNCTION__));
		}
#endif
