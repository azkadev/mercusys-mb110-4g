...
#ifdef MWDS
	{"ApMWDSinfo",				Set_Ap_MWDS_Show_Proc},
#ifdef APCLI_SUPPORT
	{"ApCliMWDSinfo",			Set_ApCli_MWDS_Show_Proc},
#endif /* APCLI_SUPPORT */
#endif /* MWDS */
	{NULL,}
};
...
static int _rtmp_BssEntry_cmp(const void *a, const void *b)
{
	return ((const BSS_ENTRY*)b)->Rssi - ((const BSS_ENTRY*)a)->Rssi;
}
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
	...
	/* zhu haiming: port from MT7628, Change ',' to '\b', 
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
	...
	PSTRING value = NULL;
	INT Status = NDIS_STATUS_SUCCESS;
	PSTRING tmp = pIoctlCmdStr->u.data.pointer;
...
#define MAX_TPIE_LEN (1024*24)
            ...
            pObj = (POS_COOKIE) pAd->OS_Cookie;
            DBGPRINT(RT_DEBUG_OFF, ("==>OID_GET_TPIE_INFO\n"));
                   ...
                   && (pAd->ApCfg.ApCliTab[ifIndex].SsidLen != 0))
            {
                   for (i = 0; i < MaxWcidNum; i++)
                   {
                        ...
                        if ( IS_ENTRY_APCLI(pEntry)
                                ...
                                && (pEntry->PortSecured == WPA_802_1X_PORT_SECURED))
                                  {
                                          ...
                                          {
                                                 DBGPRINT(RT_DEBUG_TRACE, ("pAd->ScanTab.BssEntry[%d].Bssid = %02x:%02x:%02x:%02x:%02x:%02x\n", k, 
                                                  ...
                                                  if (MAC_ADDR_EQUAL(pAd->ScanTab.BssEntry[k].Bssid, pEntry->Addr))
                                                  {
                                                           ...
                                                           UINT tmpLen = 0;
                                                          UCHAR *ptmp = NULL;     
                                                           DBGPRINT(RT_DEBUG_OFF, ("Find connected AP in ScanTab\n") );
                                                      tmpLen = strlen(msg);
                              ...
                              {
                                           status = -1;
                                           DBGPRINT(RT_DEBUG_OFF, ("Can't find connected AP in ScanTab\n"));
                                }
                break;
                           }
                   }
            ...
            //DBGPRINT(RT_DEBUG_OFF, ("%s %d wrq->u.data.length = %d, msg = %s\r\n", __FUNCTION__, __LINE__, wrq->u.data.length, msg)); 
            error = copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length); 
            if (error == 0)         
            {                       
                   //DBGPRINT(RT_DEBUG_OFF, ("%s %d: copy_to_user() OK: %s\n", __FUNCTION__, __LINE__, msg));  
                   status = NDIS_STATUS_SUCCESS;
            }               
            else            
            {                       
                   //DBGPRINT(RT_DEBUG_OFF, ("%s %d: copy_to_user() fail, bytes: %d\n", __FUNCTION__, __LINE__, error)); 
                   status = NDIS_STATUS_FAILURE;   
            }
		...
		#define STATION_LIST_LEN 	(sizeof(wlanif_staList_t) + MAX_LEN_OF_MAC_TABLE*STATION_INFO_LEN)
                ...
                //DBGPRINT(RT_DEBUG_ERROR, ("%s %d: STATION_LIST_LEN = %d, MAX_LEN_OF_MAC_TABLE = %d, STATION_INFO_LEN = %d\r\n", __FUNCTION__, __LINE__, STATION_LIST_LEN, MAX_LEN_OF_MAC_TABLE, STATION_INFO_LEN));   
                ...
                {       
                    ...
                    //CHAR rssiMax = RTMPMaxRssi(pAd, pEntry->RssiSample.AvgRssi0,
                                            //pEntry->RssiSample.AvgRssi1,
                                            //pEntry->RssiSample.AvgRssi2);
                    
                    if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC) && (pEntry->apidx == MAIN_MBSSID))
                    {   
                        //DBGPRINT(RT_DEBUG_ERROR, ("rssiAvg = %d, rssiMax = %d\r\n", rssiAvg, rssiMax));
                        //DBGPRINT(RT_DEBUG_ERROR, ("copy entry index = %d\r\n", i));
                        ...
                        msg->staInfo[idx].rssi = rssiAvg + 90; /*95��qca�ĵ�Ƶ����*/;   
                        //msg->staInfo[idx].rssi = rssiAvg + 95; /*95��qca�ĵ�Ƶ����*/; 
                        //DBGPRINT(RT_DEBUG_ERROR, ("copy entry msg->staInfo[%d].rssi = %d\r\n", idx, msg->staInfo[idx].rssi));
                ...
                {       
                    ...
                    for (i = 0; i < idx; i++)
                    {
                        DBGPRINT(RT_DEBUG_ERROR, ("%s %d entry[%d] mac = %02x:%02x:%02x:%02x:%02x:%02x\r\n", __FUNCTION__, __LINE__, i, PRINT_MAC(msg->staInfo[i].mac))); 
                    }
                    */
                    msg->sta_num = idx; 
                    wrq->u.data.length = STATION_LIST_LEN;
                    //DBGPRINT(RT_DEBUG_ERROR, ("%s %d wrq->u.data.length = %d\r\n", __FUNCTION__, __LINE__, wrq->u.data.length)); 
                    error = copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length); 
                    if (error == 0)     
                    {           
                        //DBGPRINT(RT_DEBUG_ERROR, ("%s %d: copy_to_user() OK\n", __FUNCTION__, __LINE__)); 
                        status = NDIS_STATUS_SUCCESS;
                    }       
                    else        
                    {           
                        DBGPRINT(RT_DEBUG_ERROR, ("%s %d: copy_to_user() fail\n", __FUNCTION__, __LINE__)); 
                        status = NDIS_STATUS_FAILURE;   
                    }   
                }   
                else
                {       
                    //DBGPRINT(RT_DEBUG_ERROR, ("%s %d: No entrys get.\n", __FUNCTION__, __LINE__));
                    status = NDIS_STATUS_FAILURE;
                }       
        ...
        case OID_802_11R_GET_STATION_INFO:
            {
                ...
                //DBGPRINT(RT_DEBUG_ERROR, ("sta mac = %02x:%02x:%02x:%02x:%02x:%02x\r\n", PRINT_MAC(pSta->mac)));  
                COPY_MAC_ADDR(sta.mac, pSta->mac);
                for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++)  
                {
                    MAC_TABLE_ENTRY *pEntry = &pAd->MacTab.Content[i];      
                    DBGPRINT(RT_DEBUG_ERROR, ("sta mac = %02x:%02x:%02x:%02x:%02x:%02x, pEntry->Addr = %02x:%02x:%02x:%02x:%02x:%02x\r\n", PRINT_MAC(sta.mac), PRINT_MAC(pEntry->Addr)));
                    ...
                    if (IS_EQUAL_MAC(sta.mac, pEntry->Addr))        
                    {
                        ...
                        //CHAR rssi = (RTMPAvgRssi(pAd, &pEntry->RssiSample) + 95); /*95��qca�ĵ�Ƶ����*/
                        ...
                        DBGPRINT(RT_DEBUG_ERROR, ("%s %d: sta.sta.lastRxRate = %d, sta.sta.lastTxRate = %d, sta.sta.rssi = %d, sta.sta.idle = %d\n", __FUNCTION__, __LINE__, sta.sta.lastRxRate, sta.sta.lastTxRate, sta.sta.rssi, sta.sta.idle));      
                        found = 1;
                        break;
                    }
                }
                ...
                {       
                    ...
                    DBGPRINT(RT_DEBUG_ERROR, ("%s %d: Status = %d, wrq->u.data.length = %d\n", __FUNCTION__, __LINE__, Status, wrq->u.data.length));        
                    ...
                    {
                        //DBGPRINT(RT_DEBUG_ERROR, ("%s %d: copy_to_user() success\n", __FUNCTION__, __LINE__));            
                        //return NDIS_STATUS_SUCCESS;   
                    }   
                }
                else
                {
                    DBGPRINT(RT_DEBUG_ERROR, ("%s %d: status info not found.\n", __FUNCTION__, __LINE__));      
                    Status = NDIS_STATUS_FAILURE;
                }
            }
            break;
        case OID_802_11R_GET_PHYCAP_INFO:
            {
                ...
                {
                    if ( IS_EQUAL_MAC(staPhyCapInfo.mac, pAd->ApCfg.MBSSID[i].Bssid))  
                    {
        ...
        case OID_802_11R_GET_EXAP_INFO:
			...
			#define RSSI_BASE (-100)
                    ...
                    for (i = 0; i < pAd->ScanTab.BssNr; i++)
                    {
                        ...
                        if (j >= maxBssNum)
                        {
                            printk("%s %d we just get %d entries.", __FUNCTION__, __LINE__, maxBssNum);
                            break;
                        }
                    }
        ...
        case OID_802_11R_CLR_ASSOC_INFO:
            {           
                UCHAR Addr[MAC_ADDR_LEN];
                MAC_TABLE_ENTRY *pEntry = NULL;
                printk("%s %d OID_802_11R_CLR_ASSOC_INFO.", __FUNCTION__, __LINE__);
                ...
                if ( !IS_ENTRY_CLIENT(pEntry) )
                    break;
                printk("%s %d MacTableDeleteEntry.", __FUNCTION__, __LINE__);
                ...
                if (pAd->ApCfg.bMACRepeaterEn == TRUE)
                {
                    ...
                    pReptEntry = RTMPLookupRepeaterCliEntry(pAd, TRUE, Addr);
...
#endif /* DOT11_N_SUPPORT */
                        printk("%s %d mlmeEnque:APCLI_CTRL_DISCONNECT_REQ.", __FUNCTION__, __LINE__);
	...
	IN  PSTRING arg)
{
	...
	if (pObj->ioctl_if_type != INT_APCLI)
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s:: unexpected if type %#x\n", __FUNCTION__, pObj->ioctl_if_type));
#ifdef APCLI_AUTO_CONNECT_SUPPORT
		return FALSE;
#endif /* APCLI_AUTO_CONNECT_SUPPORT */
	}
	...
	IN PSTRING arg)
{
	...
	{
		DBGPRINT(RT_DEBUG_WARN, ("Set_ApCli_AutoConnect_Proc() is still running\n"));
		return TRUE;
	}
	...
	IN	PSTRING			arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("IF(%02X:%02X:%02X:%02X:%02X:%02X) Set_WscGetConf_Proc trigger WSC state machine, wscGetConfMode=%ld\n",
                                mac_addr[0],
                                mac_addr[1],
                                mac_addr[2],
                                mac_addr[3],
	...
	switch(cmd)
	{
		...
		case CMD_RTPRIV_IOCTL_AP_SIOCGIWRATEQ:
         ...
         case CMD_RTPRIV_IOCTL_CHAN_UTILIZATION:
            //printk("%s: %d obtain channel util %d\n", __func__, __LINE__, pAd->phy_ctrl.QloadChanUtil);
            copy_to_user(pData, &(pAd->QloadChanUtil), sizeof(UINT8));
