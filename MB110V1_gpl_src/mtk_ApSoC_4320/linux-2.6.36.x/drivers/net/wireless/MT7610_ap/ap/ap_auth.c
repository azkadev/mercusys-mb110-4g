    ...
    if (! ApCheckAccessControlList(pAd, Addr2, apidx))
    {
		...
		if (pEntry)
		{
			if (pEntry->apidx == MAIN_MBSSID)
			{
	            ...
	            memcpy(event.client_addr, Addr2, MAC_ADDR_LEN);
	            /*TO DO -rssi*/
	            DBGPRINT(RT_DEBUG_ERROR, ("%s %d auth rej addr = %02X:%02X:%02X:%02X:%02X:%02X, sysindex = %d\r\n",
	                    __FUNCTION__, __LINE__, event.client_addr[0],  event.client_addr[1], event.client_addr[2], event.client_addr[3],
	                    event.client_addr[4], event.client_addr[5], RtmpOsGetNetIfIndex(pAd->ApCfg.MBSSID[apidx].MSSIDDev)) );
	            mtk_deliver_band_steering_event(RtmpOsGetNetIfIndex(pAd->ApCfg.MBSSID[apidx].MSSIDDev),
	                    ATH_EVENT_BSTEERING_TX_AUTH_FAIL, sizeof(event), (u_int8_t*)&event);
			}
        }
#endif
