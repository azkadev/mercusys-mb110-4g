...
void percentage_delta_pwr(RTMP_ADAPTER *ad) 
{
	...
	*/
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s::drop the BBP transmit power by %d dBm!\n",
		__FUNCTION__, 
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s::<After> total drop power = %d + %d dBm, TXBE_R4 = 0x%0x , PowerPercentageWithBBP = %d\n", 
...
{
	UINT32 i, choffset, idx, ss_offset_g;
...
void MT76x0_UpdateRssiForChannelModel(RTMP_ADAPTER * pAd)
{
	...
	DBGPRINT(RT_DEBUG_INFO, ("%s:: rx0_rssi(%d), rx1_rssi(%d)\n", 
		__FUNCTION__, rx0_rssi, /*rx1_rssi*/(INT32)(pAd->ApCfg.RssiSample.LastRssi1)));	
...
INT32 mt76x0_sku_calculate_TxPwrAdj(struct _RTMP_ADAPTER *ad,struct _TXWI_NMAC *txwi_n)
{
	...
	if (currentBW == txwi_n->BW )
		return 0;

	MCS = txwi_n->MCS;

	if(txwi_n->PHYMODE == MODE_HTMIX || txwi_n->PHYMODE == MODE_HTGREENFIELD)
	{
		MCS = txwi_n->MCS %8;
		if(txwi_n->BW == BW_20)
		{
			sku_table_delta = ad->chipCap.rate_pwr_table.HT20in40Adj[MCS];
		}
		else if(txwi_n->BW == BW_40)
		{
			sku_table_delta = ad->chipCap.rate_pwr_table.HT40in80Adj[MCS];
		}
	}
	if  (txwi_n->PHYMODE == MODE_VHT)
	{
		if(txwi_n->BW == BW_20)
		{
			...
			sku_table_delta = ad->chipCap.rate_pwr_table.VHT20in4080Adj[MCS];
		}
 		else if(txwi_n->BW == BW_40)
 		{
			sku_table_delta = ad->chipCap.rate_pwr_table.VHT40in80Adj[MCS];
		}
	}
	DBGPRINT(RT_DEBUG_INFO, ("%s ==>current bw %d\n", __FUNCTION__, txwi_n->BW));
	DBGPRINT(RT_DEBUG_INFO, ("%s ==>current sku table delta %d\n", __FUNCTION__, sku_table_delta));
	if (sku_table_delta !=0)
	{
		/* according to mtk, these registers should be set before set txpwradj. but why? */
		RTMP_BBP_IO_READ32(ad, IBI_R9, &bbp_val1);
		RTMP_BBP_IO_READ32(ad, TXBE_R5, &bbp_val2);
		bbp_val1=bbp_val1&0xfffffbff;
		bbp_val2=bbp_val2&0xfff0ffff;
		RTMP_BBP_IO_WRITE32(ad, IBI_R9, bbp_val1);
		RTMP_BBP_IO_WRITE32(ad, TXBE_R5, bbp_val2);
	...
	DBGPRINT(RT_DEBUG_INFO, ("%s ==>PwrAdj %d\n", __FUNCTION__, PwrAdj));
	return PwrAdj;
}
...
#ifdef SINGLE_SKU_V2
	pChipOps->SkuTxPwrAdj = mt76x0_sku_calculate_TxPwrAdj;
#else
	pChipOps->SkuTxPwrAdj = NULL;
#endif
...
extern UCHAR GetVHT40CenCh(UCHAR prim_ch);
	...
	IN UCHAR 			channel)
{
...
#endif
	CHAR ch_delta_pwr = 0;
	INT32 rate_pwr = 0, rate_pwr_before_adjust = 0, sku_pwr = 0, bwDeltaPwr = 0, bwDeltaPwr40 = 0, bwDeltaPwr80 = 0;
	BOOLEAN bFound = FALSE;
	/* CHAR SkuBasePwr; */
	const CHAR DefaultTargetPwr = pAd->DefaultTargetPwr;

	INT32 VHT20in4080[SINGLE_SKU_TABLE_VHT20_LENGTH]; // only for storing HT20 sku_pwr in BBPcurrentBW==40/20 case
	INT32 VHT40in80[SINGLE_SKU_TABLE_VHT_LENGTH ]; // only for storing HT40 sku_pwr in BBPcurrentBW==80/20 case
	INT32 HT20in40[SINGLE_SKU_TABLE_HT_LENGTH >>1]; // only for storing HT40 sku_pwr in BBPcurrentBW==80/20 case
	INT32 HT40in80[SINGLE_SKU_TABLE_HT_LENGTH >>1]; // only for storing HT40 sku_pwr in BBPcurrentBW==80/20 case
	/*
	...
	*/
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s ==> pAd->CommonCfg.BBPCurrentBW = %d\n", 
					__FUNCTION__, pAd->CommonCfg.BBPCurrentBW));

	
	if (pAd->chipCap.delta_tw_pwr_bw40_5G & 0x80)
	{
		if (pAd->chipCap.delta_tw_pwr_bw40_5G & 0x40)
		{
			/* We should add power offset of 40M back.*/
			bwDeltaPwr40 = pAd->chipCap.delta_tw_pwr_bw40_5G & 0x3f;
		}
		else
		{
			bwDeltaPwr40 = -(pAd->chipCap.delta_tw_pwr_bw40_5G & 0x3f);
	...
	if (pAd->chipCap.delta_tw_pwr_bw80 & 0x80)
	{
		if (pAd->chipCap.delta_tw_pwr_bw80 & 0x40)
		{
			/* We should add power offset of 80M back.*/
			bwDeltaPwr80 = pAd->chipCap.delta_tw_pwr_bw80 & 0x3f;
		}
		else
		{
			bwDeltaPwr80 = -(pAd->chipCap.delta_tw_pwr_bw80 & 0x3f);
	...
	if (pAd->CommonCfg.BBPCurrentBW == BW_40)
	{
		bwDeltaPwr = bwDeltaPwr40;
	}
	else if (pAd->CommonCfg.BBPCurrentBW == BW_80)
	{
		bwDeltaPwr = bwDeltaPwr80;
	}
	else
	{
		bwDeltaPwr = 0;
	...
	DlListForEachSafe(ch, ch_temp, &pAd->SingleSkuPwrList, CH_POWER, List)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("%s ==>pAd->CommonCfg.Channel is %d\n", __FUNCTION__, pAd->CommonCfg.Channel));
		if (pAd->CommonCfg.Channel == ch->channel)
		{
			DBGPRINT(RT_DEBUG_TRACE, ("%s ==>20in4080 channel is %d\n", __FUNCTION__, ch->channel));
			/* cal VHT20in4080 */
			for (i = 0; i < (SINGLE_SKU_TABLE_VHT20_LENGTH); i++)
			{
				rate_pwr_before_adjust = pAd->chipCap.rate_pwr_table.VHT[i].MCS_Power + DefaultTargetPwr - bwDeltaPwr;
#ifndef ENABLE_SKU_VHT20_VHT40
				/* for VHT20 VHT40 VHT80 MCS8 and MCS9, use VHT80 MCS8 and MCS9 */
				//store VHT20 sku_pwr for TXWI
				if (i>=8)
				{
					/* VHT 20 can't go beyond MCS8 */
					VHT20in4080[i] = (ch->PwrVHT80[8] > rate_pwr_before_adjust)? \
						rate_pwr_before_adjust : ch->PwrVHT80[8];
				}
				else
				{
					VHT20in4080[i] = (ch->PwrHT20[i] > rate_pwr_before_adjust)? \
						rate_pwr_before_adjust : ch->PwrHT20[i];
				}
#else
				//store VHT20 sku_pwr for TXWI
				VHT20in4080[i] = (ch->PwrVHT20[i] > rate_pwr_before_adjust)? \
					rate_pwr_before_adjust : ch->PwrVHT20[i];
#endif

			DBGPRINT(RT_DEBUG_TRACE, ("%s ==>VHT20in4080[%d].sku_pwr is %d\n", __FUNCTION__, i, 
					VHT20in4080[i]));
			}
			...
			{
				rate_pwr_before_adjust = pAd->chipCap.rate_pwr_table.HT[i].MCS_Power + DefaultTargetPwr - bwDeltaPwr;
				//store VHT20 sku_pwr for TXWI
				HT20in40[i] = (ch->PwrHT20[i] > rate_pwr_before_adjust)? \
					rate_pwr_before_adjust : ch->PwrHT20[i];
			}
	...
	DlListForEachSafe(ch, ch_temp, &pAd->SingleSkuPwrList, CH_POWER, List)
	{
		DBGPRINT(RT_DEBUG_TRACE, ("%s ==>CentralChannel is %d\n", __FUNCTION__, GetVHT40CenCh(pAd->CommonCfg.Channel)));
		/* calc VHT40in80 */
		if (GetVHT40CenCh(pAd->CommonCfg.Channel)== ch->channel)
		{
			DBGPRINT(RT_DEBUG_TRACE, ("%s ==>40in80 channel is %d\n", __FUNCTION__, ch->channel));
			for (i = 0; i < (SINGLE_SKU_TABLE_VHT_LENGTH ); i++)
			{
				rate_pwr_before_adjust = pAd->chipCap.rate_pwr_table.VHT[i].MCS_Power + DefaultTargetPwr - bwDeltaPwr + bwDeltaPwr40;
#ifndef ENABLE_SKU_VHT20_VHT40
				/* for VHT20 VHT40 VHT80 MCS8 and MCS9, use VHT80 MCS8 and MCS9 */
				//store VHT20 sku_pwr for TXWI
				if (i>=8)
				{
					//store VHT40 sku_pwr for TXWI
					VHT40in80[i]  = (ch->PwrVHT80[i] > rate_pwr_before_adjust)? \
						rate_pwr_before_adjust : ch->PwrVHT80[i];
				}
				else
				{
					//store VHT40 sku_pwr for TXWI
					VHT40in80[i] = (ch->PwrHT40[i] > rate_pwr_before_adjust)? \
						rate_pwr_before_adjust : ch->PwrHT40[i];
				}
#else
				//store VHT40 sku_pwr for TXWI
				VHT40in80[i] = (ch->PwrVHT40[i] > rate_pwr_before_adjust)? \
			...
			for (i = 0; i < (SINGLE_SKU_TABLE_HT_LENGTH >>1); i++)
			{
				rate_pwr_before_adjust = pAd->chipCap.rate_pwr_table.HT[i].MCS_Power + DefaultTargetPwr - bwDeltaPwr + bwDeltaPwr40;
				//store HT40 sku_pwr for TXWI
				HT40in80[i] = (ch->PwrHT40[i] > rate_pwr_before_adjust)? \
					rate_pwr_before_adjust : ch->PwrHT40[i];
			}
	...
	DlListForEachSafe(ch, ch_temp, &pAd->SingleSkuPwrList, CH_POWER, List)
	{
		if (channel == ch->channel)
		{
			for (i = 0; i < (SINGLE_SKU_TABLE_HT_LENGTH >> 1); i++)
			{
				...
				pAd->chipCap.rate_pwr_table.HT[i].sku_pwr = sku_pwr;

				pAd->chipCap.rate_pwr_table.HT20in40Adj[i] = HT20in40[i] - pAd->chipCap.rate_pwr_table.HT[i].sku_pwr + bwDeltaPwr;
				pAd->chipCap.rate_pwr_table.HT40in80Adj[i] = HT40in80[i] - pAd->chipCap.rate_pwr_table.HT[i].sku_pwr + bwDeltaPwr - bwDeltaPwr40;
				DBGPRINT(RT_DEBUG_TRACE, ("%s ==>HT20in40Adj[%d].sku_pwr is %d\n", __FUNCTION__, i, 
					pAd->chipCap.rate_pwr_table.HT20in40Adj[i]));
...
#endif
			{
				...
				if (pAd->CommonCfg.BBPCurrentBW == BW_20)
				{
					...
					if (i >= 8)
					{
						sku_pwr = (ch->PwrVHT80[8] > rate_pwr_before_adjust) ? \
								rate_pwr_before_adjust : ch->PwrVHT80[8];
					}
					else
					{
					...
					if (i >= 8)
					{
						...
						sku_pwr = (ch->PwrVHT20[8] > rate_pwr_before_adjust) ? \
				...
				pAd->chipCap.rate_pwr_table.VHT[i].sku_pwr = sku_pwr;
				/* mind the bwDeltaPwr; if current bwDeltaPwr < 0, the targetpower will be lower, leading to a high output. so VHT20in4080Adj
				should add it back. */
				if (i < SINGLE_SKU_TABLE_VHT20_LENGTH)
				{
					pAd->chipCap.rate_pwr_table.VHT20in4080Adj[i] = VHT20in4080[i] - pAd->chipCap.rate_pwr_table.VHT[i].sku_pwr + bwDeltaPwr;
				}

				pAd->chipCap.rate_pwr_table.VHT40in80Adj[i] = VHT40in80[i] - pAd->chipCap.rate_pwr_table.VHT[i].sku_pwr + bwDeltaPwr - bwDeltaPwr40;

				if (i < SINGLE_SKU_TABLE_VHT20_LENGTH)
					DBGPRINT(RT_DEBUG_TRACE, ("%s ==>VHT20in4080Adj[%d] is %d\n", __FUNCTION__, i, 
									pAd->chipCap.rate_pwr_table.VHT20in4080Adj[i]));
				
				DBGPRINT(RT_DEBUG_TRACE, ("%s ==>VHT40in80Adj[%d] is %d\n", __FUNCTION__, i, 
									pAd->chipCap.rate_pwr_table.VHT40in80Adj[i]));
...
#endif /* RTMP_TEMPERATURE_COMPENSATION */
	{
		DBGPRINT(RT_DEBUG_TRACE, ("No TSSI or Temperature Compensation used.\n"));
	}
}
    ...
    IN PRTMP_ADAPTER pAd)
{
	...
	RTMP_IO_READ32(pAd, CAL_R28, &reg_val);
	DBGPRINT(RT_DEBUG_TRACE, ("==== CAL_R28(%#x) [%x] ,RXBackup [%x]\n", CAL_R28, reg_val, pAd->chipCap.RXIQBackup));  
	if ((reg_val & 0xffff) == 0)
	{
		DBGPRINT(RT_DEBUG_WARN, (" RX Cal =0 , use RXBackup [%x] to restore!!\n", pAd->chipCap.RXIQBackup));  

		reg_val = pAd->chipCap.RXIQBackup;
		RTMP_IO_WRITE32(pAd, CAL_R28, reg_val);

		RTMP_IO_READ32(pAd, CAL_R28, &reg_val);
		if (reg_val != pAd->chipCap.RXIQBackup)
		{
			DBGPRINT(RT_DEBUG_ERROR, (" result [%#x] %#x!!\n", reg_val, pAd->chipCap.RXIQBackup)); 
		}
	}
	else
	{
		DBGPRINT(RT_DEBUG_TRACE, ("CAL_R28(%#x) [%x] not 0 , backup [%x] \n", CAL_R28, reg_val, pAd->chipCap.RXIQBackup));
