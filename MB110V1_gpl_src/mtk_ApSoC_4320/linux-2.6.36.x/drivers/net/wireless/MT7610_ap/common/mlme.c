...
VOID TP_SetWSCLedStatus(unsigned int WSCLedStatus)
{
	...
	/*
	if (TP_WSCLEDStatus_5G == TP_WSC_LED_SUCCESS && WSCLedStatus == TP_WSC_LED_END)
	{
		return;
	}
	*/
...
VOID TP_CheckWSCLEDStatus(PWSC_CTRL pWscCtrl)
{
	...
	/* zeng weiji, 如果2.4G WPS已成功连上，则5G WPS状态设为结束，否则LED灯会偶尔闪烁 */
...
/* Add TP-Link Spcific Vendor IE Support. liuliming */
                    ...
                    //hex_dump1("======pEid Octet=====:", pEid->Octet, 3);
#endif /* TP_IE_SUPPORT */
					break;
...
#endif
#if defined(APCLI_SUPPORT) || defined(TP_NETWORK_ROAMING)
	UCHAR i;
#endif
	...
	if (Idx == BSS_NOT_FOUND) 
	{
		if (Tab->BssNr >= MAX_LEN_OF_BSS_TABLE)
	    {
			...
			*/
            ...
            {
                if (MAC_ADDR_EQUAL(pAd->ApCfg.MBSSID[i].Bssid, ie_list->Bssid)
