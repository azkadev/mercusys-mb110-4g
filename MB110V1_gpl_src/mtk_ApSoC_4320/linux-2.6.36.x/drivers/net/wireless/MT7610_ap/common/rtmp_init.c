	...
	IN  BOOLEAN		bHardReset)
{
...

	{
		...
		RTMP_IO_READ32(pAd,0x150c,&pn_mode_value);
		pn_mode_value |= 0x01;
		RTMP_IO_WRITE32(pAd,0x150c,pn_mode_value);
	}
	DBGPRINT(RT_DEBUG_TRACE, ("<-- NICInitializeAsic\n"));
...
INT RtmpRaDevCtrlInit(VOID *pAdSrc, RTMP_INF_TYPE infType)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("AP Driver version-%s\n", AP_DRIVER_VERSION));
#endif /* CONFIG_AP_SUPPORT */

	DBGPRINT(RT_DEBUG_OFF, ("pAd->infType=%d\n", pAd->infType));
