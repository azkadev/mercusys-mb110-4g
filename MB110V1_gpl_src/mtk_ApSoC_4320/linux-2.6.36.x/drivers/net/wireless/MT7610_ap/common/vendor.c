				    ...
				    BCN_IE_LIST *ie_list)
{
	...
	if (vendor_ie->pointer != NULL && vendor_ie->length != 0) {
		NdisMoveMemory(vendor_ie_temp, vendor_ie->pointer,
			    vendor_ie->length);
		os_free_mem(NULL,vendor_ie->pointer);
		NdisMoveMemory(vendor_ie_temp + vendor_ie->length, info_elem,
			    info_elem->Len + 2);
		vendor_ie->pointer = vendor_ie_temp;
		vendor_ie->length = vendor_ie_len;
	} else {
		NdisMoveMemory(vendor_ie_temp, info_elem, info_elem->Len + 2);
		vendor_ie->pointer = vendor_ie_temp;
		vendor_ie->length = vendor_ie_len;
	}
}*/
