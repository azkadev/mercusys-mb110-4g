...
VOID WpaEAPPacketAction(
    ...
    IN MLME_QUEUE_ELEM *Elem) 
{   
	DBGPRINT(RT_DEBUG_TRACE, ("%s receiving EAP packets in PSK case, Not use!!!\n", __func__));
}
    ...
    IN MLME_QUEUE_ELEM  *Elem) 
{
	...
	if ((pEntry->AllowInsPTK == TRUE) && bWPA2) {
		...
		DBGPRINT(RT_DEBUG_WARN,("%s(%d)-%d: update CCMP_BC_PN to %llu\n",
			__func__, pEntry->Aid, kid, pEntry->CCMP_BC_PN[kid]));
	}
...
#ifdef MWDS
            if(pEntry &&
                   (pEntry->apidx < MAX_APCLI_NUM))
            {
                if((pEntry->PortSecured == WPA_802_1X_PORT_SECURED) && (pEntry->AuthMode != Ndis802_11AuthModeWPAPSK))
                {
                    if( pAd->ApCfg.ApCliTab[ifIndex].wdev.bDefaultMwdsStatus &&
                            pAd->ApCfg.ApCliTab[ifIndex].wdev.bSupportMWDS )
                        MWDSAPCliPeerEnable(pAd, &pAd->ApCfg.ApCliTab[pEntry->apidx], pEntry, TRUE);
                    else
                        MWDSAPCliPeerDisable(pAd, &pAd->ApCfg.ApCliTab[pEntry->apidx], pEntry);
                }
            }
#endif /* MWDS */
#endif /* APLCI_SUPPORT */
    ...
    IN MLME_QUEUE_ELEM  *Elem) 
{
	...
	if (pEntry->AllowUpdateRSC == TRUE) {
		...
		pEntry->Init_CCMP_BC_PN_Passed[kid] = FALSE;
		DBGPRINT(RT_DEBUG_WARN,("%s(%d)-%d: update CCMP_BC_PN to %llu\n",
			__FUNCTION__, pEntry->Aid, kid, pEntry->CCMP_BC_PN[kid]));
	}
