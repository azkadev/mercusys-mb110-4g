							...
							PUCHAR pTargetIPAddr)
{
	...
	UINT8 i;
	BOOLEAN IsDAD = FALSE;
	PNET_DEV NetDev = pMbss->MSSIDDev;
	DBGPRINT(RT_DEBUG_OFF, ("%s\n", __FUNCTION__));
							...
							IN MLME_QUEUE_ELEM *Elem)
{
	...
	for (APIndex = 0; APIndex < MAX_MBSSID_NUM(pAd); APIndex++) {
		if (MAC_ADDR_EQUAL(WNMFrame->Hdr.Addr3, pAd->ApCfg.MBSSID[APIndex].Bssid)) {
	...
	if (!pWNMCtrl) {
		DBGPRINT(RT_DEBUG_TRACE, ("%s Can not find Peer Control\n", __func__));
		return;
	}
	...
	if ((NetDev== NULL) || ((NetDev!= NULL) &&
			!(RTMP_OS_NETDEV_STATE_RUNNING(NetDev))))
	{
		/* the interface is down, so we can not send any msg */
		return;
	}
	...
	if (IsFound) {
		DBGPRINT(RT_DEBUG_TRACE, ("%s Find peer address in BTMPeerList already\n", __func__));
		return;
	}
	...
	if (!BTMPeerEntry) {
		DBGPRINT(RT_DEBUG_TRACE, ("%s Not available memory\n", __func__));
		return;
	}
						  ...
						  IN MLME_QUEUE_ELEM *Elem)
{
	...
	DlListForEach(BTMPeerEntry, &pWNMCtrl->BTMPeerList, BTM_PEER_ENTRY, List)
	{
		KV_PRINT(("peer entry in list: %02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(BTMPeerEntry->PeerMACAddr)));
		if (MAC_ADDR_EQUAL(BTMPeerEntry->PeerMACAddr, WNMFrame->Hdr.Addr2))
			IsFound = TRUE;
	...
	if (!IsFound) {
			KV_PRINT(("Not found peer entry in list\n"));
		//return;
	}
	else
	{	
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
	...
	PNET_DEV NetDev = pAd->ApCfg.MBSSID[Event->ControlIndex].MSSIDDev;
...
#if 0 //Haipin: Why should we do this?
    		pEntry->BTMDisassocCount = (pEntry->BTMDisassocCount);
#endif
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	NetDev = pAd->ApCfg.MBSSID[event.ControlIndex].MSSIDDev;
...
static UINT8 _resolve_reg_class(UINT8 channel)
{
    UINT8 regClass = IEEE80211_RRM_REGCLASS_RESERVED;
	...
	IN UINT32 btm_req_data_len)
{
	...
	/*set Request Mode*/
	//p_btm_req_data->reqmode |= BTM_ABRIDGED_OFFSET;
		...
		*/
		if (nr_flag) {
			...
			if (bss_index == BSS_NOT_FOUND) {
				DBGPRINT(RT_DEBUG_ERROR,("%s() BSS_NOT_FOUND\n", __func__));
				//continue;
				/*use default instead*/
				p_info->regclass = _resolve_reg_class(p_info->channum);
				p_info->capinfo = 0x1100;
				p_info->is_ht = 1;
				p_info->is_vht = 1;
				p_info->mobility = 0;
				p_info->phytype = 7;
		...
		BssidInfo.word = 0;
		BssidInfo.field.APReachAble = 3;//(p_info->ap_reachability == 0)?3:p_info->ap_reachability;
		BssidInfo.field.Security = 1;//p_info->security;
		BssidInfo.field.KeyScope = p_info->key_scope;
		BssidInfo.field.SepctrumMng = 1;//(p_info->capinfo & (1 << 8))?1:0;
		...
		BssidInfo.field.HT = 0;//p_info->is_ht;
		BssidInfo.field.VHT = 0;//p_info->is_vht;
		...
		DBGPRINT(RT_DEBUG_ERROR,
					("%s:%d RegulatoryClass is = %d\n",  __FUNCTION__, __LINE__, NeighborRepInfo.RegulatoryClass));

		{
			UINT8 OpSubIELen = p_info->preference ? 3:0;
			RRM_InsertNeighborRepIE_New(pAd, (pos+TmpLen), (PULONG)&TmpLen,
					sizeof(RRM_NEIGHBOR_REP_INFO)+OpSubIELen , &NeighborRepInfo);
			DBGPRINT(RT_DEBUG_ERROR,
						("%s:%d TmpLen=%d\n",  __FUNCTION__, __LINE__, TmpLen));
		}
		/*for samsung S8 */
		//RRM_InsertWBCSubIE(pAd,(pos+TmpLen),&TmpLen);
