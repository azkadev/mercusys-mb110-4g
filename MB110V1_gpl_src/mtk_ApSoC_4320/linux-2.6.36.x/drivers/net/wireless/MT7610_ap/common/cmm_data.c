				...
				if(IS_ENTRY_MWDS(pMacEntry))
				{
					...
					if(IS_MWDS_OPMODE_APCLI(pMacEntry))
					{
                        ...
                        if(pMacEntry->apidx < MAX_APCLI_NUM)
						    pTxBlk->pApCliEntry = &pAd->ApCfg.ApCliTab[pMacEntry->apidx];
					}
					else
#endif /* APCLI_SUPPORT */
	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	if (pAd->AmsduAttack_Sequence == pHeader->Sequence) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s:AMSDU ATTACK and drop others fragment packet of same sequence!\n", __func__));
		RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
		return;
	} else
		pAd->AmsduAttack_Sequence = 0xffff;
	if ((pHeader->FC.ToDs == 0) && (pHeader->FC.FrDs == 1)) { /*STA RX, check amsdu DA == 802.11 header RA?*/
		if (NdisCmpMemory(pHeader->Addr1, pDA, MAC_ADDR_LEN)) {
			pAd->AmsduAttack_Sequence = pHeader->Sequence;
			DBGPRINT(RT_DEBUG_ERROR, ("%s:AP TX(STA RX) AMSDU ATTACK\n", __func__));
			DBGPRINT(RT_DEBUG_OFF, ("802.11 Addr1=%02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(pHeader->Addr1)));
			DBGPRINT(RT_DEBUG_OFF, ("802.3 DA Addr=%02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(pDA)));
			RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
			return;
		}
	}
	if ((pHeader->FC.ToDs == 1) && (pHeader->FC.FrDs == 0)) {/*AP RX, check amsdu SA == 802.11 header TA?*/
		if (NdisCmpMemory(pHeader->Addr2, pSA, MAC_ADDR_LEN)) {
			pAd->AmsduAttack_Sequence = pHeader->Sequence;
			DBGPRINT(RT_DEBUG_ERROR, ("%s:STA TX(AP RX) AMSDU ATTACK\n", __func__));
			DBGPRINT(RT_DEBUG_OFF, ("802.11 Addr2=%02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(pHeader->Addr2)));
			DBGPRINT(RT_DEBUG_OFF, ("802.3 DA Addr=%02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(pSA)));
			RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
			return;
		}
	...
	*/
	...
	switch (TypeLen)
	{
		...
		case 0x86dd:
			{
#ifdef CONFIG_DOT11V_WNM
				MULTISSID_STRUCT *pMbss = (MULTISSID_STRUCT *)pAd->ApCfg.MBSSID;
	...
	IN RX_BLK *pRxBlk)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("pRxBlk->wcid:0x%x    pRxWI->RxWIWirelessCliID:0x%x\n",
		pRxBlk->wcid, wcid));
	if (VALID_WCID(wcid)) {
		pEntry = &pAd->MacTab.Content[wcid];
		if (!pEntry) {
			DBGPRINT(RT_DEBUG_ERROR, ("%s pEntry not find in wcid:0x%x\n", __func__, wcid));
			goto done;
		} else
			DBGPRINT(RT_DEBUG_TRACE, ("%s  SN:%d,FN:%d,PN:%llu, sec_mode: %d, mFrag:%d\n", __func__,
				pHeader->Sequence, pHeader->Frag, pRxBlk->CCMP_PN,
				pEntry->WepStatus, pHeader->FC.MoreFrag));
	}
	else
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s invalid wcid:0x%x\n", __func__, wcid));
		goto done;
	}
	...
	if (pHeader->Frag == 0)		/* Frag. Number is 0 : First frag or only one pkt*/
	{
		...
		if (pHeader->FC.MoreFrag)
		{
			...
			pAd->FragFrame.wcid = wcid;  /* to tell the fragment Buffer from which STA */
			if ((pEntry->WepStatus != Ndis802_11EncryptionDisabled) && (pEntry->PortSecured == WPA_802_1X_PORT_SECURED)) {
				pAd->FragFrame.sec_on = TRUE;
				pAd->FragFrame.sec_mode = pEntry->WepStatus;
				pAd->FragFrame.LastPN = pRxBlk->CCMP_PN;
				DBGPRINT(RT_DEBUG_OFF, ("Fragment PACKET INIT, wcid(%d)-Aid(%d) %d,%d,%llu - %d,%d,%llu \n",
					pAd->FragFrame.wcid, pEntry->Aid, pHeader->Sequence, pHeader->Frag, pRxBlk->CCMP_PN,
	...
	{
		DBGPRINT(RT_DEBUG_TRACE, ("Fragment Others PACKET, wcid(%d)-Aid(%d) %d,%d,%llu - %d,%d,%llu \n",
			pAd->FragFrame.wcid, pEntry->Aid, pHeader->Sequence, pHeader->Frag, pRxBlk->CCMP_PN,
			pAd->FragFrame.Sequence, pAd->FragFrame.LastFrag, pAd->FragFrame.LastPN));
		if ((pHeader->Sequence != pAd->FragFrame.Sequence) ||
			(pHeader->Frag != (pAd->FragFrame.LastFrag + 1)) ||
			(pAd->FragFrame.sec_on && ((pAd->FragFrame.sec_mode != pEntry->WepStatus) ||
