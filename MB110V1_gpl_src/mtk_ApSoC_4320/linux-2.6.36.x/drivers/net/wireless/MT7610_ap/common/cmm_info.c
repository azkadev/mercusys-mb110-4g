	...
	IN	PSTRING			arg)
{
    ...
    DBGPRINT(RT_DEBUG_ERROR, ("%s %d: send_event_to_network_roaming\r\n", __FUNCTION__, __LINE__)); 
#endif
...
/* 
    ...
    ==========================================================================
*/
#define	LINE_LEN	(4+33+3+20+23+9+7+7+3)	/* Channel+SSID+Bssid+Security+Signal+WiressMode+ExtCh+NetworkType*/
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	memset(msg, 0, sizeof(CHAR)*CHANNEL_LIST_LEN);
	sprintf(msg,"%s","\n");
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
    ...
    if (TRUE == bConnect)
    {
        ...
        RtmpDrvRateGet(pAd, HTPhyMode.field.MODE, HTPhyMode.field.ShortGI,
                       ...
                       newRateGetAntenna(HTPhyMode.field.MCS),&DataRate);
		DataRate /= 1000;
        staInfo.isi_rxratekbps = DataRate;
        staInfo.isi_inact = pEntry->NoDataIdleCount;
#if 0 //def TPIE_SUPPORT
		if (TRUE == bConnect)
		{
				ULONG Idx;
				DBGPRINT(RT_DEBUG_WARN, ("ApCli%d Connected Status : Connected \n", ifIndex ));
				DBGPRINT(RT_DEBUG_WARN, ("ApCli%d Connected AP-BSSID : %02X:%02X:%02X:%02X:%02X:%02X \n", ifIndex, PRINT_MAC(pEntry->Addr)));
				DBGPRINT(RT_DEBUG_WARN, ("ApCli%d Connected AP-SSID :  %s \n", ifIndex, pAd->ApCfg.ApCliTab[ifIndex].Ssid));
				DBGPRINT(RT_DEBUG_WARN, ("ApCli%d Connected LinkTime:  %lu \n", ifIndex, pAd->ApCfg.ApCliTab[ifIndex].ApCliLinkUpTime));
				DBGPRINT(RT_DEBUG_WARN, ("ApCli%d Connected RSSI : %-5d,%-5d,%-5d \n", ifIndex, pEntry->RssiSample.AvgRssi0,pEntry->RssiSample.AvgRssi1,pEntry->RssiSample.AvgRssi2));
				DBGPRINT(RT_DEBUG_WARN, ("ApCli%d Connected Rate : [BW-PhyMode-ShortGI-ldpc-STBC-MCS]= %-4s%-6s%-2d%-2d%-2d%-4d \n", ifIndex, get_bw_str(pEntry->HTPhyMode.field.BW),
				...
				DBGPRINT(RT_DEBUG_WARN, ("ApCli%d Connected WirelessMode : %-6s \n", ifIndex, get_phymode_str(pEntry->HTPhyMode.field.MODE)));
				DBGPRINT(RT_DEBUG_WARN, ("ApCli%d Connected BW : %-4s \n", ifIndex, get_bw_str(pEntry->MaxHTPhyMode.field.BW)));
				DBGPRINT(RT_DEBUG_WARN, ("ApCli%d Connected Channel : %d \n", ifIndex, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Channel));
				...
				if (Idx != BSS_NOT_FOUND)
				{
					...
					{
						INT32 i = 0;
						DBGPRINT(RT_DEBUG_ERROR, ("TPIE len:%d content:\n", pAd->ScanTab.BssEntry[Idx].TpIE.IELen));
						for(i=0; i < pAd->ScanTab.BssEntry[Idx].TpIE.IELen; i++)
							DBGPRINT(RT_DEBUG_TRACE, ("[%d]:0x%02X  ", i, pAd->ScanTab.BssEntry[Idx].TpIE.IE[i]));
					}

					DBGPRINT(RT_DEBUG_WARN, ("ApCli%d Connected TPIE : OUI[%02X-%02X-%02X],IE[%-128s] \n",
						ifIndex,
					...
					DBGPRINT(RT_DEBUG_WARN, ("ApCli%d Connected TPIE : NULL \n",ifIndex));
		}
#endif /* TPIE_SUPPORT */
    ...
    IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
    ...
    /* allocate memory */
    maxtblen = sizeof(MAC_STAT_ENTRY) * MaxWcidNum;
    ...
    NdisZeroMemory(pMacTab, maxtblen);
    for (i = 0; i < MaxWcidNum; i++)
    {
        ...
        if ((pEntry->Sst == SST_ASSOC) && (IS_ENTRY_APCLI(pEntry) || IS_ENTRY_CLIENT(pEntry)))
        {
            ...
            pMacTab[entry_num].AvgRssi0 = pEntry->RssiSample.AvgRssi0;
            pMacTab[entry_num].AvgRssi1 = pEntry->RssiSample.AvgRssi1;
            pMacTab[entry_num].AvgRssi2 = pEntry->RssiSample.AvgRssi2;
    ...
    {
        DBGPRINT(RT_DEBUG_TRACE, ("%s: Not enough user space for mac stat\n", __FUNCTION__));
        goto end;
    }
	...
	for (i=0; i<MaxWcidNum; i++)
	{
			...
			&& (pEntry->Sst == SST_ASSOC))
		{
            ...
            if(IS_ENTRY_APCLI(pEntry))
            {
                ...
                {
                    if (pAd->ApCfg.ApCliTab[pEntry->apidx].ApCliMlmeAux.bSupportMWDS)
                        printk("%-8s", "YES");
                    else
                        printk("%-8s", "NO");
                }
 	...
 	IN	PSTRING			arg)
{
	...
	if (pAd->ApCfg.bMACRepeaterEn)
 		...
 		&& (pAd->ApCfg.ApCliTab[ifIndex].SsidLen != 0))
 	{
 		...
 		for (i=0; i<MaxWcidNum; i++)
 		{
 			...
 			if ( IS_ENTRY_APCLI(pEntry)
				...
				&& (MAC_ADDR_EQUAL(pEntry->Addr, pApCliEntry->ApCliMlmeAux.Bssid))
 				...
 				{
 					DBGPRINT(RT_DEBUG_TRACE, ("ApCli%d Connected AP : %02X:%02X:%02X:%02X:%02X:%02X   SSID:%s   ProtStatus:%s\n",ifIndex,
 						pEntry->Addr[0], pEntry->Addr[1], pEntry->Addr[2],
 						pEntry->Addr[3], pEntry->Addr[4], pEntry->Addr[5],
					...
					bConnect=TRUE;
#if 1
					sprintf(arg+strlen(arg),"%-7d", pEntry->RssiSample.AvgRssi0);
					sprintf(arg+strlen(arg),"%-7d", pEntry->RssiSample.AvgRssi1);
					sprintf(arg+strlen(arg),"%-7d", pEntry->RssiSample.AvgRssi2);
					sprintf(arg+strlen(arg),"\n");

					sprintf(arg+strlen(arg), "ApCli%d Connected AP : %02X:%02X:%02X:%02X:%02X:%02X   SSID:%s   ProtStatus:%s\n",ifIndex,
 						pEntry->Addr[0], pEntry->Addr[1], pEntry->Addr[2],
 						pEntry->Addr[3], pEntry->Addr[4], pEntry->Addr[5],
 						pAd->ApCfg.ApCliTab[ifIndex].Ssid,
						pEntry->PortSecured == WPA_802_1X_PORT_SECURED ? "Secured" : "NOT Secured");
#endif
#ifdef MWDS
                        if(pAd->ApCfg.ApCliTab[ifIndex].ApCliMlmeAux.bSupportMWDS)
                            DBGPRINT(RT_DEBUG_TRACE, ("MWDSCap : YES\n"));
                        else
                            DBGPRINT(RT_DEBUG_TRACE, ("MWDSCap : NO\n"));
#endif /* MWDS */
		...
		if (!bConnect)
		{
			DBGPRINT(RT_DEBUG_TRACE, ("ApCli%d Connected AP : Disconnect\n",ifIndex));
