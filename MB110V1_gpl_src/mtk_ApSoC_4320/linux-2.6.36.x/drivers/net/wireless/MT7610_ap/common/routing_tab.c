	...
	IN UINT32 Flag)
{
	...
	if (pMbss->pRoutingEntryPool)
	{
		...
		for (i = 0; i < ROUTING_POOL_SIZE; i++)
			insertTailList(&pMbss->RoutingEntryFreeList, (LIST_ENTRY *)(pMbss->pRoutingEntryPool+i));
    ...
    for (i = 0; i < ROUTING_HASH_TAB_SIZE; i++)
    {
    	...
    	while(pRoutingEntry)
    	{
    		...
    		if(pRoutingEntry->Valid && ROUTING_ENTRY_TEST_FLAG(pRoutingEntry, Flag))
    		{
                ...
                {
    			    delEntryList(&pMbss->RoutingTab[i], (LIST_ENTRY *)pRoutingEntry);
    			    NdisZeroMemory(pRoutingEntry, sizeof(ROUTING_ENTRY));
    			    insertTailList(&pMbss->RoutingEntryFreeList, (LIST_ENTRY *)pRoutingEntry);
                }
    		}
	...
	IN UINT32 Flag)
{
	...
	while (pRoutingEntry)
	{
		if (pRoutingEntry->Valid && MAC_ADDR_EQUAL(pMac, pRoutingEntry->Mac))
		{
            ...
            if(!IS_ROUTING_ENTRY(pRoutingEntry))
            {
                delEntryList(&pMbss->RoutingTab[HashId], (LIST_ENTRY *)pRoutingEntry);
			    NdisZeroMemory(pRoutingEntry, sizeof(ROUTING_ENTRY));
			    insertTailList(&pMbss->RoutingEntryFreeList, (LIST_ENTRY *)pRoutingEntry);
            }
			break;
	...
	IN PROUTING_ENTRY pRoutingEntry)
{
    ...
    RTMP_SEM_LOCK(&pMbss->RoutingTabLock);
    insertTailList(&pMbss->RoutingTab[HashId], (LIST_ENTRY *)pRoutingEntry);
    RTMP_SEM_UNLOCK(&pMbss->RoutingTabLock);
}
    ...
    IN UINT32 SrcIP)
{
	...
	pDev = pAd->ApCfg.MBSSID[ifIndex].MSSIDDev;

	if (pDev == NULL)
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s: (%d)NET DEVICE is NULL!\n", __FUNCTION__, ifIndex));
		return FALSE;
	}

    skb = arp_create(ARPOP_REQUEST, ETH_P_ARP, pRoutingEntry->IPAddr, pDev,
