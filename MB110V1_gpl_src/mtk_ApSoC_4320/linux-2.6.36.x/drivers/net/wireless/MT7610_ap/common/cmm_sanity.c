...
#ifdef TP_IE_SUPPORT
                else if (NdisEqualMemory(pEid->Octet, TP_OUI, 3))
                {
                    /* Copy to pVIE which will report to bssid list.*/
                    Ptr = (PUCHAR) pVIE;
                    NdisMoveMemory(Ptr + *LengthVIE, &pEid->Eid, pEid->Len + 2);
...
SanityCheck:
	...
	if (Sanity != 0x7)
	{
		if (((bFromBeaconReport == FALSE) && (Sanity != 0x7))
			||((bFromBeaconReport == TRUE) && (!(Sanity & 0x1))))
		{
...
#ifdef TP_IE_SUPPORT
                if (bVendor != NULL && (pAd->ApCfg.ap_customer_oui.pointer!= NULL) && (eid_len >= pAd->ApCfg.ap_customer_oui.length) &&
                        NdisEqualMemory(eid_data, pAd->ApCfg.ap_customer_oui.pointer, pAd->ApCfg.ap_customer_oui.length))
                {
                    bVendor->element_id = IE_VENDOR_SPECIFIC;
                    bVendor->len = eid_len;
                    NdisMoveMemory(bVendor->custom_ie, eid_data, eid_len);
                    break;
                }
#endif /* TP_IE_SUPPORT */
