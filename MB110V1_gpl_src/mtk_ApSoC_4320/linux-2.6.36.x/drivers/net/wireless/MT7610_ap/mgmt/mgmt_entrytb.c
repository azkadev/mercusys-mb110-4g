...
#ifdef MWDS
			pEntry->wdev = &(pAd->ApCfg.MBSSID[apidx].wdev);
			pEntry->wcid = i;
            pEntry->func_tb_idx = pAd->ApCfg.MBSSID[apidx].wdev.func_idx; //add qun
#endif
					...
					IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
					{
...
#ifdef TP_NETWORK_ROAMING
						{
							if ( APID_USED_FOR_ROAMING == apidx )
							{
								...
								mtk_deliver_band_steering_event(RtmpOsGetNetIfIndex(pAd->ApCfg.MBSSID[apidx].MSSIDDev), ATH_EVENT_BSTEERING_STA_NUM_REPORT, sizeof(num_status), (u_int8_t*)&num_status);
							}
						}
#endif
						break;
