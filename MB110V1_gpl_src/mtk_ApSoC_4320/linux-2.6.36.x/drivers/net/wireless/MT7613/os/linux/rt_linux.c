...
void  printLevel(int Category,int SubCategory, int Level, const char *fmt, ...)
{
	...
	if ((0x1 << Category) & (DebugCategory))
	{
		...
		{
			va_start(args, fmt);
            length = vsnprintf(buffer, sizeof(buffer) - 1, fmt, args);
            va_end(args);
            MTWF_PRINT(buffer);
			 if (length >= sizeof(buffer) - 2)
	        {
	            printk("\n\nMT7613-->!!!  log buffer may be not enough (%d < %d)  !!! \n\n", sizeof(buffer), length);
	        }
		}
	}
}
...
VOID RtmpOsPktRcvHandle(PNDIS_PACKET pNetPkt)
{
	...
	//MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s(): f_shuai \n", __FUNCTION__));
	{

		PUCHAR pPktHdr,pLayerHdr;
		pPktHdr = GET_OS_PKT_DATAPTR(pNetPkt);
		pLayerHdr = (pPktHdr);


		if((*(pLayerHdr + 9) == 0x01))//ICMP:0x01
		{
			MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(): f_shuai It's a ICMP packet\n", __FUNCTION__));
			MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): f_shuai source ip=%d.%d.%d.%d\n",__FUNCTION__,pLayerHdr[12],pLayerHdr[13],pLayerHdr[14],pLayerHdr[15]));
			MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(): f_shuai target ip=%d.%d.%d.%d\n",__FUNCTION__,pLayerHdr[16],pLayerHdr[17],pLayerHdr[18],pLayerHdr[19]));
			if((*(pLayerHdr + 20) == 0x08))
				MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): f_shuai It's a Echo Request\n", __FUNCTION__));
			if((*(pLayerHdr + 20) == 0x0))
				MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): f_shuai It's a Echo Reply\n", __FUNCTION__));
		}
		if((*(pLayerHdr + 9) == 0x11))//UDP:17,0x11
		{
			PUCHAR pUdpHdr;
			UINT16 srcPort,dstPort;

			pUdpHdr =pLayerHdr+20;
			srcPort = OS_NTOHS(get_unaligned((PUINT16)(pUdpHdr)));//srcPort=68
			dstPort = OS_NTOHS(get_unaligned((PUINT16)(pUdpHdr+2)));

			//DBGPRINT(RT_DEBUG_TRACE, ("%s(): f_shuai srcPort=%d\n", __FUNCTION__,srcPort));
			//DBGPRINT(RT_DEBUG_TRACE, ("%s(): f_shuai dstPort=%d\n", __FUNCTION__,dstPort));
			/*It's a DHCP packet */
			if((srcPort ==67 && dstPort ==68) || (srcPort ==68 && dstPort ==67)){
				//MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s(): f_shuai It's a DHCP packet\n", __FUNCTION__));
				MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(): f_shuai It's a DHCP packet STA MAC =%x-%x-%x-%x-%x-%x\n", __FUNCTION__,pUdpHdr[36],pUdpHdr[37],pUdpHdr[38],pUdpHdr[39],pUdpHdr[40],pUdpHdr[41]));

				if((*(pUdpHdr + 250) == 0x01))
					MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(): f_shuai It's a DHCP Discover packet\n", __FUNCTION__));

				if((*(pUdpHdr + 250) == 0x02))
					MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(): f_shuai It's a DHCP Offer packet\n", __FUNCTION__));

				if((*(pUdpHdr + 250) == 0x03))
					MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(): f_shuai It's a DHCP Request packet\n", __FUNCTION__));

				if((*(pUdpHdr + 250) == 0x05))
					MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(): f_shuai It's a DHCP ACK packet\n", __FUNCTION__));
			}
		}
	}
#endif
