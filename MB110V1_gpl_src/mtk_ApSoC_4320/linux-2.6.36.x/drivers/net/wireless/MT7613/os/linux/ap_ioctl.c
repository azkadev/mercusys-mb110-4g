...
struct iw_priv_args ap_privtab[] = {
	...
	/* conflict with RTPRIV_IOCTL_GET_CHANNEL_LIST */
/*	{
...
INT rt28xx_ap_ioctl(void *net_dev_obj, void *data_obj, int cmd) /* snowpin for ap/sta */
{
...
#ifdef APSD_SUPPORT
		subcmd = wrq->u.data.flags = wrqin->u.data.flags;
		RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_GET_MAC_TABLE, subcmd, NULL, 0);
#else
	...
	/* conflict with RTPRIV_IOCTL_GRATEINFO */
	...
	case RTPRIV_IOCTL_GSITESURVEY:
		...
		break;
#ifdef AWND_SUPPORT
	case RTPRIV_IOCTL_GSITESURVEY_DECO:
		RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_GSITESURVEY_DECO, 0, NULL, 0);
		break;

    case RTPRIV_IOCTL_GET_CONFIG_GENERIC:
        RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_GET_CONFIG_GENERIC, 0, NULL, 0);
        break;
#endif /* AWND_SUPPORT */
...
/* added for apsd by jiangzheyu 2019/3/5 */
	...
	case RTPRIV_IOCTL_CHAN_UTILIZATION:
	{
		RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_CHAN_UTILIZATION, 0, wrq->u.data.pointer, 0);
	}
		break;
#endif
	case RTPRIV_IOCTL_APC_DISCONNECT:
	{
		RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_APC_DISCONNECT, 0, wrq->u.data.pointer, 0);
	}
		break;
#endif /* APSD_SUPPORT */
	...
	case RTPRIV_IOCTL_GET_STA_INFO:
		RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_GET_STA_INFO, 0, NULL, 0);
		break;
	default:
