...
#if defined(CONFIG_AP_SUPPORT) && defined(MWDS)
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		/* TP-link feature: bmcast arp packet use 3addr format in mwds mode */
		/* if recived mwds's entry's packet, we not do MAT, but the arp sender mac is not same with 802.3's SA,
		    so, we change 802.3's SA equal to arp sender mac */
		{
			PUCHAR	    sendMac = NULL, arpHdr = NULL;
			PUCHAR 		pPktHdr = GET_OS_PKT_DATAPTR(pRxPkt);
			UINT16		protoType, arpOpcode;

			if (pPktHdr && GET_OS_PKT_LEN(pRxPkt) >= (MAT_ETHER_HDR_LEN + 8 + MAC_ADDR_LEN))
			{
				/* Get the upper layer protocol type of this 802.3 pkt */
				arpHdr    = pPktHdr + MAT_ETHER_HDR_LEN;
				protoType = OS_NTOHS(get_unaligned((PUINT16)(pPktHdr + 12)));
				arpOpcode = OS_NTOHS(get_unaligned((PUINT16)(arpHdr + 6)));
				if(protoType == ETH_P_ARP && arpOpcode == 1 && isBcastEtherAddr(pPktHdr))
				{
					sendMac = arpHdr + 8;
					NdisMoveMemory((pPktHdr+MAC_ADDR_LEN), sendMac, MAC_ADDR_LEN);
					#if 0
					printk("==> %s %d arp packet hanlder trace, sendMac = %02x-%02x-%02x-%02x-%02x-%02x packet_len=%d\n",
					         __FUNCTION__, __LINE__,PRINT_MAC(sendMac),GET_OS_PKT_LEN(pRxPkt));
					#endif
				}
			}
		}
