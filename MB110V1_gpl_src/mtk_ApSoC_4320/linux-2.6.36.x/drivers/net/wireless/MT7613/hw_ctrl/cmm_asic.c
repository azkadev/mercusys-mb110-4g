...
BOOLEAN AsicMcastEntryInsert(RTMP_ADAPTER *pAd, PMULTICAST_GROUP_ID pGrpId, UINT8 BssIdx, UINT8 Type, PUCHAR MemberAddr,
							 PNET_DEV dev, UINT8 WlanIndex)
{
	...
	if (arch_ops->archMcastEntryInsert)
		Ret = arch_ops->archMcastEntryInsert(pAd, pGrpId, BssIdx, Type, MemberAddr, dev, WlanIndex);

	return Ret;
}

BOOLEAN AsicMcastEntryDelete(RTMP_ADAPTER *pAd, PMULTICAST_GROUP_ID pGrpId, UINT8 BssIdx, PUCHAR MemberAddr, PNET_DEV dev,
							 UINT8 WlanIndex)
{
	INT32 Ret = 0;
	struct _RTMP_ARCH_OP *arch_ops = hc_get_arch_ops(pAd->hdev_ctrl);
	...
	if (arch_ops->archMcastEntryDelete)
		Ret = arch_ops->archMcastEntryDelete(pAd, pGrpId, BssIdx, MemberAddr, dev, WlanIndex);
