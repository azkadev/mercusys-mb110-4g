...
#ifdef AWND_SUPPORT
			if (!pApCliEntry->ApcliInfStat.AwndEnableAutoConnect)
			{
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_OFF,
							 ("ApCliIfUp(5G) - Stop auto connecting while AWND disallow it!\n"));
				continue;
			}
#endif

			MTWF_LOG(DBG_CAT_ALL, CATCLIENT_APCLI, DBG_LVL_TRACE, ("(%s) ApClii interface[%d] startup.\n", __func__, ifIndex));
#ifdef WSC_INCLUDED
			if ((pApCliEntry->wdev.WscControl.bWscTrigger) &&
	...
	for (entryIdx = 0; entryIdx < pScanTab->BssNr; entryIdx++) {
		...
		if (NdisEqualMemory(pCfgSsid, pBssEntry->Ssid, CfgSsidLen) &&
			...
			if (((((wdev->SecConfig.AKMMap & pBssEntry->AKMMap) != 0) ||
				...
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE,
						 ("I/F(apclii%d) ApCliAutoConnectExec:(Len=%d,Ssid=%s, Channel=%d, Rssi=%d)\n",
						  ifIdx, pBssEntry->SsidLen, pBssEntry->Ssid,
						  pBssEntry->Channel, pBssEntry->Rssi));
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE,
						 ("I/F(apclii%d) ApCliAutoConnectExec::(AuthMode=%s, EncrypType=%s)\n", ifIdx,
