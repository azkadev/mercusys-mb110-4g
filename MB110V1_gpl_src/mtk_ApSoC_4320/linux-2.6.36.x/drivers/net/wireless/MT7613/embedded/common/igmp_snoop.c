	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < MAX_LEN_OF_MULTICAST_FILTER_TABLE; i++) {
		...
		if (pMulticastFilterTable->Content[i].Valid == TRUE) {
			...
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_IGMP, DBG_LVL_OFF, ("\n==>groupId(%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x)\n",
				pEntry->groupId.data[0], pEntry->groupId.data[1], pEntry->groupId.data[2], pEntry->groupId.data[3],
				pEntry->groupId.data[4], pEntry->groupId.data[5], pEntry->groupId.data[6], pEntry->groupId.data[7],
				pEntry->groupId.data[8], pEntry->groupId.data[8], pEntry->groupId.data[10], pEntry->groupId.data[11],
				pEntry->groupId.data[12], pEntry->groupId.data[13]));
	...
	if (isIgmpPkt(pDstMacAddr, pIpHeader)) {
		...
		case IGMP_V2_MEMBERSHIP_REPORT: /* IGMP version 2 membership report. */
			...
			if ( !isDeniedGroup(pGroupIpAddr) ) 
			{
				...
				AsicMcastEntryInsert(pAd, &groupId, wdev->bss_info_argument.ucBssIndex, MCAT_FILTER_DYNAMIC, pSrcMacAddr, wdev->if_dev, Wcid);
			}
			break;
		...
		case IGMP_LEAVE_GROUP: /* IGMP version 1 and version 2 leave group. */
			pGroupIpAddr = (PUCHAR)(pIgmpHeader + 4);
			getGroupIDFromIPAddr(pGroupIpAddr, &groupId, ETH_P_IP);
			AsicMcastEntryDelete(pAd, &groupId, wdev->bss_info_argument.ucBssIndex, pSrcMacAddr, wdev->if_dev, Wcid);
			break;
	...
	for (i = 0; i < MAX_LEN_OF_MULTICAST_FILTER_TABLE; i++) {
		...
		if (pEntry->Valid == TRUE) {
			...
			if ((pEntry->type == MCAT_FILTER_DYNAMIC)
				...
				AsicMcastEntryDelete(pAd, &(pEntry->groupId), wdev->bss_info_argument.ucBssIndex, pMemberAddr, wdev->if_dev, Wcid);
		}
	}
}
	...
	OUT PMULTICAST_FILTER_TABLE_ENTRY *ppGroupEntry)
{
	if (IS_MULTICAST_MAC_ADDR(pSrcBufVA)) {
		...
		*ppGroupEntry = MulticastFilterTableLookup(pAd->pMulticastFilterTable, &groupId,
									wdev->if_dev);
		if (IgmpMldPkt) {
			*ppGroupEntry = NULL;
