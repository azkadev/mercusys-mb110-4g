...
VOID MacTableResetNonMapWdev(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	if ((IS_MAP_TURNKEY_ENABLE(pAd)) &&
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(%d): MacTableDeleteEntry %02x:%02x:%02x:%02x:%02x:%02x\n",
				__func__, __LINE__, PRINT_MAC(pMacEntry->Addr)));
...
INT	rtmp_set_channel(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR Channel)
{
	...
	if (Success == TRUE)
	{
		...
		send_event_to_network_roaming(pAd->net_dev, IWEVCUSTOM, IEEE80211_EV_CHAN_CHANGE, &Channel);
...
/* modified foe apsd by JiangZheyu 2019/3/5 */
#define	MAC_LINE_LEN	(1+14+4+4+4+4+10+10+10+6+6)	/* "\n"+Addr+aid+psm+datatime+rxbyte+txbyte+current tx rate+last tx rate+ actual rate+"\n" */
...
/*			pDst->TxPER = pEntry->SaveTxPER; */
	...
	for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	{
		...
		if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC))
		{
			...
			sprintf(msg+strlen(msg),"%-6lu",pEntry->actualRate); /* Obtain actual data rate */
			sprintf(msg+strlen(msg),"%-10d",0/*pAd->MacTab.Content[i].HSCounter.LastDataPacketTime*/); /* ToDo*/
			sprintf(msg+strlen(msg),"%-10d",0/*pAd->MacTab.Content[i].HSCounter.TotalRxByteCount*/); /* ToDo*/
			sprintf(msg+strlen(msg),"%-10d",0/*pAd->MacTab.Content[i].HSCounter.TotalTxByteCount*/); /* ToDo*/
			//sprintf(msg+strlen(msg),"%-10u", (UINT)pEntry->RxCount);
			//sprintf(msg+strlen(msg),"%-10u", (UINT)pEntry->DebugTxCount);
			sprintf(msg+strlen(msg),"%s","\n");
			idx++;
		}
	}
	...
	wrq->u.data.length = idx * MAC_LINE_LEN;
	if (copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: copy_to_user() fail\n", __FUNCTION__));
	}
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	{
		pEntry = &(pAd->MacTab.Content[i]);
		if ((pEntry->Sst == SST_ASSOC) && (IS_ENTRY_APCLI(pEntry) || IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_PEER_AP(pEntry)))
		{
			...
			pMacTab[entry_num].IsApCli = (pEntry->EntryType == ENTRY_AP);
...
/* added for apsd by jiangzheyu 2019/3/29 */
#ifdef APSD_SUPPORT
...
#endif
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
							 ("ApClii%d   Connected AP : %02X:%02X:%02X:%02X:%02X:%02X   SSID:%s, avgrssi: %-7d, %-7d, %-7d\n",
		...
		if (!bConnect)
		{
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApClii%d Connected AP : Disconnect\n", ifIndex));
#ifdef APSD_SUPPORT
