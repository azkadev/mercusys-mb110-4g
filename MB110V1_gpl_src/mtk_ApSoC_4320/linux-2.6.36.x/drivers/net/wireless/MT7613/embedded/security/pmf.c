	...
	IN PVOID SystemSpecific3)
{
	...
	if (pEntry) {
		...
		if (IS_ENTRY_PEER_AP(pEntry)) {
			...
			cntl_disconnect_request(pEntry->wdev, CNTL_DISASSOC, pStaCfg->Bssid, REASON_DISASSOC_STA_LEAVING, NULL);
		} else
#endif /* CONFIG_STA_SUPPORT */
		{
#ifdef CONFIG_AP_SUPPORT
	...
	IN RTMP_STRING *arg)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if (os_str_tol(arg, 0, 10))
			pAd->StaCfg[pObj->ioctl_if].wdev.SecConfig.PmfCfg.Desired_MFPC = TRUE;
		else
			pAd->StaCfg[pObj->ioctl_if].wdev.SecConfig.PmfCfg.Desired_MFPC = FALSE;

		MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR, ("[PMF]%s:: staidx=%d, Desired MFPC=%d\n", __func__
				 , pObj->ioctl_if, pAd->StaCfg[pObj->ioctl_if].wdev.SecConfig.PmfCfg.Desired_MFPC));
	}
#endif /* CONFIG_STA_SUPPORT */
	return TRUE;
}
	...
	IN RTMP_STRING *arg)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if (os_str_tol(arg, 0, 10))
			pAd->StaCfg[pObj->ioctl_if].wdev.SecConfig.PmfCfg.Desired_MFPR = TRUE;
		else
			pAd->StaCfg[pObj->ioctl_if].wdev.SecConfig.PmfCfg.Desired_MFPR = FALSE;

		MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR, ("[PMF]%s:: staidx=%d, Desired MFPR=%d\n", __func__
				 , pObj->ioctl_if, pAd->StaCfg[pObj->ioctl_if].wdev.SecConfig.PmfCfg.Desired_MFPR));
	}
#endif /* CONFIG_STA_SUPPORT */
	return TRUE;
}
	...
	IN RTMP_STRING *arg)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if (os_str_tol(arg, 0, 10))
			pAd->StaCfg[pObj->ioctl_if].wdev.SecConfig.PmfCfg.Desired_PMFSHA256 = TRUE;
		else
			pAd->StaCfg[pObj->ioctl_if].wdev.SecConfig.PmfCfg.Desired_PMFSHA256 = FALSE;

		MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_ERROR, ("[PMF]%s:: staidx=%d, Desired PMFSHA256=%d\n", __func__
				 , pObj->ioctl_if, pAd->StaCfg[pObj->ioctl_if].wdev.SecConfig.PmfCfg.Desired_PMFSHA256));
	}
#endif /* CONFIG_STA_SUPPORT */
	return TRUE;
}
	...
	IN RTMP_STRING *arg)
{
	...
	pObj = (POS_COOKIE) pAd->OS_Cookie;
	pPmfCfg = &pAd->StaCfg[pObj->ioctl_if].wdev.SecConfig.PmfCfg;
