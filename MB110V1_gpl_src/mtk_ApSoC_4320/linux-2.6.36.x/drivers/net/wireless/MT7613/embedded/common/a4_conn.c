...
)
{
	...
	if (a4_entry) {
		...
		a4_entry->valid = 1;
		a4_entry->wcid = wcid;
#ifdef MWDS_ENTRY_HAVE_ADDR
		PMAC_TABLE_ENTRY pEntry = &adapter->MacTab.Content[wcid];
		NdisCopyMemory(a4_entry->Addr, pEntry->Addr, MAC_ADDR_LEN);
#endif
...
#ifdef MWDS_ENTRY_HAVE_ADDR
	pMbss = &adapter->ApCfg.MBSSID[if_index];
	a4_entry_list = &pMbss->a4_entry_list;
	RTMP_SEM_LOCK(&pMbss->a4_entry_lock);
	DlListForEach(a4_entry, a4_entry_list, A4_CONNECT_ENTRY, List)
	{
		if (a4_entry && a4_entry->valid && VALID_WCID(a4_entry->wcid)) {
			pEntry = &adapter->MacTab.Content[a4_entry->wcid];
			if (!MAC_ADDR_EQUAL(a4_entry->Addr, pEntry->Addr)) {
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					("%s(%d): wcid: %d, A4ConnEntry mac:%02x-%02x-%02x-%02x-%02x-%02x,	\
					MacTable mac:%02x-%02x-%02x-%02x-%02x-%02x, doesn't match! Age out!\n",
					__func__, __LINE__, a4_entry->wcid, PRINT_MAC(a4_entry->Addr), PRINT_MAC(pEntry->Addr)));

				//SET_ENTRY_A4(pEntry, A4_TYPE_NONE);  //should not change MacTable entry
				DlListDel(&a4_entry->List);
				os_free_mem(a4_entry);
				RoutingTabSetAllFree(adapter, if_index, a4_entry->wcid, ROUTING_ENTRY_A4);
				break;
			}
		}
	}
	RTMP_SEM_UNLOCK(&pMbss->a4_entry_lock);
#endif
...
#ifdef MWDS_ENTRY_HAVE_ADDR
				if (wdev->wdev_idx != if_index)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
							("%s(%d): tx wdev index is %d, A4 Entry wdev index is %d, doesn't match! Don't Send!\n",
							__func__, __LINE__, if_index, wdev->wdev_idx));
					continue;
				}
#endif
...
)
{
	...
	if (IS_ENTRY_A4(entry) == FALSE) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,
				("a4_ap_peer_enable Enabled A4 for entry:%02x-%02x-%02x-%02x-%02x-%02x\n",
		...
		if (NDIS_STATUS_SUCCESS != CmdExtWtblUpdate(adapter,
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("a4_ap_peer_enable SET_A4_ENTRY OK!\n"));
	...
	if (type > GET_ENTRY_A4(entry)) {
		...
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("a4_ap_peer_enable SET_A4_ENTRY type:%d OK!\n", type));
	}
...
)
{
	...
	if (IS_ENTRY_A4(entry) == FALSE) {
		...
		if (CmdExtWtblUpdate(adapter,
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("a4_apcli_peer_enable SET_A4_ENTRY OK!\n"));
		else
			return FALSE;
	}

	if (type > GET_ENTRY_A4(entry)) {
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("a4_apcli_peer_enable SET_A4_ENTRY type:%d OK!\n", type));
	...
	DlListForEach(a4_entry, a4_entry_list, A4_CONNECT_ENTRY, List) {
		if (a4_entry && a4_entry->valid && VALID_WCID(a4_entry->wcid)) {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					("Wcid:%d, Proxy Mac: %02X:%02X:%02X:%02X:%02X:%02X\n",
					a4_entry->wcid, PRINT_MAC(proxy_mac_addr)));
