...
#ifdef TP_NETWORK_ROAMING
				{
					if ( APID_USED_FOR_ROAMING == pEntry->func_tb_idx )
					{
						bs_current_sta_num_status num_status;
						memset(&num_status, 0, sizeof(num_status));
						num_status.max_sta_num = pAd->ApCfg.MBSSID[pEntry->func_tb_idx].MaxStaNum;
						num_status.current_sta_num = pAd->ApCfg.MBSSID[pEntry->func_tb_idx].StaCount;
	...
	if (pEntry && !IS_ENTRY_NONE(pEntry)) {
		...
		if (MAC_ADDR_EQUAL(pEntry->Addr, pAddr)) {
			...
			if (IS_ENTRY_CLIENT(pEntry)
...
#ifdef TP_NETWORK_ROAMING
                {
                    u_int8_t apidx = pEntry->func_tb_idx;
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d: call RtmpOSWrielessEventSendExt to send msg RT_WLAN_EVENT_EXPIRED\r\n", __FUNCTION__, __LINE__));
