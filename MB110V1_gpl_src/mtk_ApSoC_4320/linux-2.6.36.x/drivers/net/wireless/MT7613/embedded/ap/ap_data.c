	...
	if (tr_entry->EntryType != ENTRY_CAT_MCAST)
		...
		if (wdev->IgmpSnoopEnable) {
			...
			if (((InIgmpGroup == IGMP_IN_GROUP)
				 ...
				 && (!isInvalidM2UIP(pAd, pkt_va + 14))) /* Modified by Zeng Yi.2012-11-12. */
			{
...
INT ap_legacy_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, TX_BLK *pTxBlk)
{
	...
	if(RTMP_GET_PACKET_DHCP(pTxBlk->pPacket))
		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s __debug__ RTMP_GET_PACKET_DHCP It is DHCP pkt\n",__FUNCTION__));

	if(RTMP_GET_PACKET_PING(pTxBlk->pPacket))
		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s __debug__ RTMP_GET_PACKET_PING is Ping pkt\n",__FUNCTION__));

	if(RTMP_GET_PACKET_ARP(pTxBlk->pPacket))
		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s __debug__ RTMP_GET_PACKET_ARP is ARP pkt\n",__FUNCTION__));
#endif
...
INT ap_tx_pkt_handle(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, TX_BLK *pTxBlk)
{
	...
	//MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s __debug__ pTxBlk->TxFrameType =%d\n",__FUNCTION__,pTxBlk->TxFrameType));
#endif
...
VOID ap_cls3_err_action(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	if (pEntry) {
		/*ApLogEvent(pAd, pAddr, EVENT_DISASSOCIATED); */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(%d): mac_entry_delete %02x:%02x:%02x:%02x:%02x:%02x\n",
...
#if 0
//2019.12.11 mjz
// do not return false here to be compatible with TP-Link RE which sends some 3-address packets in 4-address mode
#ifdef MWDS
...
INT ap_rx_pkt_foward(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, PNDIS_PACKET pPacket)
{
	...
	if (pHeader802_3[0] & 0x01) {
		if ((pMbss->StaCount > 1)  && pMbss->IsolateInterStaTraffic == 0
		   ) {
...
INT ap_ieee_802_3_data_rx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, RX_BLK *pRxBlk, MAC_TABLE_ENTRY *pEntry)
{
	...
	if (rx_chk_amsdu_invalid_frame(pAd, pRxBlk, wdev) == NDIS_STATUS_FAILURE) {
		MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): invalid amsdu frame drop it!\n", __func__));
		return FALSE;
	}
...
#ifdef TX_FAULT_DEBUG
//	MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s __debug__ pRxBlk->Addr1 %x-%x-%x-%x-%x-%x\n",__FUNCTION__,PRINT_MAC(pRxBlk->Addr1)));
	MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s __debug__ STA MAC pRxBlk->Addr2 %x-%x-%x-%x-%x-%x\n",__FUNCTION__,PRINT_MAC(pRxBlk->Addr2)));
//	MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s __debug__ pRxBlk->Addr3 %x-%x-%x-%x-%x-%x\n",__FUNCTION__,PRINT_MAC(pRxBlk->Addr3)));
#endif
	...
	*/
	...
	if (rx_chk_amsdu_invalid_frame(pAd, pRxBlk, wdev) == NDIS_STATUS_FAILURE) {
		MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): invalid amsdu frame drop it!\n", __func__));
		return FALSE;
	}
...
BOOLEAN ap_dev_rx_mgmt_frm(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, MAC_TABLE_ENTRY *pEntry)
{
	...
	*/
	...
	/* Signal in MLME_QUEUE isn't used, therefore take this item to save min SNR. */
	{
		...
		if (recv_wdev == NULL)
		{
			struct wifi_dev *wdev0 =  pAd->wdev_list[0];
			struct wifi_dev *wdev2 =  pAd->wdev_list[2];
			recv_wdev = wdev0->DevInfo.WdevActive ? wdev0 : wdev2;
		}
