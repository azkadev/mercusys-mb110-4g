...
static INT ge_enq_dataq_pkt(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, PNDIS_PACKET pkt, UCHAR q_idx)
{
	...
	if(RTMP_GET_PACKET_DHCP_DISCOVER(pkt))
		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s __debug__ RTMP_GET_PACKET_DHCP_DISCOVER\n",__FUNCTION__));
	if(RTMP_GET_PACKET_DHCP_OFFER(pkt))
		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s __debug__ RTMP_GET_PACKET_DHCP_OFFER\n",__FUNCTION__));
	if(RTMP_GET_PACKET_DHCP_REQ(pkt))
		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s __debug__ RTMP_GET_PACKET_DHCP_REQ\n",__FUNCTION__));
	if(RTMP_GET_PACKET_DHCP_ACK(pkt))
		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s __debug__ RTMP_GET_PACKET_DHCP_ACK\n",__FUNCTION__));

	if(RTMP_GET_PACKET_PING_REQ(pkt))
		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s __debug__ RTMP_GET_PACKET_PING_REQ\n",__FUNCTION__));
	if(RTMP_GET_PACKET_PING_RSP(pkt))
		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s __debug__ RTMP_GET_PACKET_PING_RSP\n",__FUNCTION__));

	if(RTMP_GET_PACKET_PING(pkt))
...
#ifdef TX_FAULT_DEBUG
		//MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s __debug__ pTxBlk->resource_idx =%d KickRingBitMap =%d\n",__FUNCTION__,pTxBlk->resource_idx,KickRingBitMap));

		if(RTMP_GET_PACKET_DHCP(pTxBlk->pPacket))
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s __debug__ RTMP_GET_PACKET_DHCP It is DHCP pkt\n",__FUNCTION__));
		if(RTMP_GET_PACKET_DHCP_DISCOVER(pTxBlk->pPacket))
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s __debug__ RTMP_GET_PACKET_DHCP_DISCOVER\n",__FUNCTION__));
		if(RTMP_GET_PACKET_DHCP_OFFER(pTxBlk->pPacket))
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s __debug__ RTMP_GET_PACKET_DHCP_OFFER\n",__FUNCTION__));
		if(RTMP_GET_PACKET_DHCP_REQ(pTxBlk->pPacket))
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s __debug__ RTMP_GET_PACKET_DHCP_REQ\n",__FUNCTION__));
		if(RTMP_GET_PACKET_DHCP_ACK(pTxBlk->pPacket))
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s __debug__ RTMP_GET_PACKET_DHCP_ACK\n",__FUNCTION__));

		if(RTMP_GET_PACKET_PING_REQ(pTxBlk->pPacket))
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s __debug__ RTMP_GET_PACKET_PING_REQ\n",__FUNCTION__));
		if(RTMP_GET_PACKET_PING_RSP(pTxBlk->pPacket))
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s __debug__ RTMP_GET_PACKET_PING_RSP\n",__FUNCTION__));
...
#ifdef TX_FAULT_DEBUG
			MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s __debug__ asic_kickout_data_tx\n",__FUNCTION__));
#endif
