						  ...
						  IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (!IsFound) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Not found peer entry in list\n"));
		/*return;*/
	}
	else
	{
	...
	if ((NetDev== NULL) || ((NetDev!= NULL) && !(RTMP_OS_NETDEV_STATE_RUNNING(NetDev))))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s netDev is Down\n", __func__));
	...
	RTMP_STRING *buffer)
{
	INT loop;
	RTMP_STRING *macptr;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MT7613->wifi call wnm read paramters file\n"));
	...
	*p_btm_req_ie_len = TmpLen;
	...
	for (i = 0; i < p_btm_req_data->num_candidates; i++, p_info++) {
...

		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:%d TmpLen=%d\n",  __FUNCTION__, __LINE__, TmpLen));
		}
