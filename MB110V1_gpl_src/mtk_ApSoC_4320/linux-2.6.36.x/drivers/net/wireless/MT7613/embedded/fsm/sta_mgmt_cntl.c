...
static VOID join_iterate_by_cfg(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	//[lixiangkui start] channel auto switch
	if (pApCliEntry->ApcliInfStat.ProbeReqCnt == 7) {
		//BOOLEAN Cancelled;
		POS_COOKIE pObj= (POS_COOKIE) pAd->OS_Cookie;

		RtmpOSNetDevProtect(1);
		old_if = pObj->ioctl_if;
		pObj->ioctl_if = ifIndex;
		
		old_if_type= pObj->ioctl_if_type;
		pObj->ioctl_if_type = INT_APCLI;

		Set_ApCli_AutoConnect_Proc(pAd, "2");	//restart partial scan RootAp 

		pObj->ioctl_if = old_if;
...
INT LinkDown_Infra(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, BOOLEAN ReqByAP, BOOLEAN has_inter_sm_data, MLME_QUEUE_ELEM *Elem)
{
	PSTA_ADMIN_CONFIG pStaCfg = GetStaCfgByWdev(pAd, wdev);
#ifdef MWDS
	PSTA_ADMIN_CONFIG pApCliMWDSEntry = &pAd->StaCfg[MAIN_MBSSID];
#endif
...
#ifdef MWDS
			MWDSAPCliPeerDisable(pAd, pApCliMWDSEntry, &pAd->MacTab.Content[pStaCfg->MacTabWCID]);
#endif /* MWDS */
#ifdef CONFIG_MAP_SUPPORT
