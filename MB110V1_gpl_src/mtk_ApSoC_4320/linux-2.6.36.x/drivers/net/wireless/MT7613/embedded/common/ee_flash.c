...
static int raspi_flash_read(u_char *buf, u32 from, u32 len)
{
		/* read all flash data into memory */
	int ret = 0;
	int index = 0;
	u_char *partition_buf = NULL;
    
	/* read all eeprom data in memory into flash. */
    	if (!((from >= CONFIG_RT_SECOND_IF_RF_OFFSET) && (from + len < CONFIG_RT_SECOND_IF_RF_OFFSET + MAX_RF_DATA_SIZE)))
	{
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\n####  read data address is not in radio partition !! ####\n"));
		ret = -1;
		goto exit;
	}
	else
	{
		os_alloc_mem(NULL, (UCHAR **)&partition_buf, sizeof(UCHAR)*EEPROM_SIZE_ALL);
		if (buf == NULL || partition_buf == NULL)
		{
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\n####  malloc fail !! ####\n"));
			ret = -1;
			goto exit;
		...
		else
		{
			...
			MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n####  read 5G cal from flash !! ####\n"));
			spiflash_ioctl_read(partition_buf, DEFAULT_WHOLE_RF_OFFSET, EEPROM_SIZE_ALL);

			for (index = 0; index < len; index++)
			{
				buf[index] = partition_buf[index + from -DEFAULT_WHOLE_RF_OFFSET];
			}
		}
	}
...
exit:
	if (partition_buf)
		os_free_mem(partition_buf);	
	
	return ret;
}

static int raspi_flash_write(const u_char *buf, u32 to, u32 len)
{
	int ret = 0;
	int index = 0;
	u_char *partition_buf = NULL;
	/* Write all eeprom data in memory into flash. */
		if (!((to >= CONFIG_RT_SECOND_IF_RF_OFFSET) && (to + len < CONFIG_RT_SECOND_IF_RF_OFFSET + MAX_RF_DATA_SIZE)))
	{
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\n####  write data address is not in radio partition !! ####\n"));
		ret = -1;
		goto exit;
	}
	else
	{
		...
		else
		{
			...
			for (index = 0; index < len; index++)
			{
				partition_buf[index + to - DEFAULT_WHOLE_RF_OFFSET] = buf[index];
			}
			spiflash_ioctl_erase(DEFAULT_WHOLE_RF_OFFSET, EEPROM_SIZE_ALL);
			spiflash_ioctl_write(partition_buf, DEFAULT_WHOLE_RF_OFFSET, EEPROM_SIZE_ALL);
		}		
	}
...
int rtmp_ee_flash_write(PRTMP_ADAPTER pAd, UINT32 Offset, USHORT Data)
{
	if (IS_EE_INIT(pAd)) {
		...
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("write cal to mem only!! \n"));
	}
