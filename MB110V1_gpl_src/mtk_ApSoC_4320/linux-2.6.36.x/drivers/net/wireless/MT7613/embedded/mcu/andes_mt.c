...
static VOID event_get_tx_statistic_handle(struct _RTMP_ADAPTER *pAd, UINT8 *data, UINT32 Length)
{
	...
	/*  EntryLifeCheck Function, add for TP-Link  :
	     		we update pEntry->ContinueTxFailCnt and pEntry->NoDataIdleCount every 500 ms, and check whether pEntry->ContinueTxFailCnt
	    is bigger than pAd->ApCfg.EntryLifeCheck. in addition: MacTableMaintenance() well check pEntry->ContinueTxFailCnt too, and send DeAuth to Disconnect the STA
	*/
	if (entry->TxStatRspCnt >= 1)
	{
		MAC_TABLE_ENTRY		*pEntry = NULL;
		STA_TR_ENTRY		*tr_Entry = NULL;

		/* ignore invaild WCID such as  0xFF */
		...
		/*
		if (pEntry & tr_Entry.PsDeQWaitCnt)
		...
		*/
		...
		if ( !tx_success && ( event->u4EntryTxCount > 0 ) )/* all packet send failed */
		{

#ifdef DOT11_N_SUPPORT
        /* we do not need NoBADataCountDown set here, set BA ASAP, tp */
#if 0
			/* 8x500ms = 4S, we allow to add BA after 4s */
			pEntry->NoBADataCountDown = 8;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Entry %02x:%02x:%02x:%02x:%02x:%02x ->NoBADataCountDown=64\n",
									PRINT_MAC(pEntry->Addr) ));
#endif
#endif /* DOT11_N_SUPPORT */

			/* Update the continuous transmission failed counter.*/
			pEntry->ContinueTxFailCnt += event->u4EntryTxFailCount;
			tr_Entry->ContinueTxFailCnt += event->u4EntryTxFailCount;

			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Entry(%02x:%02x:%02x:%02x:%02x:%02x)->ContinueTxFailCnt(%d) pAd->ApCfg.EntryLifeCheck(%ld)\n",
									PRINT_MAC(pEntry->Addr),pEntry->ContinueTxFailCnt, pAd->ApCfg.EntryLifeCheck));

			if(pEntry->PsMode == PWR_ACTIVE)
			{
				/* tear down all BA BAOriWcidArray,
				     Considering the low trafic, according to the field test situation maybe Comment out the word segment */
#ifdef DOT11_N_SUPPORT
				int tid;
				for (tid=0; tid<NUM_OF_TID; tid++)
					ba_ori_session_tear_down(pAd, pEntry->wcid,  tid, FALSE, FALSE);
#endif /* DOT11_N_SUPPORT */
		...
		else
		{
			...
			if ((pEntry->PsMode != PWR_SAVE) && (pEntry->NoBADataCountDown > 0))
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Entry %02x:%02x:%02x:%02x:%02x:%02x ->NoBADataCountDown=0\n",
						PRINT_MAC(pEntry->Addr) ));
				}
			}
#endif /* DOT11_N_SUPPORT */
			...
			/* update NoDataIdleCount when sucessful send packet to STA.
			    maybe other mlme frame included, so we do not update NoDataIdleCount*/
