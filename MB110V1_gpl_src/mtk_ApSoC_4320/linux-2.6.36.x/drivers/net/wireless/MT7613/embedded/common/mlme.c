...
struct _RTMP_CHIP_OP *ops = hc_get_chip_ops(pAd->hdev_ctrl);
	...
	pWscControl = &pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.WscControl;
...
VOID MlmeUpdateVhtTxRates(RTMP_ADAPTER *pAd, PMAC_TABLE_ENTRY pEntry, struct wifi_dev *wdev)
{
	...
	if (WMODE_CAP_AC(wdev->PhyMode) &&
		...
		if (wlan_operate_get_vht_bw(wdev) == VHT_BW_2040) {
			if ((wdev->HTPhyMode.field.BW == BW_20) && (ucMaxMcs == 9))
			{
				ucMaxMcs = 8;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<===MSC, %s(%d): BW is HT20, set max MCS to 8!\n",
	 				__func__, __LINE__));
			}
		}
	...
	IN UCHAR RxPhyMode)
{
	...
	*/
	...
	if ((pAd->ApCfg.bMACRepeaterEn) && (bToApCli == TRUE)) {
		RTMP_SPIN_LOCK_IRQSAVE(&pAd->ApCfg.ReptCliEntryLock, &flags);
		...
		RTMP_SPIN_UNLOCK_IRQRESTORE(&pAd->ApCfg.ReptCliEntryLock, &flags);
	}
...
#ifdef RTMP_MAC_PCI
	RTMP_SPIN_LOCK_IRQSAVE(&pAd->Mlme.TaskLock, &flags);

	if (pAd->Mlme.bRunning) {
		RTMP_SPIN_UNLOCK_IRQRESTORE(&pAd->Mlme.TaskLock, &flags);
		return;
	} else
		pAd->Mlme.bRunning = TRUE;
