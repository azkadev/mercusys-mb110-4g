...
#ifdef TX_FAULT_DEBUG
		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s fp_tx_pkt_allowed\n",__FUNCTION__));
#endif
		if (allowed) {
...
#ifdef TX_FAULT_DEBUG
				MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s __debug__ RTMP_SET_PACKET_DHCP It is DHCP pkt\n",__FUNCTION__));
			//	if(RTMP_GET_PACKET_DHCP(pPacket))
			//		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s __debug__ RTMP_GET_PACKET_DHCP It is DHCP pkt\n",__FUNCTION__));
#endif
...
#ifdef TX_FAULT_DEBUG
				MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s __debug__ It is Ping pkt\n",__FUNCTION__));
#endif
				RTMP_SET_PACKET_HIGH_PRIO(pPacket, 1);
				RTMP_SET_PACKET_TXTYPE(pPacket, TX_LEGACY_FRAME);
			}
#ifdef TX_FAULT_DEBUG
			if (*pSrcBuf == ICMP_TYPE_ECHO_REQ)
			{
				RTMP_SET_PACKET_PING_REQ(pPacket, 1);
				MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPCheckEtherType ICMP_TYPE_ECHO_REQ\n"));

			}
			if (*pSrcBuf == ICMP_TYPE_ECHO_RSP)
			{
				RTMP_SET_PACKET_PING_RSP(pPacket, 1);
				MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPCheckEtherType ICMP_TYPE_ECHO_RSP\n"));
...
/* for MSP check always 0xFF for Rx log easily check PASS / FAIL */
	...
	case ETH_TYPE_ARP: {
		...
		RTMP_SET_PACKET_TXTYPE(pPacket, TX_LEGACY_FRAME);
#ifdef TX_FAULT_DEBUG
		MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s __debug__ It is ARP pkt\n",__FUNCTION__));
		//if(RTMP_GET_PACKET_ARP(pPacket))
		//	MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s __debug__ RTMP_GET_PACKET_ARP is ARP pkt\n",__FUNCTION__));
#endif
	}
	break;
...
VOID de_fragment_data_pkt(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		("de_fragment_data_pkt: SN:%d,FN:%d,PN:%llu, sec_mode: %d\n",
	...
	if (pRxBlk->FN == 0) {
		...
		if ((pRxBlk->SN != pAd->FragFrame.Sequence) ||
			...
			((pAd->FragFrame.sec_on) && (pAd->FragFrame.sec_mode == pRxBlk->sec_mode) && (pRxBlk->CCMP_PN != (pAd->FragFrame.LastPN + 1)))) {
			...
			*/
			...
			if ((wdev->wdev_type == WDEV_TYPE_AP || wdev->wdev_type == WDEV_TYPE_GO) &&
...

				{
					...
					pSrcEntry = MacTableLookup(pAd, pRxBlk->Addr2); /*pHeader802_3 + MAC_ADDR_LEN == SAAddr2*/
					if (pSrcEntry) {
						STA_TR_ENTRY *tr_entry = &pAd->MacTab.tr_entry[pSrcEntry->wcid];

						if (tr_entry && (tr_entry->PortSecured != WPA_802_1X_PORT_SECURED)) {

							MAC_TABLE_ENTRY *pDaEntry = MacTableLookup(pAd, pRxBlk->Addr3); /*find DA Entry*/
							if (pDaEntry) {
...
#ifdef APSD_SUPPORT
	static int num_multi = 0;
	static int num_no_multi = 0;
#endif
				...
				*/
				...
				if (!bTAMatchSA) {
						...
						*/
						...
						{
							if (0x01 & pRxBlk->Addr3[0])
							{
								MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(%d):  multicast dst(%02x:%02x:%02x:%02x:%02x:%02x), num_multi:%d, num_no_multi:%d\n",
									__func__, __LINE__, PRINT_MAC(pRxBlk->Addr3), num_multi, num_no_multi));
								num_multi++;

								if ((pMovedEntry->NoDataIdleCount > THESHOLD_FOR_ENTRY_DEL) ||
									((pEntry->PsMode != PWR_SAVE) && (pEntry->ContinueTxFailCnt >= pAd->ApCfg.EntryLifeCheck)))
								{
									MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
										("%s(%d): mac_entry_delete, src(%02x:%02x:%02x:%02x:%02x:%02x), dst(%02x:%02x:%02x:%02x:%02x:%02x)\n",
										__func__, __LINE__, PRINT_MAC(pMovedEntry->Addr), PRINT_MAC(pRxBlk->Addr3)));
									mac_entry_delete(pAd, pMovedEntry);
								}
								else
								{
									MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
										("%s(%d): src(%02x:%02x:%02x:%02x:%02x:%02x), dst(%02x:%02x:%02x:%02x:%02x:%02x), NoDataIdleCount:%d, PsMode:%d, ContinueTxFailCnt:%d\n",
										__func__, __LINE__, PRINT_MAC(pMovedEntry->Addr), PRINT_MAC(pRxBlk->Addr3),
										pMovedEntry->NoDataIdleCount, pEntry->PsMode, pEntry->ContinueTxFailCnt));
								}
							}
							else
							{
						...
						{
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
								("%s(%d): src(%02x:%02x:%02x:%02x:%02x:%02x), dst(%02x:%02x:%02x:%02x:%02x:%02x), tr_entry->PortSecured != WPA_802_1X_PORT_SECURED\n",
								__func__, __LINE__, PRINT_MAC(pMovedEntry->Addr), PRINT_MAC(pRxBlk->Addr3)));
						}
#else /* APSD_SUPPORT */
						mac_entry_delete(pAd, pMovedEntry);
	...
	if (!RTMPCheckWPAframe(pAd, pEntry, pData, data_len, wdev_idx, eth_frame)) {
		...
		if ((pEntry->wdev->wdev_type == WDEV_TYPE_AP || pEntry->wdev->wdev_type == WDEV_TYPE_GO) &&
			...
			if (FC != NULL && FC->Wep &&
				...
				MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("==> De-Auth this STA(%02x:%02x:%02x:%02x:%02x:%02x)\n",
...
INT rx_chk_amsdu_invalid_frame(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, struct wifi_dev *wdev)
{
	...
	if (!trEntry) {
		MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("teEntry is null\n"));
		return NDIS_STATUS_SUCCESS;
	}
	...
	if (RX_BLK_TEST_FLAG(pRxBlk, fRX_AMSDU) && (pRxBlk->AMSDU_ADDR)) {
		...
		if ((AmsduState == FIRST_AMSDU_FORMAT) &&
			...
			MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("80211 Addr=%02x:%02x:%02x:%02x:%02x:%02x\t", PRINT_MAC(pRxBlk->AMSDU_ADDR)));
			MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_OFF,
...
#endif /* ROAMING_ENHANCE_SUPPORT */
#endif /* APCLI_SUPPORT */
	if ((pRxBlk->CipherMis == 1) && (FC->Type == FC_TYPE_DATA)) {
...
BOOLEAN mtd_rx_done_handle(RTMP_ADAPTER *pAd)
{
	...
	//2020.5.7 mjz: set max_rx_process_cnt to 32 to reduce RX interrupt
	const UINT16 max_rx_process_cnt = 32;
...
BOOLEAN rtmp_rx_done_handle(RTMP_ADAPTER *pAd)
{
	...
	RxProcessed = RxPending = 0;
#ifdef TX_FAULT_DEBUG
	MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(): f_shuai rtmp_rx_done_handle\n", __FUNCTION__));
#endif
	while (1) {
		if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST)
			&& (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_POLL_IDLE)))
...
//2019.12.12 by mjz: fix bug 302534 & 302576
#if 0
