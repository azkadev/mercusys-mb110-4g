	...
	if ((pMbss->wdev.WscControl.WscConfMode != WSC_DISABLE) &&
		...
		if (!ApCheckAccessControlList(pAd, auth_info.addr2, apidx)) {
			...
			if (pEntry)
			{
				...
				{
					struct bs_auth_reject_ind event;
					memset(&event, 0, sizeof(struct bs_auth_reject_ind));
					memcpy(event.client_addr, auth_info.addr2, MAC_ADDR_LEN);
					/*TO DO -rssi*/
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d auth rej addr = %02X:%02X:%02X:%02X:%02X:%02X, sysindex = %d\r\n", 
					__FUNCTION__, __LINE__, event.client_addr[0],  event.client_addr[1], event.client_addr[2], event.client_addr[3],
					 event.client_addr[4], event.client_addr[5], RtmpOsGetNetIfIndex(wdev->if_dev)) );
					mtk_deliver_band_steering_event(RtmpOsGetNetIfIndex(wdev->if_dev), ATH_EVENT_BSTEERING_TX_AUTH_FAIL, sizeof(event), (u_int8_t*)&event);
				}
			}
#endif
