...
#if defined(CUSTOMER_VENDOR_IE_SUPPORT) || defined(TP_IE_SUPPORT)
extern INT RTMPIoctlQueryScanResult(
...
#ifdef AWND_SUPPORT
     {"ApCliEnableFromAwnd",			Set_ApCli_Enable_From_Awnd_Proc},
     {"AwndCfg",                    Set_Awnd_Cfg_Proc},
#endif
...
#ifdef TP_NETWORK_ROAMING
extern void send_event_to_network_roaming(struct net_device *dev,  u_int16_t siocType, u_int16_t event, char *data);
static int mcs_to_numstreams(int mcs)
{
	int numstreams = 0;   
	...
	if (((mcs >= 16) && (mcs <= 23)) || ((mcs >= 39) && (mcs <= 52)))		
		numstreams = 3;   /* four streams mcs rates */	 
	if (((mcs >= 24) && (mcs <= 31)) || ((mcs >= 53) && (mcs <= 76)))	   
		numstreams = 4;   
	return numstreams;
}
static int _rtmp_BssEntry_cmp(const void *a, const void *b)
{
	return ((const wlanif_bssInfo_t*)b)->rssi - ((const wlanif_bssInfo_t*)a)->rssi;
}
#endif
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
	...
	while ((this_char = strsep((char **)&tmp, "\b")) != NULL) {
		if (!*this_char)
			continue;
			...
			if (pEntry) {
				...
				*/
				if (IS_FT_RSN_STA(pEntry)) {
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(%d): MacTableDeleteEntry %02x:%02x:%02x:%02x:%02x:%02x\n",
						__func__, __LINE__, PRINT_MAC(Addr)));
...
#define VOW_CMD_STR_LEN 16
	...
	case OID_AP_VENDOR_IE_SET: {
		...
		/* 2019.11.4 mjz
		* the beacon of rai2 do not contain TPIE after modifying parameters of host network,
		* do not judge the state here
		*/
		/*
		if (wdev->if_up_down_state == FALSE) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		...
		*/
		...
		if (ret == NDIS_STATUS_SUCCESS) {
			unsigned long flags;
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				("Set::OID_AP_VENDOR_IE_SET apidx=%d\n",apidx));
			...
			RTMP_SPIN_LOCK_IRQSAVE(&ap_vendor_ie->vendor_ie_lock, &flags);
			...
			RTMP_SPIN_UNLOCK_IRQRESTORE(&ap_vendor_ie->vendor_ie_lock, &flags);
	...
	case OID_SET_OUI_FILTER:
	{
		...
		if (wrq->u.data.length == 0) {
			...
			RTMP_SPIN_LOCK_IRQSAVE(&ap_customer_oui->oui_filter_lock, &flags);
			if (ap_customer_oui->pointer != NULL) {
				os_free_mem(ap_customer_oui->pointer);
				ap_customer_oui->pointer = NULL;
			}
			ap_customer_oui->length = 0;
			RTMP_SPIN_UNLOCK_IRQRESTORE(&ap_customer_oui->oui_filter_lock, &flags);
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
	...
	case OID_APCLI_VENDOR_IE_SET:
	{
		...
		if (ret == NDIS_STATUS_SUCCESS) {
			...
			RTMP_SPIN_LOCK_IRQSAVE(&apcli_vendor_ie->vendor_ie_lock, &flags);
			...
			RTMP_SPIN_UNLOCK_IRQRESTORE(&apcli_vendor_ie->vendor_ie_lock, &flags);
		} else {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s()::alloc memory fail\n", __func__));
		}
	...
	case OID_802_11_VENDOR_IE_SHOW:
		...
		break;
#ifdef TP_NETWORK_ROAMING
	case OID_802_11R_GET_STATION_LIST_INFO:
		{
			#define STATION_INFO_LEN	(sizeof(wlanif_staInfo_t))
			#define STATION_LIST_LEN 	(sizeof(wlanif_staList_t) + MAX_LEN_OF_MAC_TABLE*STATION_INFO_LEN)
			int i = 0;	
			int idx = 0;	
			wlanif_staList_t *msg = NULL;	
			int status = 0; 
			int error = 0;	
			idx = 0;	/*os_alloc_mem(NULL, (UCHAR **)&msg, sizeof(CHAR)*(MAX_LEN_OF_MAC_TABLE*MAC_LINE_LEN)); */
			//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d: STATION_LIST_LEN = %d, MAX_LEN_OF_MAC_TABLE = %d, STATION_INFO_LEN = %d\r\n", __FUNCTION__, __LINE__, STATION_LIST_LEN, MAX_LEN_OF_MAC_TABLE, STATION_INFO_LEN)); 
			os_alloc_mem(NULL, (UCHAR **)(&msg), STATION_LIST_LEN); 	
			if (msg == NULL)	
			{
			...
			memset(msg, 0, STATION_LIST_LEN);
			msg->staInfo = (wlanif_staInfo_t*)((UCHAR*)(msg) + sizeof(wlanif_staList_t));
				
			for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++)	
			{		
				MAC_TABLE_ENTRY *pEntry = &pAd->MacTab.Content[i];
				CHAR rssiAvg = RTMPAvgRssi(pAd, &pEntry->RssiSample);
				//CHAR rssiMax = RTMPMaxRssi(pAd, pEntry->RssiSample.AvgRssi[0],
										//pEntry->RssiSample.AvgRssi[1],
										//pEntry->RssiSample.AvgRssi[2]);
				if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC) && (pEntry->func_tb_idx == MAIN_MBSSID))		
				{	
					//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("rssiAvg = %d, rssiMax = %d\r\n", rssiAvg, rssiMax));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("copy entry index = %d\r\n", i));
					COPY_MAC_ADDR(msg->staInfo[idx].mac, pEntry->Addr);
					//msg->staInfo[idx].isBTMSupported = 0;
					msg->staInfo[idx].isBTMSupported = pEntry->BssTransitionManmtSupport;
					//UINT8 octet1 = 0;
					memcpy(&octet1, (UINT8 *)(&pEntry->RrmEnCap.word), sizeof(UINT8));
					//printk("%s %d RrmEnCap.word = %llu, octet1 = 0x%x\r\n", __FUNCTION__, __LINE__, pEntry->RrmEnCap.word, octet1);
					msg->staInfo[idx].isRRMSupported = (octet1 != 0) ? 1 : 0;
					msg->staInfo[idx].rssi = rssiAvg + 90; /*95ÊÇqcaµÄµçÆµ»ùÊý*/;	
					//msg->staInfo[idx].rssi = rssiAvg + 95; /*95ÊÇqcaµÄµçÆµ»ùÊý*/; 
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("copy entry msg->staInfo[%d].rssi = %d\r\n", idx, msg->staInfo[idx].rssi));
					msg->staInfo[idx].lastRxRate = pEntry->LastRxRate;
					msg->staInfo[idx].lastTxRate = pEntry->LastTxRate;
					//msg->staInfo[idx].idle = 0; 
					msg->staInfo[idx].idle = pEntry->NoDataIdleCount;
					/*The follow are the members of phyCapInfo*/
					msg->staInfo[idx].phyCapInfo.valid = 1; 
					msg->staInfo[idx].phyCapInfo.maxChWidth = pEntry->MaxHTPhyMode.field.BW;
					//msg->staInfo[idx].phyCapInfo.phyMode = pEntry->MaxHTPhyMode.field.MODE; 
					//msg->staInfo[idx].phyCapInfo.phyMode = pEntry->MaxHTPhyMode.field.BW; 					
					if (pEntry->MaxHTPhyMode.field.BW == BW_20 || pEntry->MaxHTPhyMode.field.BW == BW_40)
					{
						msg->staInfo[idx].phyCapInfo.phyMode = wlanif_phymode_ht;
					}
					else if (pEntry->MaxHTPhyMode.field.BW == BW_80)
					{
						msg->staInfo[idx].phyCapInfo.phyMode = wlanif_phymode_vht;
					}
					else
					{
						msg->staInfo[idx].phyCapInfo.phyMode = wlanif_phymode_basic;
					}
		
					
					msg->staInfo[idx].phyCapInfo.maxMCS = pEntry->MaxHTPhyMode.field.MCS;
					msg->staInfo[idx].phyCapInfo.numStreams = mcs_to_numstreams(pEntry->MaxHTPhyMode.field.MCS);
					msg->staInfo[idx].phyCapInfo.maxTxPower = GetCuntryMaxTxPwr(pAd, msg->staInfo[idx].phyCapInfo.phyMode, pEntry->wdev, pEntry->MaxHTPhyMode.field.BW);
					idx++;		
				}	
			}	
			if (idx > 0)	
			{		
				...
				for (i = 0; i < idx; i++)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d entry[%d] mac = %02x:%02x:%02x:%02x:%02x:%02x\r\n", __FUNCTION__, __LINE__, i, PRINT_MAC(msg->staInfo[i].mac))); 
				}
				
				msg->sta_num = idx; 
				wrq->u.data.length = STATION_LIST_LEN;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d wrq->u.data.length = %d\r\n", __FUNCTION__, __LINE__, wrq->u.data.length)); 
				error = copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length); 
				if (error == 0) 	
				{			
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d: copy_to_user() OK\n", __FUNCTION__, __LINE__)); 
					status = NDIS_STATUS_SUCCESS;
				}		
				else		
				{			
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d: copy_to_user() fail\n", __FUNCTION__, __LINE__)); 
					status = NDIS_STATUS_FAILURE;	
				}	
			}	
			else
			{		
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d: No entrys get.\n", __FUNCTION__, __LINE__));
				status = NDIS_STATUS_FAILURE;
			}		
		...
		break;
	case OID_802_11R_GET_STATION_INFO:
		{
			wlanif_stainfo_t sta;	
			wlanif_stainfo_t *pSta = NULL;	
			int i = 0;	
			int found = 0;	
			memset(&sta, 0, sizeof(wlanif_stainfo_t));	
			pSta = (wlanif_stainfo_t*)(wrq->u.data.pointer);	
			//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("sta mac = %02x:%02x:%02x:%02x:%02x:%02x\r\n", PRINT_MAC(pSta->mac)));	
			COPY_MAC_ADDR(sta.mac, pSta->mac);
			for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++)	
			{
				MAC_TABLE_ENTRY *pEntry = &pAd->MacTab.Content[i];		
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("sta mac = %02x:%02x:%02x:%02x:%02x:%02x, pEntry->Addr = %02x:%02x:%02x:%02x:%02x:%02x\r\n", PRINT_MAC(sta.mac), PRINT_MAC(pEntry->Addr)));
				/*search the entry which mac is equal*/ 	
				if (IS_EQUAL_MAC(sta.mac, pEntry->Addr))		
				{
					sta.sta.idle = 1;
					sta.sta.lastRxRate = pEntry->LastRxRate;
					sta.sta.lastTxRate = pEntry->LastTxRate;
					sta.sta.rssi = (RTMPAvgRssi(pAd, &pEntry->RssiSample) + 90);
					//CHAR rssi = (RTMPAvgRssi(pAd, &pEntry->RssiSample) + 95); /*95ÊÇqcaµÄµçÆµ»ùÊý*/
					//sta.sta.rssi = RTMPMaxRssi(pAd, pEntry->RssiSample.AvgRssi[0],
						//				pEntry->RssiSample.AvgRssi[1],
							//			pEntry->RssiSample.AvgRssi[2]);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d sta.sta.rssi = %d \r\n", __FUNCTION__, __LINE__, sta.sta.rssi));
					sta.sta.rxByteCount = pEntry->RxBytes;	
					sta.sta.rxPacketCount = (ULONG)pEntry->RxPackets.QuadPart;
					sta.sta.txByteCount = pEntry->TxBytes;
					sta.sta.txPacketCount =  (ULONG)pEntry->TxPackets.QuadPart;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d: sta.sta.lastRxRate = %d, sta.sta.lastTxRate = %d, sta.sta.rssi = %d, sta.sta.idle = %d\n", __FUNCTION__, __LINE__, sta.sta.lastRxRate, sta.sta.lastTxRate, sta.sta.rssi, sta.sta.idle));		
					found = 1;
					break;
				}
			}
			...
			if (found)	
			{		
				wrq->u.data.length = sizeof(wlanif_stainfo_t);
				Status = NDIS_STATUS_FAILURE;
				Status = copy_to_user(wrq->u.data.pointer, &sta, wrq->u.data.length);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d: Status = %d, wrq->u.data.length = %d\n", __FUNCTION__, __LINE__, Status, wrq->u.data.length));		
				...
				{
					//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s %d: copy_to_user() success\n", __FUNCTION__, __LINE__));			
					//return NDIS_STATUS_SUCCESS;	
				}	
			}
			else
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d: status info not found.\n", __FUNCTION__, __LINE__));		
				Status = NDIS_STATUS_FAILURE;
			}
		}
		break;
	case OID_802_11R_GET_PHYCAP_INFO:
		{
			//u_int8_t mac[ETHER_ADDR_LEN]; 
			phycapInfo staPhyCapInfo;	
			phycapInfo *pPhyCapInfo = (phycapInfo*)(wrq->u.data.pointer);
			int i = 0;
			int found = 0;
			memset(&staPhyCapInfo, 0, sizeof(phycapInfo));
			COPY_MAC_ADDR(&staPhyCapInfo.mac, pPhyCapInfo->mac);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d: target mac = %02x:%02x:%02x:%02x:%02x:%02x, Bssid = %02x:%02x:%02x:%02x:%02x:%02x\n", __FUNCTION__, __LINE__, PRINT_MAC(staPhyCapInfo.mac), PRINT_MAC(pAd->CommonCfg.Bssid)));
			for( i = 0; i < pAd->ApCfg.BssidNum; i++ )
			{
				if ( IS_EQUAL_MAC(staPhyCapInfo.mac, pAd->ApCfg.MBSSID[i].wdev.bssid))	
				{
					staPhyCapInfo.phyCapInfo.maxChWidth =	pAd->ApCfg.MBSSID[i].wdev.MaxHTPhyMode.field.BW;			
					staPhyCapInfo.phyCapInfo.maxMCS = pAd->ApCfg.MBSSID[i].wdev.MaxHTPhyMode.field.MCS; 			
					staPhyCapInfo.phyCapInfo.maxTxPower = GetCuntryMaxTxPwr(pAd, pAd->ApCfg.MBSSID[i].wdev.PhyMode, &(pAd->ApCfg.MBSSID[i].wdev), 
																				pAd->ApCfg.MBSSID[i].wdev.MaxHTPhyMode.field.BW);			
					staPhyCapInfo.phyCapInfo.numStreams = mcs_to_numstreams(pAd->ApCfg.MBSSID[i].wdev.MaxHTPhyMode.field.MCS);			
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MaxHTPhyMode BW:%d\nMCS:%d\nmaxTxPower:%d\nStreams:%d\n",staPhyCapInfo.phyCapInfo.maxChWidth,
						staPhyCapInfo.phyCapInfo.maxMCS,
						staPhyCapInfo.phyCapInfo.maxTxPower,
						staPhyCapInfo.phyCapInfo.numStreams));	
					/*
					staPhyCapInfo.phyCapInfo.maxChWidth =	pAd->ApCfg.MBSSID[i].wdev.HTPhyMode.field.BW;			
					staPhyCapInfo.phyCapInfo.maxMCS = pAd->ApCfg.MBSSID[i].wdev.HTPhyMode.field.MCS;				
					staPhyCapInfo.phyCapInfo.maxTxPower = GetCuntryMaxTxPwr(pAd, pAd->CommonCfg.Channel);			
					staPhyCapInfo.phyCapInfo.numStreams = mcs_to_numstreams(pAd->ApCfg.MBSSID[i].wdev.HTPhyMode.field.MCS); 		
					printk("HTPhyMode BW:%d\nMCS:%d\nmaxTxPower:%d\nStreams:%d\n",staPhyCapInfo.phyCapInfo.maxChWidth,
						staPhyCapInfo.phyCapInfo.maxMCS,
						staPhyCapInfo.phyCapInfo.maxTxPower,
						staPhyCapInfo.phyCapInfo.numStreams);				
					*/
			...
			if ( !found )
			{
				for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++)	
				{		
					MAC_TABLE_ENTRY *pEntry = &pAd->MacTab.Content[i];		/*search the entry which mac is equal*/ 	
					
					if (IS_EQUAL_MAC(staPhyCapInfo.mac, pEntry->Addr) && IS_ENTRY_CLIENT(pEntry) )
					{			
						staPhyCapInfo.phyCapInfo.maxChWidth = pAd->MacTab.Content[i].HTPhyMode.field.BW;			
						staPhyCapInfo.phyCapInfo.maxMCS = pAd->MacTab.Content[i].HTPhyMode.field.MCS;			
						staPhyCapInfo.phyCapInfo.maxTxPower = GetCuntryMaxTxPwr(pAd, pEntry->wdev->PhyMode, 
																					pEntry->wdev, pAd->MacTab.Content[i].HTPhyMode.field.BW);			
						staPhyCapInfo.phyCapInfo.numStreams = mcs_to_numstreams(pAd->MacTab.Content[i].HTPhyMode.field.MCS);			
						//staPhyCapInfo.phyCapInfo.phyMode = pAd->CommonCfg.PhyMode;	
						//staPhyCapInfo.phyCapInfo.phyMode = pAd->MacTab.Content[i].HTPhyMode.field.BW;
						if (pAd->MacTab.Content[i].HTPhyMode.field.BW == BW_20 || pAd->MacTab.Content[i].HTPhyMode.field.BW == BW_40)
						{
							staPhyCapInfo.phyCapInfo.phyMode = wlanif_phymode_ht;
						}
						else if (pEntry->MaxHTPhyMode.field.BW == BW_80)
						{
							staPhyCapInfo.phyCapInfo.phyMode = wlanif_phymode_vht;
						}
						else
						{
							staPhyCapInfo.phyCapInfo.phyMode = wlanif_phymode_basic;
						}
						
						staPhyCapInfo.phyCapInfo.valid = 1; 
						found = 1;
						break;
					}	
				}
			...
			if (found) 
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d: Found phyinfo of mac = %02x:%02x:%02x:%02x:%02x:%02x, \r\n phyCapInfo: maxChWidth = %d, maxMCS = %d, maxTxPower = %d, numStreams = %d, phyMode = %d\r\n", __FUNCTION__, __LINE__, PRINT_MAC(staPhyCapInfo.mac), staPhyCapInfo.phyCapInfo.maxChWidth, staPhyCapInfo.phyCapInfo.maxMCS, staPhyCapInfo.phyCapInfo.maxTxPower, staPhyCapInfo.phyCapInfo.numStreams, staPhyCapInfo.phyCapInfo.phyMode));
				wrq->u.data.length = sizeof(phycapInfo);	
				if (copy_to_user(wrq->u.data.pointer, &staPhyCapInfo, wrq->u.data.length) != 0) 
				{		
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: entry not found\n", __FUNCTION__));	
				Status = -EFAULT;
			}
		}
		break;
	case OID_802_11R_GET_RSSI_INFO:
		{
			//u_int8_t mac[ETHER_ADDR_LEN] = {0};
			RssiInfo rssiInfo;
			RssiInfo *pRssiInfo = (RssiInfo*)(wrq->u.data.pointer);
			int i = 0;	
			memset(&rssiInfo, 0, sizeof(RssiInfo));
			COPY_MAC_ADDR(&rssiInfo.mac, pRssiInfo->mac);	
			for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++)	
			{		
				MAC_TABLE_ENTRY *pEntry = &pAd->MacTab.Content[i];
				/*search the entry which mac is equal*/ 	
				if (IS_EQUAL_MAC(pRssiInfo->mac, pEntry->Addr))
				{			
					rssiInfo.rssi = (RTMPAvgRssi(pAd, &pEntry->RssiSample) + 90);
				}	
			}		
			wrq->u.data.length = sizeof(RssiInfo);
			if (copy_to_user(wrq->u.data.pointer, &rssiInfo, wrq->u.data.length))
			{		
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: copy_to_user() fail\n", __FUNCTION__));	
			}	
		}	
		break;
	case OID_802_11R_SCAN_EXAP_INFO:
			{
				struct iw_scan_req *pScanopt = NULL;
				int scanType = 0;
				int ssidLength = 0;
				int waitCnt = 0;
				NDIS_802_11_SSID targetSsid;
				PNDIS_802_11_SSID pTargetSsid = NULL;

				memset(&targetSsid, 0, sizeof(NDIS_802_11_SSID));
				pScanopt = (struct iw_scan_req*)(wrq->u.data.pointer);
				if (pScanopt->scan_type == 0)
				{
				...
				ssidLength = strlen(pScanopt->essid);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d ssidLength = %d\r\n", __FUNCTION__, __LINE__, ssidLength));
				if (ssidLength !=0)
				{
					strncpy(targetSsid.Ssid, pScanopt->essid, ssidLength);
					targetSsid.SsidLength = ssidLength;
					pTargetSsid = &targetSsid;
				}
				else
				{
					pTargetSsid = NULL;
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d scan target SSID %s, scanType = %d\r\n", __FUNCTION__, __LINE__, pTargetSsid->Ssid, scanType));
				waitCnt = 0;
				ApSiteSurvey_by_wdev(pAd, pTargetSsid, scanType, FALSE, &pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev);
				...
				//send_event_to_network_roaming(pAd->net_dev, SIOCGIWSCAN, 1/*IEEE80211_REASON_COMPLETED*/, &ch);
				send_event_to_network_roaming(pAd->net_dev, SIOCGIWSCAN, 1/*IEEE80211_REASON_COMPLETED*/, NULL);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d scan finished, send IEEE80211_REASON_COMPLETED to user space.\r\n", __FUNCTION__, __LINE__));
			}
		break;
	
	case OID_802_11R_GET_EXAP_INFO:
		{
				struct wifi_dev *wdev;
				SCAN_CTRL *ScanCtrl = NULL;
				INT32 ifIndex = pObj->ioctl_if;
...
#ifndef RSSI_BASE
				#define RSSI_BASE (-100)
#endif
				int i = 0, j = 0, maxBssNum = 0, length = 0;
				wlanif_bssInfo_t *pBssList = NULL;
				BSS_ENTRY	*pBss = NULL;

				length = copy_from_user(&maxBssNum, wrq->u.data.pointer, sizeof(int));
				if (length != 0)
				{
					Status	= -EINVAL;
					break;
				...
				pBssList = (wlanif_bssInfo_t*)kmalloc((maxBssNum*sizeof(wlanif_bssInfo_t)), MEM_ALLOC_FLAG);
				if (NULL == pBssList)
				{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d pAd->ScanTab.BssNr = %d\r\n", __FUNCTION__, __LINE__, ScanCtrl->ScanTab.BssNr));
				if (0 == ScanCtrl->ScanTab.BssNr)
				{
					printk("%s %d No extend AP found.\r\n", __FUNCTION__, __LINE__);
					Status	= -EINVAL;
					break;
				}

				/* sort by rssi */
				sort(ScanCtrl->ScanTab.BssEntry, ScanCtrl->ScanTab.BssNr, sizeof(BSS_ENTRY), _rtmp_BssEntry_cmp, NULL);

				for (i = 0; i < ScanCtrl->ScanTab.BssNr; i++)
				{
					pBss = &ScanCtrl->ScanTab.BssEntry[i];
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d: ScanTab.BssEntry[%d]: ssid = %s, Channel = %d, Rssi = %d\r\n", __FUNCTION__, __LINE__, i, pBss->Ssid, pBss->Channel, pBss->Rssi));

					if (pBss->Channel == 0)
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d pBss[%d] channel == 0, continue", __FUNCTION__, __LINE__, i));
						continue;
					}

					if (pBss->Rssi < RSSI_BASE)
					{
						continue;
					}
					
					if (!NdisEqualMemory((char *)pAd->ApCfg.MBSSID[pObj->ioctl_if].Ssid, pBss->Ssid, MAX_LEN_OF_SSID ))
					{
						//printk("%s %d pAd->ApCfg.MBSSID[%d].Ssid = %s, pBss->Ssid = %s not equal, continue.\r\n", __FUNCTION__, __LINE__, pObj->ioctl_if, pAd->ApCfg.MBSSID[pObj->ioctl_if].Ssid, pBss->Ssid);
						continue;
					}
					...
					pBssList[j].channel = pBss->Channel;
					pBssList[j].rssi = pBss->Rssi - RSSI_BASE;
					memcpy(pBssList[j].essid, pBss->Ssid, MAX_LEN_OF_SSID);
					memcpy(pBssList[j].bssid, pBss->Bssid, MAC_ADDR_LEN);
					j++;

					if (j >= maxBssNum)
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d we just get %d entries.", __FUNCTION__, __LINE__, j));
						break;
					}
				}

				maxBssNum = j;
				wrq->u.data.length = (sizeof(CHAR))*(maxBssNum * sizeof(wlanif_bssInfo_t)) + sizeof(unsigned int);
				copy_to_user(wrq->u.data.pointer, &maxBssNum, sizeof(int));
				copy_to_user(wrq->u.data.pointer + sizeof(maxBssNum), pBssList, (maxBssNum * sizeof(wlanif_bssInfo_t)));
				kfree(pBssList);
			}
		break;
	case OID_802_11R_CLR_ASSOC_INFO:
		{			
			UCHAR Addr[MAC_ADDR_LEN];
			MAC_TABLE_ENTRY *pEntry = NULL;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d OID_802_11R_CLR_ASSOC_INFO.", __FUNCTION__, __LINE__));
			COPY_MAC_ADDR(Addr, wrq->u.data.pointer);
			pEntry = MacTableLookup(pAd, Addr);
			if (pEntry == NULL)
				break;
			if ( !IS_ENTRY_CLIENT(pEntry) )
			...
			if (pAd->ApCfg.bMACRepeaterEn == TRUE)
			{
				...
				pReptEntry = RTMPLookupRepeaterCliEntry(pAd, TRUE, Addr, TRUE);
				if (pReptEntry && (pReptEntry->CliConnectState != 0))
				{
					apCliIdx = pReptEntry->MatchApCliIdx;
					CliIdx = pReptEntry->MatchLinkIdx;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s %d mlmeEnque:APCLI_CTRL_DISCONNECT_REQ.", __FUNCTION__, __LINE__));
	...
	IN INT cmd)
{
	...
	case RT_OID_WSC_QUERY_MODE:
    {
    	INT WscMode;
    	//DBGPRINT(RT_DEBUG_TRACE, ("Query::RT_OID_WSC_MODE \n"));
#ifdef APCLI_SUPPORT
        if (pObj->ioctl_if_type == INT_APCLI)
        {
        	INT ApCliIdx = pObj->ioctl_if;
        	APCLI_MR_APIDX_SANITY_CHECK(ApCliIdx);
        	WscMode = pAd->ApCfg.ApCliTab[ApCliIdx].WscControl.WscMode;
        }
        else
#endif /* APCLI_SUPPORT */
        {
        	WscMode = pAd->ApCfg.MBSSID[apidx].wdev.WscControl.WscMode;
        }

        wrq->u.data.length = sizeof(INT);
        if (copy_to_user(wrq->u.data.pointer, &WscMode, wrq->u.data.length))
        {
        	Status = -EFAULT;
        }
        break;
    }
	case RT_OID_WSC_QUERY_LOCK:
	{
			WLAN_WPS_LOCK WscLock;
			OS_NDIS_MINIPORT_TIMER *_os_timer;

			//DBGPRINT(RT_DEBUG_TRACE, ("Query::RT_OID_WSC_QUERY_LOCK \n"));
#ifdef APCLI_SUPPORT
			if (pObj->ioctl_if_type == INT_APCLI)
			{
				INT ApCliIdx = pObj->ioctl_if;
				...
				WscLock.setupLock = sharedWscLock;

				if (WPS_LOCK_FOREVER == WscLock.setupLock)
				{
					WscLock.setupLockTime = -1;
				}
				else if (TRUE == WscLock.setupLock)
				{
					if(TRUE == pAd->ApCfg.ApCliTab[ApCliIdx].WscControl.WscSetupLockTimerRunning)
					{
						_os_timer = (OS_NDIS_MINIPORT_TIMER *)(&(pAd->ApCfg.ApCliTab[ApCliIdx].WscControl.WscSetupLockTimer.TimerObj));
						WscLock.setupLockTime = (_os_timer->expires - jiffies) / OS_HZ;
					}
					else
					{
						WscLock.setupLockTime = 0;
					}
				}
				else
				{
...
#endif /* APCLI_SUPPORT */
			{
				...
				{
					if(TRUE == pAd->ApCfg.MBSSID[apidx].wdev.WscControl.WscSetupLockTimerRunning)
					{
						_os_timer = (OS_NDIS_MINIPORT_TIMER *)(&(pAd->ApCfg.MBSSID[apidx].wdev.WscControl.WscSetupLockTimer.TimerObj));
						WscLock.setupLockTime = (_os_timer->expires - jiffies) / OS_HZ;
					}
					else
					{
			...
			wrq->u.data.length = sizeof(WLAN_WPS_LOCK);
			if (copy_to_user(wrq->u.data.pointer, &WscLock, wrq->u.data.length))
			{
				Status = -EFAULT;
			}
			break;
		...
		case OID_GET_AP_METRICS:
			{
				...
				ap_metric.cu = get_channel_utilization(pAd, pObj->ioctl_if);
	...
	IN	RTMP_STRING *arg)
{
	...
	if (value_interval == 0 || ((value_interval >= 10) && (value_interval < MAX_GROUP_REKEY_INTERVAL)))
...
#ifdef CONFIG_ATE
	bss.rssi = (unsigned int)(rx_stat->LastRssi[0] - pAd->BbpRssiToDbmDelta);
#else
	bss.rssi = (unsigned int)(pAd->ApCfg.RssiSample.LastRssi[0] - pAd->BbpRssiToDbmDelta);
#endif
	bss.channel = (unsigned int)wdev->channel; 
    // Copy the information into the user buffer
...
static inline UINT16 channelToFreq(
	UCHAR channel)
{
    if (channel <= 0)
    {
...
#define MAX_OF_TP_SCAN_ENTRY 128
VOID RTMPIoctlGetSiteSurveyDeco(
	IN	PRTMP_ADAPTER	pAdapter,
	IN	RTMP_IOCTL_INPUT_STRUCT	 *wrq)
{
    RTMP_STRING *msg;
    ...
    TP_SCAN_AP_RANGE *range;
    TP_SCAN_AP_ENTRY *head;
    PSTA_ADMIN_CONFIG pApCliEntry = &pAdapter->StaCfg[0];

    POS_COOKIE pObj = (POS_COOKIE) pAdapter->OS_Cookie;
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(%d): to find ioctl_if_type(0x%x), if_idx(%d)\n",
				 __func__, __LINE__, pObj->ioctl_if_type, pObj->ioctl_if));
    ...
    TotalLen = sizeof(TP_SCAN_AP_RANGE) + sizeof(TP_SCAN_AP_ENTRY)*MAX_OF_TP_SCAN_ENTRY;
    ...
    range = (TP_SCAN_AP_RANGE *)msg;
    head = (TP_SCAN_AP_ENTRY *)(msg + sizeof(TP_SCAN_AP_RANGE));
    ...
    if (ascii2int(this_char, &range->start) == FALSE)
        range->start = 0;

    MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): After check, this_char = %s, out = %d\n"
			 , __FUNCTION__, this_char, range->start));

	WaitCnt = 0;
	while ((ScanRunning(pAdapter) == TRUE) && (WaitCnt++ < 200))
		OS_WAIT(500);

    range->total = ScanTab->BssNr;
    range->band = AWND_BAND_5G;

    os_free_mem(this_char);
	if (range->total == 0) {
		wrq->u.data.length = sizeof(TP_SCAN_AP_RANGE);
		Status = copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(%d) - wrq->u.data.length = %d\n",
            __FUNCTION__, __LINE__, wrq->u.data.length));
    ...
    if (pApCliEntry->CfgSsidLen > 0)
           MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("RTMPIoctlGetSiteSurveyDeco(5G): tgt bss: %s\n", 
                        pApCliEntry->CfgSsid));

	for (i = range->start; i < range->total && range->valid < MAX_OF_TP_SCAN_ENTRY; i++) {
...
/*
		if (0 == pBss->TpIE.IELen)
			continue;
 */

        if ((pBss->SsidLen == 0) ||
            (pApCliEntry->CfgSsidLen != pBss->SsidLen) ||
            (!RTMPEqualMemory(pApCliEntry->CfgSsid, pBss->Ssid, pApCliEntry->CfgSsidLen)))
        {
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RTMPIoctlGetSiteSurveyDeco(5G): drop bss: %s\n", 
                pBss->Ssid));
            continue;
        }

        head->index = i;
        NdisMoveMemory(head->ssid, pBss->Ssid, pBss->SsidLen);
        head->ssidLen = pBss->SsidLen;
        NdisMoveMemory(head->bssid, pBss->Bssid, MAC_ADDR_LEN);

		if (pBss->Rssi < -95)
			head->rssi = 0;
		else
			head->rssi = pBss->Rssi + 95;

        head->freq = channelToFreq(pBss->Channel);
        head->channel = pBss->Channel;


        head->netInfo.id = 221;
        head->netInfo.len = pBss->TpIE.IELen + sizeof(pBss->TpIE.OUI);

        NdisMoveMemory(head->netInfo.oui, pBss->TpIE.OUI, 3);
        NdisMoveMemory(head->netInfo.tpie, pBss->TpIE.IE, TP_IE_MAX_LEN);
    ...
    range->over = i;
	wrq->u.data.length = sizeof(TP_SCAN_AP_RANGE) + sizeof(TP_SCAN_AP_ENTRY)*(range->valid);
	Status = copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length);

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPIoctlGetSiteSurveyDeco - wrq->u.data.length = %d\n", 
        wrq->u.data.length));
    os_free_mem(msg);
}
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
    ...
    if (wrq->u.data.length < (sizeof(struct ieee80211_wlanconfig)))
	  return -EFAULT;

    if (copy_from_user(&config, wrq->u.data.pointer, sizeof(struct ieee80211_wlanconfig))) {
        return -EFAULT;
    }
    ...
    Idx = BssTableSearch(&ScanCtrl->ScanTab, tpie->tp_macaddr, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Channel);
    if (BSS_NOT_FOUND != Idx && TRUE == bConnect)
    {
        if (ScanCtrl->ScanTab.BssEntry[Idx].TpIE.IELen > 0 && ScanCtrl->ScanTab.BssEntry[Idx].TpIE.IELen < TP_IE_MAX_LEN)
        {
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPIoctlGetTPIE TPIE len:%d content:\n", ScanCtrl->ScanTab.BssEntry[Idx].TpIE.IELen));
            for(i=0; i < ScanCtrl->ScanTab.BssEntry[Idx].TpIE.IELen; i++)
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("[%d]:0x%02X  ", i, ScanCtrl->ScanTab.BssEntry[Idx].TpIE.IE[i]));
        }

        awnNetInfo.id = 221;
        awnNetInfo.len = ScanCtrl->ScanTab.BssEntry[Idx].TpIE.IELen + sizeof(awnNetInfo.oui);
        awnNetInfo.oui[0] = ScanCtrl->ScanTab.BssEntry[Idx].TpIE.OUI[0];
        awnNetInfo.oui[1] = ScanCtrl->ScanTab.BssEntry[Idx].TpIE.OUI[1];
        awnNetInfo.oui[2] = ScanCtrl->ScanTab.BssEntry[Idx].TpIE.OUI[2];
        memcpy(awnNetInfo.tpie, ScanCtrl->ScanTab.BssEntry[Idx].TpIE.IE, TP_IE_MAX_LEN);
        memcpy(tpie->tp_ie, &awnNetInfo, sizeof(awnNetInfo));
        tpie->status = 0;
    }

    if (copy_to_user(wrq->u.data.pointer, &config, sizeof(struct ieee80211_wlanconfig))) 
    {
       return -EFAULT;
    }
    return 0;
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
		...
		for (i = 0; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
			...
			if (IS_ENTRY_PEER_AP(pEntry)
				...
				if (pEntry->wdev == &pAd->StaCfg[ifIndex].wdev) {
                        /* get the largest */
                        if (pEntry->RssiSample.AvgRssi[0] < 0 && pEntry->RssiSample.AvgRssi[1] < 0)
                        {
                            if (pEntry->RssiSample.AvgRssi[0] >= pEntry->RssiSample.AvgRssi[1])
                                MaxRssi = pEntry->RssiSample.AvgRssi[0];
                            else
                                MaxRssi = pEntry->RssiSample.AvgRssi[1];
                        }
                        else if (pEntry->RssiSample.AvgRssi[0] < 0)
                        {
                            MaxRssi = pEntry->RssiSample.AvgRssi[0];
                        }
                        else if (pEntry->RssiSample.AvgRssi[1] < 0)
                        {
                            MaxRssi = pEntry->RssiSample.AvgRssi[1];
                        }
    ...
    if (TRUE == bConnect)
    {
        config.data.connStaus.wds_state = IEEE80211_S_RUN;
    }
    else
    {
        config.data.connStaus.wds_state = IEEE80211_S_INIT;
    }

    config.data.connStaus.rssi = MaxRssi;
    config.status = IEEE80211_WLANCONFIG_OK;

    strcpy(config.data.connStaus.ssid, pAd->StaCfg[ifIndex].CfgSsid);
    config.data.connStaus.ssid[pAd->StaCfg[ifIndex].CfgSsidLen] = '\0';

/*
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("apclii%d ssid:||%s||\n", ifIndex, pAd->ApCfg.ApCliTab[ifIndex].Ssid));
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("apclii%d CfgSsid:||%s||\n", ifIndex, pAd->ApCfg.ApCliTab[ifIndex].CfgSsid));
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("apclii%d staCfg_Ssid:||%s||\n", ifIndex,  pAd->StaCfg[ifIndex].Ssid));
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("apclii%d staCfg_CfgSsid:||%s||\n", ifIndex,  pAd->StaCfg[ifIndex].CfgSsid));
*/
...
static inline wlan_phymode_e convertPhyMode(
	IN PRTMP_ADAPTER pAd)
{
    UCHAR   phymode;
    phymode = pAd->CommonCfg.cfg_wmode ;
...
static inline wlan_chwidth_e convertChWidth(
        struct wifi_dev *wdev)
{
    UCHAR bw = wlan_config_get_ht_bw(wdev);
    ...
    case VHT_BW_80:
        return wlan_chwidth_80;
    case VHT_BW_2040:
            return wlan_chwidth_40;
    }

    return wlan_chwidth_20;
}
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
    	MTWF_LOG(DBG_CAT_TX, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(%d): to find ioctl_if_type(0x%x), if_idx(%d)\n",
				 __func__, __LINE__, pObj->ioctl_if_type, pObj->ioctl_if));
	struct wifi_dev *wdev = get_wdev_by_ioctl_idx_and_iftype(pAd, pObj->ioctl_if, pObj->ioctl_if_type);
    ...
    phy->nss = wlan_operate_get_rx_stream(wdev);
    phy->phyMode = convertPhyMode(pAd);
    phy->chwidth = convertChWidth(wdev);
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
        ...
        if (wrq->u.data.length < sizeof(IEEE80211_WLANCONFIG_CMDTYPE))
        {
            return -EFAULT;
        }
        if (copy_from_user(&cmdtype, wrq->u.data.pointer, sizeof(IEEE80211_WLANCONFIG_CMDTYPE))) {
            return -EFAULT;
        }
    
        switch (cmdtype) {
            case IEEE80211_WLANCONFIG_TP_IE_GET:
                return RTMPIoctlGetTPIE(pAd,wrq);
                break;
            case IEEE80211_WLANCONFIG_WDS_STATE_GET:
                return RTMPIoctlGetWdsState(pAd, wrq);
            case IEEE80211_WLANCONFIG_PHY_GET:
                return RTMPIoctlGetPhy(pAd, wrq);
            default:
                return -EOPNOTSUPP;
        }
...
INT RTMPIoctlGetStaInfo(
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	struct ieee80211req_sta_info staInfo;
	//struct AWND_NET_INFO	  awnNetInfo;
	POS_COOKIE pObj;
	UCHAR ifIndex;
	BOOLEAN bConnect=FALSE;
#ifdef APCLI_SUPPORT
	...
	if (wrq->u.data.length < (sizeof(struct ieee80211req_sta_info)))
		return -EFAULT;
	...
	ifIndex = pObj->ioctl_if;
#if defined(APCLI_SUPPORT) && defined(APSD_SUPPORT)
	if((pAd->ApCfg.ApCliTab[ifIndex].CtrlCurrState == APCLI_CTRL_CONNECTED)
		&& (pAd->ApCfg.ApCliTab[ifIndex].SsidLen != 0))
	{
		for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	...
	if (TRUE == bConnect)
	{
		memcpy(staInfo.isi_macaddr, pEntry->Addr, MAC_ADDR_LEN);
		DataRate=0;
		getRate(pEntry->HTPhyMode, (ULONG *)&DataRate);
		DataRate *= 1000;
		staInfo.isi_txratekbps = DataRate;

		DataRate=0;
		HTPhyMode.word = (USHORT)pEntry->LastRxRate;
		getRate(HTPhyMode,	(ULONG *)&DataRate);
		DataRate *= 1000;
		staInfo.isi_rxratekbps = DataRate;
		staInfo.isi_inact = pEntry->NoDataIdleCount;

		wrq->u.data.length = sizeof(struct ieee80211req_sta_info);
		if (copy_to_user(wrq->u.data.pointer, &staInfo, sizeof(struct ieee80211req_sta_info)))
		{
		   return -EFAULT;
		}
	}
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	INT i, j=0;
	UCHAR phyMode = pAd->ApCfg.MBSSID[apidx].wdev.PhyMode;
	...
	{
		if (pAd->ApCfg.MBSSID[apidx].wdev.rate.DesireRate[i] == rateMask[i])
		{
...
#define RSSI_BASE (-100)
	...
	IfIdx = pObj->ioctl_if;
	pWdev = &pAd->ApCfg.MBSSID[IfIdx].wdev;
	ApSiteSurvey_by_wdev(pAd, NULL, SCAN_PASSIVE, FALSE, pWdev);
	OS_WAIT(1000);
	while ((scan_in_run_state(pAd, pWdev) == TRUE) && (WaitCnt++ < 200))
		OS_WAIT(500);

	ScanTab = get_scan_tab_by_wdev(pAd, pWdev);
	printk("Total BSS is %d\n", ScanTab->BssNr);

	/* sort by rssi */
	sort(ScanTab->BssEntry, ScanTab->BssNr, sizeof(BSS_ENTRY), _rtmp_BssEntry_cmp, NULL);
	...
	for (i = 0; i < ScanTab->BssNr; i++)
	{
		...
		pBssInfo[j].authMode = SecAuthModeNewToOld(pBss->AKMMap);
		pBssInfo[j].wepStatus = SecEncryModeNewToOld(pBss->PairwiseCipher);
		printk("Bss: %d, Ssid: %s, Privacy is %d, auth is %d, wepStatus is %d\n", i, pBss->Ssid,
                pBss->Privacy, pBssInfo[j].authMode, pBssInfo[j].wepStatus);
	...
	if (maxBssNum > ScanTab->BssNr) {
		maxBssNum = ScanTab->BssNr;
	}
...
INT Set_Awnd_Cfg_Proc(
        IN  PRTMP_ADAPTER pAd,
        IN  RTMP_STRING *arg)
{

	char buf[CONFIG_STR_LEN];
	int bufLen = (UCHAR)strlen(arg);

	if(bufLen < CONFIG_STR_LEN)
	{
		NdisZeroMemory(buf, CONFIG_STR_LEN);
		NdisMoveMemory(buf, arg, strlen(arg));
		bufLen = (UCHAR)strlen(arg);

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<==(%s)Config string is \"%s\"\n", __FUNCTION__, buf));

	RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_CUSTOM,
						OID_AWND_CFG_EVENT, NULL, (PUCHAR)buf, bufLen);
	}
	else
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("<==(%s)Lenth of config string must not large than %d!\n", __FUNCTION__, CONFIG_STR_LEN));
	}

	return TRUE;
}

INT Set_ApCli_Enable_From_Awnd_Proc(
	IN  PRTMP_ADAPTER pAd,
	IN  RTMP_STRING *arg)
{
	UINT Enable;
	...
	Enable = os_str_tol(arg, 0, 16);
	pAd->StaCfg[ifIndex].ApcliInfStat.AwndEnableAutoConnect = (Enable > 0) ? TRUE : FALSE;
    	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("I/F(apclii%d) Set_ApCli_Enable_From_Awnd_Proc::(AwndEnableAutoConnect = %d)\n",
			  ifIndex,
			  pAd->StaCfg[ifIndex].ApcliInfStat.AwndEnableAutoConnect));
	return TRUE;
}
#endif
	...
	IN  RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("I/F(apclii%d) Set_ApCli_Enable_Proc::(enable = %d)\n",
			  ifIndex,
	...
	IN  RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Set_ApCli_Bssid_Proc (%2X:%2X:%2X:%2X:%2X:%2X)\n",
	...
	IN RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("I/F(apclii%d) Set_ApCli_AutoConnect_Proc::(Len=%d,Ssid=%s)\n",
...
#endif /* DOT11W_PMF_SUPPORT */
	{
		...
		for (i = 1; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
			...
			if (IS_ENTRY_CLIENT(pEntry)) {
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(%d): MacTableDeleteEntry %02x:%02x:%02x:%02x:%02x:%02x\n",
					__func__, __LINE__, PRINT_MAC(pEntry->Addr)));
	...
	IN	ULONG					Data)
{
	...
	case CMD_RTPRIV_IOCTL_GET_MAC_TABLE:
		if (subcmd == 0)
            ...
            RTMPIoctlGetMacTableStat(pAd, wrq);
/* added for apsd by JiangZheyu 2019/3/5 */
#ifdef APSD_SUPPORT
#ifdef MAC_REPEATER_SUPPORT
        else if (subcmd == 2)
            RTMPIoctlGetReptCliMapTable(pAd, wrq);
#endif
#endif
		break;
...
#ifdef AWND_SUPPORT
	case CMD_RTPRIV_IOCTL_GSITESURVEY_DECO:
		RTMPIoctlGetSiteSurveyDeco(pAd, wrq);
		break;

    case CMD_RTPRIV_IOCTL_GET_CONFIG_GENERIC:
        RTMPIoctlGetConfigGeneric(pAd, wrq);
        break;
#endif /* AWND_SUPPORT */
...
/* modified for apsd by JiangZheyu 2019/3/5 */
#ifdef APSD_SUPPORT
    { 
        UCHAR subcmd = *((UCHAR*)pData);
        UCHAR *staAddr = (UCHAR*)pData + 1;
        if (subcmd == 0)
            APPeerDisassocRootAp(pAd, staAddr);
        else
            APEnableMACEntry(pAd, staAddr);
    }
		break;
#else
	{
...
/* added for apsd by JiangZheyu 2019/3/5 */ 
#ifdef AP_QLOAD_SUPPORT
	case CMD_RTPRIV_IOCTL_CHAN_UTILIZATION:
	{
		QBSS_LoadUpdate(pAd, 0);
#ifdef APSD_SUPPORT
		copy_to_user(pData, &(pAd->phy_ctrl.QloadChanUtil), sizeof(UINT8));
#endif
		wrq->u.data.length = sizeof(UINT8);
	}
		break;
#endif
	case CMD_RTPRIV_IOCTL_GET_CHANNEL_LIST:
		RTMPIoctlGetChannelList(pAd,wrq);
		break;
	case CMD_RTPRIV_IOCTL_GET_STA_INFO:
				...
				for (i = 0; i < MAX_NUM_OF_MONITOR_STA; i++) {
					...
					if (pMacEntry) {
						...
						if (pMacEntry->mnt_band == 0) { /* no more use for other band */
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
									 ("%s(%d)::call MacTableDeleteEntry(WCID=%d)- %02X:%02X:%02X:%02X:%02X:%02X\n",
									  __func__, __LINE__, pMacEntry->wcid, PRINT_MAC(pMacEntry->Addr)));
	...
	if (MAC_ADDR_EQUAL(ZERO_MAC_ADDR, pAd->curMntAddr)) {
		if (pMntEntry->bValid) {
			...
			if (pMacEntry) {
				...
				if (pMacEntry->mnt_band == 0) /* no more use for other band */
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: MacTableDeleteEntry %02x:%02x:%02x:%02x:%02x:%02x\n",
