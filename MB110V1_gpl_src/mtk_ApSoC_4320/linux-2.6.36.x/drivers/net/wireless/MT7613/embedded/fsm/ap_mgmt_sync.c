...
#ifdef WAPP_SUPPORT
	//Just delete to reduce too much serial print in cycle by nrd
	...
	if ((ProbeReqParam->report_param.vendor_ie.element_id == IE_VENDOR_SPECIFIC) &&
		...
		RtmpOSWrielessEventSend(wdev->if_dev, RT_WLAN_EVENT_CUSTOM, RT_PROBE_REQ_REPORT_EVENT,
				   ...
				   && (ProbeReqParam->report_param.vendor_ie.len > 0))
		   {
				   ...
				   pProbeReqReportTemp.vendor_ie.element_id = ProbeReqParam->report_param.vendor_ie.element_id;
				   pProbeReqReportTemp.vendor_ie.len = ProbeReqParam->report_param.vendor_ie.len;
				   NdisMoveMemory(pProbeReqReportTemp.vendor_ie.custom_ie, ProbeReqParam->report_param.vendor_ie.custom_ie,
												   ProbeReqParam->report_param.vendor_ie.len);
				   RtmpOSWrielessEventSend(wdev->if_dev, RT_WLAN_EVENT_CUSTOM, RT_PROBE_REQ_REPORT_EVENT,
								   NULL, (PUCHAR)&pProbeReqReportTemp, MAC_ADDR_LEN + 2 + ProbeReqParam->report_param.vendor_ie.len);
		   }
#endif /* TP_IE_SUPPORT */
	...
	*/
...
#ifdef TP_IE_SUPPORT
	RTMP_SPIN_LOCK_IRQSAVE(&pAd->ApCfg.MBSSID[wdev->func_idx].ap_vendor_ie.vendor_ie_lock, &flags);

	if (pAd->ApCfg.MBSSID[wdev->func_idx].ap_vendor_ie.pointer != NULL) 
	{
		   MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen, 
												   pAd->ApCfg.MBSSID[wdev->func_idx].ap_vendor_ie.length,  
												   pAd->ApCfg.MBSSID[wdev->func_idx].ap_vendor_ie.pointer,
												   END_OF_ARGS);
		   FrameLen += TmpLen;
	}
	RTMP_SPIN_UNLOCK_IRQRESTORE(&pAd->ApCfg.MBSSID[wdev->func_idx].ap_vendor_ie.vendor_ie_lock, &flags);
#endif /* TP_IE_SUPPORT */
	...
	{	
		if (pEntry->func_tb_idx == MAIN_MBSSID)
		{
			...
			memcpy(probe.sender_addr, ProbeReqParam->Addr2, MAC_ADDR_LEN);
			//printk("%s %d call func mtk_deliver_band_steering_event to send ATH_EVENT_BSTEERING_PROBE_REQ\r\n", __FUNCTION__, __LINE__);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s %d probe.rssi = %d\r\n", __FUNCTION__, __LINE__, probe.rssi));
			mtk_deliver_band_steering_event(RtmpOsGetNetIfIndex(pAd->ApCfg.MBSSID[wdev->func_idx].wdev.if_dev), ATH_EVENT_BSTEERING_PROBE_REQ, sizeof(probe), (u_int8_t*)&probe);
		}
	}	
#endif
