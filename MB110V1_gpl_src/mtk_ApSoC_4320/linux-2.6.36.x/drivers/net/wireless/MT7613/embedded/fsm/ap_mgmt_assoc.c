...
/* 2020.7.3 mjz: 11v does not depend on HT */
	...
	IE_LISTS *ie_lists)
{
	...
	while (((UCHAR *)eid_ptr + eid_ptr->Len + 1) < ((UCHAR *)Fr + MsgLen)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MT7613->wifi eid_ptr->Eid is %d\n", eid_ptr->Eid));
		switch (eid_ptr->Eid) {
		case IE_SSID:
			if (((Sanity & 0x1) == 1))
				break;
...
#endif /* MBO_SUPPORT */
#if defined(MWDS) && !defined(ONEMESH_BACKHAUL_4ADDR)
...
#ifdef TP_NETWORK_ROAMING
extern void mtk_deliver_band_steering_event(u_int32_t ifindex, int type, int len, u_int8_t* data);
...
#ifdef APSD_SUPPORT
/* Used to represent an associated station*/
typedef struct _AssocSta
{
    uint8_t mac[MAC_ADDR_LEN];
    /* Represent the band of the station associated with. 1 means 2.4G band, and 0 means 5G */
    uint8_t band;
} AssocSta;

static VOID RTMPSendWirelessEventForAPSD(
	IN VOID *pAdSrc,
	...
	IN CHAR Rssi)
{
	...
	memcpy(assocsta.mac, (u_int8_t*)pAddr, MAC_ADDR_LEN);
	assocsta.band = 1; /* 1 means this is 5G sta */
	RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_CUSTOM, Event_flag, NULL, (PUCHAR)&assocsta, sizeof(assocsta));
#else
...
SendAssocResponse:
...
#ifdef ONEMESH_BACKHAUL_4ADDR
		if (pEntry->func_tb_idx == BACKHAUL_MBSSID) {
			pEntry->DevPeerRole |= BIT(MAP_ROLE_BACKHAUL_STA);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("onemesh forces to support 4addr on backhaul\n"));
		} else {
			pEntry->DevPeerRole &= (~BIT(MAP_ROLE_BACKHAUL_STA));
		}
#endif
	...
	if (IS_RRM_ENABLE(wdev))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("MT7613-->wifi RRM is enabled\n"));
	}
	else
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("MT7613-->wifi RRM is not enabled\n"));
	}
...
assoc_post:
	...
	if (pEntry && tr_entry && (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED)) {
		...
		{
			memcpy(&assoc.client_addr, ie_list->Addr2, MAC_ADDR_LEN);
			if (ie_list->ExtCapInfo.BssTransitionManmt)
					assoc.isBTMSupported = 1;
			else
					assoc.isBTMSupported = 0;

			//assoc.isBTMSupported = 1;
			memcpy(&octet1, (unsigned char *)(&pEntry->RrmEnCap.word), sizeof(unsigned char));
			//printk("%s %d RrmEnCap.word = %llu, octet1 = 0x%x\r\n", __FUNCTION__, __LINE__, pEntry->RrmEnCap.word, octet1);
			assoc.isRRMSupported = (octet1 != 0) ? 1 : 0;
			/*
			...
			*/
			/*
			max_chwidth compare to qca type:
			enum cwm_width {
			CWM_WIDTH20,
			CWM_WIDTH40
			};
			*/
			assoc.datarate_info.max_chwidth = pEntry->MaxHTPhyMode.field.BW;
			assoc.datarate_info.num_streams = mcs_to_numstreams(pEntry->MaxHTPhyMode.field.MCS);;
			//assoc.datarate_info.phymode = pEntry->MaxHTPhyMode.field.MODE; /*MODE is MODE_CCK or MODE_OFDM, But we need is HT20, HT40*/
			if (pEntry->MaxHTPhyMode.field.BW == BW_20 || pEntry->MaxHTPhyMode.field.BW == BW_40)
			{
				assoc.datarate_info.phymode = wlanif_phymode_ht;
			}
			else if (pEntry->MaxHTPhyMode.field.BW == BW_80)
			{
				assoc.datarate_info.phymode = wlanif_phymode_vht;
			}
			else
			{
				assoc.datarate_info.phymode = wlanif_phymode_basic;
			}
			assoc.datarate_info.max_txpower = GetCuntryMaxTxPwr(pAd, pEntry->wdev->PhyMode, pEntry->wdev, op_ht_bw);
			assoc.datarate_info.max_MCS = pEntry->MaxHTPhyMode.field.MCS;
			assoc.datarate_info.is_static_smps = 1;
			assoc.datarate_info.is_mu_mimo_supported = pAd->CommonCfg.bMIMOPSEnable;
			assoc.band_cap = 0;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s %d\n assoc.client_addr = %02x:%02x:%02x:%02x:%02x:%02x,\r\n assoc.isBTMSupported = %d, \r\n assoc.isRRMSupported = %d,\r\n assoc.datarate_info.max_chwidth = %d,\r\n assoc.datarate_info.num_streams = %d,\r\n assoc.datarate_info.phymode = %d,\r\n assoc.datarate_info.max_txpower = %d,\r\n assoc.datarate_info.max_MCS = %d\r\n", 
				__FUNCTION__, __LINE__, 
				PRINT_MAC(assoc.client_addr),
				assoc.isBTMSupported, 
				assoc.isRRMSupported, 
				assoc.datarate_info.max_chwidth, 
				assoc.datarate_info.num_streams,
				assoc.datarate_info.phymode, 
				assoc.datarate_info.max_txpower,
				assoc.datarate_info.max_MCS));

			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s %d send ATH_EVENT_BSTEERING_NODE_ASSOCIATED\r\n", __FUNCTION__, __LINE__));
			mtk_deliver_band_steering_event(RtmpOsGetNetIfIndex(wdev->if_dev), ATH_EVENT_BSTEERING_NODE_ASSOCIATED, sizeof(assoc), (u_int8_t*)&assoc);
		}
#endif
...
#ifdef APSD_SUPPORT
		RTMPSendWirelessEventForAPSD(pAd, IW_ASSOC_EVENT_FLAG, pEntry->Addr, 0, 0);
#else
...
static VOID ap_peer_disassoc_action(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (VALID_UCAST_ENTRY_WCID(pAd, Elem->Wcid)) {
		...
		*/
...
#ifdef APSD_SUPPORT
		RTMPSendWirelessEventForAPSD(pAd, IW_DISASSOC_EVENT_FLAG, Addr2, 0, 0);
#else
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(%d): MacTableDeleteEntry %02x:%02x:%02x:%02x:%02x:%02x\n",
			__func__, __LINE__, PRINT_MAC(Addr2)));
...
#ifdef APSD_SUPPORT
void APPeerDisassocRootAp(RTMP_ADAPTER *pAd, UCHAR* staAddr)
{
	UCHAR Addr2[MAC_ADDR_LEN];

    printk("=======>%s:%d disassoc sta %02x:%02x:%02x:%02x:%02x:%02x from rootAp\n",
           __func__, __LINE__, staAddr[0], staAddr[1], staAddr[2], staAddr[3],
           staAddr[4], staAddr[5]);

    memcpy(Addr2, staAddr, MAC_ADDR_LEN);

    /*
     * 1. Remove the repcli entry
     */
#ifdef MAC_REPEATER_SUPPORT
		if (pAd->ApCfg.bMACRepeaterEn == TRUE)
		{
			...
			if (pReptEntry && (pReptEntry->CliConnectState != 0))
			{
				apCliIdx = pReptEntry->MatchApCliIdx;
				CliIdx = pReptEntry->MatchLinkIdx;
                printk("Now going to remove APCLI connect");
    ...
    /*
     * 2. Add this entry to repcli black list
     */
#ifdef MAC_REPEATER_SUPPORT
    if (RTMPRepeaterVaildMacEntry(pAd, Addr2))
         InsertIgnoreAsRepeaterEntryTable(pAd, Addr2);
#endif /* MAC_REPEATER_SUPPORT */
}

/* added for apsd by JiangZheyu 2019/2/21 */
VOID APEnableMACEntry(RTMP_ADAPTER *pAd, UCHAR *staAddr)
{
#ifdef MAC_REPEATER_SUPPORT
	INVAILD_TRIGGER_MAC_ENTRY *pSkipEntry = NULL;

    printk("===============>%s: %d, enable client: %02x: %02x: %02x: %02x: %02x:%02x\n",
            __func__, __LINE__, staAddr[0], staAddr[1], staAddr[2], staAddr[3], staAddr[4],
            staAddr[5]);
    pSkipEntry = RepeaterInvaildMacLookup(pAd, staAddr);
    if (pSkipEntry)
    {
...
#ifdef APSD_SUPPORT
		RTMPSendWirelessEventForAPSD(pAd, IW_DISASSOC_EVENT_FLAG, pStaAddr, 0, 0);
#else
