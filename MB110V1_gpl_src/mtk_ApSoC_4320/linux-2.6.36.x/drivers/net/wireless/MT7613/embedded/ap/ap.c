...
#ifdef APCLI_SUPPORT
/* added for apsd by JiangZheyu 2019/3/25 */
		if((IS_ENTRY_APCLI(pEntry) || IS_ENTRY_CLIENT(pEntry))
			&& (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED))
		{
			pEntry->actualRate = (((pEntry->OneSecTxBytes + pEntry->OneSecRxBytes) << 3) >> 20);
			pEntry->OneSecTxBytesTotal = pEntry->OneSecTxBytes;
			pEntry->OneSecRxBytesTotal = pEntry->OneSecRxBytes;
		}
		...
		*/
		...
		if (pEntry->NoDataIdleCount >= pEntry->StaIdleTimeout) {
			bDisconnectSta = TRUE;
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ageout %02x:%02x:%02x:%02x:%02x:%02x after %d-sec silence\n",
			...
			*/
			if (pEntry->PsMode != PWR_SAVE) {
				bDisconnectSta = TRUE;
				MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("STA-%02x:%02x:%02x:%02x:%02x:%02x had left (%d %lu)\n",
		...
		if (bDisconnectSta) {
			...
			if (pEntry->Sst == SST_ASSOC) {
				...
				MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Send DEAUTH - Reason = %d frame  TO %x %x %x %x %x %x\n",
