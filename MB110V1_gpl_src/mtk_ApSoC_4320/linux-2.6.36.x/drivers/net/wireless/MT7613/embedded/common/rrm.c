...
VOID RRM_Report_IE_Sanity(PRRM_BEACON_REP_INFO pRRM_Beacon_Rep_Info, pieee80211_bcnrpt_t pIEEE80211_Bcnrpt)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pIEEE80211_Bcnrpt->bssid=%02x:%02x:%02x:%02x:%02x:%02x\n, pIEEE80211_Bcnrpt->chnum = %d, pIEEE80211_Bcnrpt->rcpi = %d, pIEEE80211_Bcnrpt->rsni = %d\r\n",
...
//		wapp_send_bcn_report_complete(pAd, pEntry);
...
#ifdef TP_NETWORK_ROAMING
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(tproaming) %s %d bcnrptIdx = %d\r\n", __FUNCTION__, __LINE__, bcnrptIdx));
	...
	if (bcnrptIdx >= 0)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(tproaming) %s %d : bcnrptIdx[%d], values : bcnrpt.bssid=%02x:%02x:%02x:%02x:%02x:%02x, bcnrpt.chnum = %d, bcnrpt.rcpi = %d, bcnrpt.rsni = %d\r\n",
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(tproaming) %s %d wext_send_bcn_rsp_event to send ATH_EVENT_BSTEERING_RRM_REPORT, rrm.macaddr = %02x:%02x:%02x:%02x:%02x:%02x\r\n",
...
void init_rrm_capabilities(PRRM_CONFIG pRrmCfg, BSS_STRUCT *pMBss)
{
	...
	pRrmCfg->max_rrm_capabilities.field.LinkMeasureCap = 1;
	...
	pRrmCfg->max_rrm_capabilities.field.TransmitStreamCap = 1;
	pRrmCfg->max_rrm_capabilities.field.TriggeredTransmitStreamCap = 1;
	pRrmCfg->max_rrm_capabilities.field.APChannelReportCap = 1;
	pRrmCfg->max_rrm_capabilities.field.NeighReportTSFOffsetCap = pRrmCfg->bDot11kRRMNeighborRepTSFEnable ? 1 : 0;
	pRrmCfg->rrm_capabilities.word = pRrmCfg->max_rrm_capabilities.word;
}
