		...
		if (!bUPnPMsg &&
			...
			*/
			...
			if ((CurOpMode == AP_MODE) && (bReSetWscIE)  &&
			    ((!pWscControl->WscPinCode) || ((pWscControl->WscState == WSC_STATE_WAIT_M8) && (pWscControl->WscStatus == STATUS_WSC_CONFIGURED)))) {
...
#if  defined(CONFIG_MAP_SUPPORT) && defined(CONFIG_STA_SUPPORT)
...
#ifdef WSC_V2_SUPPORT
			|| (pWscControl->WscV2Info.bEnableWpsV2 && ((CurOpMode == AP_MODE) && !pWscControl->bSetupLock) && !sharedWscLock)
#endif /* WSC_V2_SUPPORT */
#endif /* CONFIG_AP_SUPPORT */
		   ) {
			pWscControl->WscStatus = STATUS_WSC_CONFIGURED;
...
irqreturn_t WSC_HDR_BTN_HANDLER_5G(int irq, void *pAd, struct pt_regs *regs)
{
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			if(((PRTMP_ADAPTER)pAd)->net_dev->flags & IFF_UP)
			{
	    		...
	    		pObj->ioctl_if = 0;
				Set_AP_WscMode_Proc((IN	PRTMP_ADAPTER)pAd, (RTMP_STRING *)"2"); /* 2: PBC */
				Set_AP_WscGetConf_Proc((IN	PRTMP_ADAPTER)pAd, (RTMP_STRING *)"1"); /* 1: Trigger */
			}
		}
#endif // CONFIG_AP_SUPPORT //
	return IRQ_NONE;
