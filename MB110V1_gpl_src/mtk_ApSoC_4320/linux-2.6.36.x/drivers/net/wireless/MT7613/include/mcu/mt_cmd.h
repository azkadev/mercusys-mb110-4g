...
typedef struct _EXT_CMD_THERMAL_PROTECT_T {
	...
	UINT8 i1HighTempThreshold;		/* if (current temperature >= cHighTempThreshold) FW notify driver with EXT_EVENT_THERMAL_PROTECT */
	UINT8 u1LowEnable;				/* 0: Disable Low temperature event to driver 1: Enable */
	UINT8 i1LowTempThreshold;		/* if (current temperature <= cLowTempThreshold) FW notify driver with EXT_EVENT_THERMAL_PROTECT */
	UINT32 u4RecheckTimer;			/* Set recheck timer in Sec */
	UINT8 u1RFOffEnable;			/* 0: Disable RFOff event to driver 1: Enable */
	UINT8 i1RFOffThreshold;			/* if (current temperature >= cRFOffThreshold) FW notify driver with EXT_EVENT_THERMAL_PROTECT; */
